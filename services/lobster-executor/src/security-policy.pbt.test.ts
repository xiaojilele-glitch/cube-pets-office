...
const nonRootUserArb = fc
  .string({
    minLength: 1,
    maxLength: 10,
    unit: fc.constantFrom(..."abcdefghijklmnopqrstuvwxyz123456789"),
  })
  .filter(s => s !== "root" && s !== "0");
    ...
    fc.assert(
      fc.property(
        ...
        (level, memBytes, cpu, pids) => {
          ...
          expect(hc.NanoCpus).toBe(
            Math.round(Number.parseFloat(cpuStr) * 1_000_000_000)
          );
...
describe("Property 8: 网络白名单解析正确性", () => {
  it("for any comma-separated string, parseNetworkWhitelist correctly splits and trims", () => {
    ...
    const domainArb = fc
      ...
      .filter(
        s => s.trim().length > 0 && !s.startsWith(".") && !s.endsWith(".")
      );
...
describe("Property 12: 敏感路径禁止挂载", () => {
  it("for any Binds config, SENSITIVE_HOST_PATHS should never appear", () => {
    ...
    const safeDirArb = fc.constantFrom(
      "/app/data",
      "/home/user",
      "/opt/work",
      "/mnt/storage"
    );
    const containerDirArb = fc.constantFrom(
      "/workspace",
      "/data",
      "/mnt",
      "/opt"
    );
    const safeBindArb = fc
      .tuple(safeDirArb, containerDirArb)
    ...
    const maybeSensitiveBindArb = fc
      .tuple(sensitivePathArb, containerDirArb)
      .map(([host, container]) => `${host}:${container}`);
    ...
    fc.assert(
      fc.property(
        bindsArb,
        fc.boolean(),
        maybeSensitiveBindArb,
        (safeBins, injectSensitive, sensitiveBind) => {
          const binds = injectSensitive
            ? [...safeBins, sensitiveBind]
            : safeBins;
