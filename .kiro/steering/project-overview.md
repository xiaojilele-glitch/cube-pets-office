...
│       └── agent-autonomy-upgrade/ # Agent 自治能力升级（自评估/竞争执行/动态协作）
│
...
| Agent 自治能力升级 | `.kiro/specs/agent-autonomy-upgrade/` | `shared/autonomy-types.ts` `shared/ring-buffer.ts` `server/core/capability-profile-manager.ts` `server/core/self-assessment.ts` `server/core/task-allocator.ts` `server/core/competition-engine.ts` `server/core/judge-agent.ts` `server/core/taskforce-manager.ts` `server/core/cost-monitor.ts` `client/src/lib/autonomy-store.ts` | 🔲 待开发 |
