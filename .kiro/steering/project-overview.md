...
| 安全沙箱 | `.kiro/specs/secure-sandbox/` | `services/lobster-executor/src/security-policy.ts` `services/lobster-executor/src/security-audit.ts` `services/lobster-executor/seccomp.json` `shared/executor/contracts.ts` | 🔲 待开发 |
...
9. **secure-sandbox** — 为 Lobster Executor 添加生产级安全沙箱（容器权限降级、资源限制、网络隔离、seccomp 过滤、审计日志）。依赖 lobster-executor-real 完成后启动，与 sandbox-live-preview 可并行。
