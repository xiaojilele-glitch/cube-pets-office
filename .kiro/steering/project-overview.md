...
1. **Frontend Mode 体验极致化** — 让 Live Demo 打开即可看到完整的多智能体协作过程在 3D 场景里跑起来，不需要任何配置。当前 Frontend Mode 已可交互，但缺少预置的演示数据和引导流程。
2. **Docker 执行器从 mock 升级为真实实现** — 这是"不只是生成报告"这个卖点的关键支撑。
3. **`/api/planets` 路由实现** — 补齐 mission 原生数据源，让任务驾驶舱完全脱离 workflow 投影层。
