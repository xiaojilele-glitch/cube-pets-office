...
| 审计链 / 不可篡改日志 | `.kiro/specs/audit-chain/` | `shared/audit/contracts.ts` `server/audit/audit-chain.ts` `server/audit/audit-store.ts` `server/audit/audit-collector.ts` `server/audit/audit-verifier.ts` `server/audit/audit-query.ts` `server/audit/anomaly-detector.ts` `server/audit/compliance-mapper.ts` `server/routes/audit.ts` `client/src/components/audit/AuditPanel.tsx` `client/src/lib/audit-store.ts` | 🔲 待开发 |
