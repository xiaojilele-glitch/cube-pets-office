...
│       │   ├── tasks-store.ts       # Mission store（mission-native 单源架构）
...
│       └── multi-region-disaster-recovery/ # 多区域灾难恢复
│
...
| Workflow 寄生依赖解耦 | `.kiro/specs/workflow-decoupling/` | `client/src/lib/tasks-store.ts` `server/core/mission-orchestrator.ts` `shared/mission/contracts.ts` | ✅ 已完成（tasks-store 已切换为 mission-native 单源） |
...
3. **Work Packages / Agent Crew 已迁移到 mission 原生数据** — tasks-store 已完成 workflow 寄生依赖解耦，所有任务驾驶舱数据均从 MissionRecord 原生字段（organization、workPackages、messageLog、agentCrew）读取。workflow-store 和 WorkflowPanel 作为独立的工作流视图继续运行，与 tasks-store 零交叉。
...
1. **~~workflow-decoupling~~** ✅ 已完成 — tasks-store 已切换为 mission-native 单源架构，workflow 寄生依赖已清除。
...
4. **mission-native-projection 后端** — MissionRecord 丰富化已由 workflow-decoupling 完成，剩余工作为实现 /api/planets 路由。
5. **demo-guided-experience** — 依赖 demo-data-engine 的 DEMO_BUNDLE。构建回放引擎 + Live Demo 入口 + 记忆/进化可视化。

### 第三梯队（依赖第二梯队完成）

6. **mission-native-projection 前端迁移** — 切换 mission-client 到 /api/planets 端点。
