...
- [ ] 1. 扩展共享类型定义
  - [ ] 1.1 扩展 WorkflowInputAttachment 接口，新增 visionReady、base64DataUrl、visualDescription 可选字段；扩展 WorkflowAttachmentExcerptStatus 类型新增 "vision_analyzed" 和 "vision_fallback"
    - 修改 `shared/workflow-input.ts`
    - 确保 normalizeWorkflowAttachment 函数兼容新字段（可选字段不影响现有逻辑）
    - _Requirements: 1.1, 1.2, 4.2_
  - [ ] 1.2 扩展 LLMMessage 接口，content 字段支持 string | LLMMessageContentPart[] 联合类型；新增 LLMMessageContentPart 类型定义
...
- [ ] 2. 实现 Vision Provider 配置与调用
  - [ ] 2.1 新增 Vision 配置读取函数 getVisionConfig()，实现 VISION_LLM_* → FALLBACK_LLM_* → LLM_* 的 fallback 链
  ...
  - [ ] 2.3 实现 analyzeImage 和 analyzeImages 函数，复用 llm-client 的调用机制发送多模态消息到 Vision LLM
...
- [ ] 3. 扩展 LLM Client 多模态消息支持
  - [ ] 3.1 修改 llm-client.ts 中的 createChatCompletion 函数，当 LLMMessage.content 为数组时直接传递到请求体
    - 修改 `server/core/llm-client.ts`
    - 在构建 body.messages 时检查 content 类型，数组格式直接传递
    - _Requirements: 3.2_
  - [ ] 3.2 修改 llm-client.ts 中的 buildResponsesInput 函数，将 image_url 条目转换为 responses API 的 input_image 格式
    - 修改 `server/core/llm-client.ts`
...
- [ ] 5. 扩展前端附件管道
  - [ ] 5.1 实现图片 Base64 编码函数 fileToBase64DataUrl，包含大图压缩逻辑（超过 4MB 时使用 Canvas 降采样）
  ...
  - [ ] 5.3 扩展 parseImageFile 函数，增加 Vision 分析路径：先编码 Base64，通过服务端 API 请求 Vision 分析，失败时回退到 OCR
    - 修改 `client/src/lib/workflow-attachments.ts`
...
- [ ] 6. 实现 Vision 分析 API 路由
  - [ ] 6.1 新建 POST /api/vision/analyze 路由，接收 base64 图片数组，调用 vision-provider 返回分析结果
...
- [ ] 7. 扩展 Agent 上下文注入
  - [ ] 7.1 扩展 AgentInvokeOptions 接口，新增 visionContexts 可选字段；修改 composeAgentMessages 函数，在用户 prompt 之前注入 Visual_Context 消息
  ...
  - [ ] 7.3 修改 RuntimeAgent.invoke 方法，当 options 包含 visionContexts 时增加 maxTokens
    - 修改 `shared/runtime-agent.ts`
...
- [ ] 8. 扩展工作流指令上下文构建
  - [ ] 8.1 修改 buildWorkflowDirectiveContext 函数，当附件包含 visualDescription 时在附件区段中输出 "[Vision Analysis]" 格式的视觉分析内容
    - 修改 `shared/workflow-input.ts`
...
- [ ] 10. 3D 场景"看图"状态
  - [ ] 10.1 在 PetWorkers.tsx 的 STATUS_BUBBLES 中新增 analyzing_image 状态文案（中英文）；在 animateWorker 中新增 examining 动画类型
    - 修改 `client/src/components/three/PetWorkers.tsx`
    - 中文："正在看图...\n让我仔细看看这张图。"
    - 英文："Analyzing image...\nLet me take a closer look."
    - examining 动画：轻微前倾 + 左右扫视
    - _Requirements: 6.2, 6.4_
  - [ ] 10.2 在 RuntimeAgent 的 Vision 分析流程中发出 analyzing_image 和 idle 事件
...
- [ ] 11. 环境变量与文档更新
  - [ ] 11.1 在 .env.example 中新增 VISION_LLM_* 系列环境变量模板和注释
    - 修改 `.env.example`
    ...
    - _Requirements: 2.1_
  - [ ] 11.2 更新 .kiro/steering/project-overview.md 模块清单表格，新增 multi-modal-vision 行
