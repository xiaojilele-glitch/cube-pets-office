...
- [x] 2. 重构主导航组件
  - [x] 2.1 更新 `client/src/components/Toolbar.tsx`
    - 收敛为 `办公室 / 任务 / 更多`
    - 保留 active 状态与可访问性
    - _Requirements: 1.1.1, 1.1.3_
  - [x] 2.2 若仍使用 HoloDock 变体，统一处理 `client/src/components/HoloDock.tsx`
    - 避免遗留双套导航语义
    - _Requirements: 1.1.2_

- [x] 3. 实现 `更多` 收纳层
  - [x] 3.1 新增 `MoreDrawer` 或等效组件
    - 收纳配置、权限、审计、血缘、帮助
    - _Requirements: 4.1.1, 4.1.2_
  - [x] 3.2 为低频页面补充返回主路径入口
    - _Requirements: 4.1.3, 5.1.3_

- [x] 4. 更新首页与路由骨架
  - [x] 4.1 更新 `client/src/pages/Home.tsx`
    - 明确 `办公室` 首页定位
    - 增加进入 `任务` 的 CTA 或显式跳转入口
    - _Requirements: 2.1.1, 2.1.3_
  - [x] 4.2 更新 `client/src/App.tsx`
    - 保留旧路由
    - 接入新导航语义
    - _Requirements: 5.1.1, 5.1.2_
