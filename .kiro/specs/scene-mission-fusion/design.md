...
_对任意_ Mission 列表（包含不同状态的 Mission），`selectDisplayMission` 函数应遵循以下优先级规则：
...
_对任意_ 字符串 `title` 和最大长度 `maxLength = 40`：
...
_对任意_ Agent 列表和最大数量 `maxCount = 3`：
...
_对任意_ 事件列表，Detail Overlay 显示的事件数量 ≤ 10，且当原始列表长度 > 10 时，显示的是最近的 10 条事件（按时间降序）。
