...
- [x] 1. 实现 ExecutionBridge 核心模块
  - [x] 1.1 创建 `server/core/execution-bridge.ts`，实现 `ExecutionBridge` 类
  ...
  - [x] 1.2 编写 detectExecutable 属性测试
    - **Property 1: 可执行内容检测**
    - **Property 2: 非可执行内容跳过**
    - **Property 3: Metadata 强制覆盖**
    - **Validates: Requirements 1.1, 1.2, 1.3, 1.4**

  - [x] 1.3 编写 ExecutionPlan 构建属性测试
    - **Property 4: ExecutionPlan 构建不变量**
    - **Validates: Requirements 2.1, 2.2, 2.4**

  - [x] 1.4 编写模式 payload 属性测试
    - **Property 8: 模式特定 payload 注入**
    - **Validates: Requirements 7.1, 7.2**

- [x] 2. 集成 ExecutionBridge 到 WorkflowEngine
  - [x] 2.1 修改 `server/core/workflow-engine.ts`，在 `runPipeline` 中 `runExecution` 完成后插入 `bridgeToExecutor` 调用
  ...
  - [x] 2.2 编写 bridgeToExecutor 单元测试
    - 测试有可执行产物时触发桥接
    - 测试无可执行产物时跳过桥接
    - 测试桥接失败时记录 issue 并继续
...
- [x] 4. 实现分发后状态更新与回调 URL 构建
  - [x] 4.1 在 `ExecutionBridge.bridge()` 中实现分发成功后的 Mission 状态更新
  ...
  - [x] 4.2 编写分发后状态一致性属性测试
    - **Property 5: 分发后 executor 上下文一致性**
    - **Validates: Requirements 3.2**

  - [x] 4.3 编写 callback URL 属性测试
    - **Property 6: Callback URL 构建正确性**
    - **Validates: Requirements 3.5**

- [x] 5. 实现错误处理与心跳超时
  - [x] 5.1 在 `ExecutionBridge` 中实现完整错误处理
...
- [x] 7. 验证事件回流映射（现有机制）
  - [x] 7.1 审查并补充 `server/index.ts` 中 `/api/executor/events` 处理逻辑
...
- [x] 8. 实现前端 ExecutorStatusPanel 组件
  - [x] 8.1 创建 `ExecutorStatusPanel` 组件
  ...
  - [x] 8.2 集成 Socket.IO 日志流到终端面板
    - 监听 job.log_stream 事件并实时显示
    - 区分 stdout/stderr 流
    - _Requirements: 5.4_

- [x] 9. 端到端集成与最终验证
  - [x] 9.1 在 `server/index.ts` 中初始化 ExecutionBridge 实例并注入到 WorkflowEngine 运行时
    - 从环境变量读取 executorBaseUrl、executionMode、defaultImage
    - 构建 callbackUrl
    - _Requirements: 7.1, 7.2, 7.3_

  - [x] 9.2 编写集成测试：mock 模式完整桥接流程
