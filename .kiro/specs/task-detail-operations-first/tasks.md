...
- [ ] 1. 梳理首屏所需派生数据
  - [ ] 1.1 在 `client/src/lib/tasks-store.ts` 或 `client/src/components/tasks/task-helpers.ts` 中新增派生函数
...
- [ ] 2. 新增首屏摘要组件
  - [ ] 2.1 创建 `TaskOperationsHero` 或等效组件
...
- [ ] 3. 接入主操作条
  - [ ] 3.1 将 `OperatorActionBar` 集成到任务详情页首屏
    - 危险动作与普通动作分层
    - 无动作时展示被动提示
    - _Requirements: 1.1.2, 1.1.3, 1.1.4_

  - [ ] 3.2 接入 waiting decision 场景
    - 在首屏显示待决策提示
    - 保持与 `DecisionPanel` 协同
    - _Requirements: 3.1.2, 4.1.3_

- [ ] 4. 重排 `TaskDetailView.tsx`
  - [ ] 4.1 将 tabs 区域下移到首屏摘要之后
