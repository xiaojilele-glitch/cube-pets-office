...
- [ ] 1. 扩展配置与类型定义
  - [ ] 1.1 在 `types.ts` 的 `LobsterExecutorConfig` 中新增 `aiImage: string` 字段；在 `LobsterExecutorHealthResponse` 中新增 `aiCapability` 字段（enabled、image、llmProvider）
    - _Requirements: 1.3, 7.3_
  - [ ] 1.2 在 `config.ts` 的 `readLobsterExecutorConfig()` 中读取 `LOBSTER_AI_IMAGE` 环境变量，默认值 `"cube-ai-sandbox:latest"`
    - _Requirements: 1.3_
  - [ ] 1.3 定义 `AIJobPayload` 接口（aiEnabled、aiTaskType、llmConfig）和 `AIResultArtifact` 接口，放入 `types.ts`
    - _Requirements: 1.4, 4.1, 5.4_

- [ ] 2. 实现 CredentialInjector 模块
  - [ ] 2.1 创建 `credential-injector.ts`，实现 `resolveAICredentials(payload, hostEnv)`、`buildAIEnvVars(creds)`、`validateCredentials(creds)` 三个函数
  ...
  - [ ]* 2.2 编写属性测试：凭证解析与覆盖优先级
    - **Property 2: 凭证解析与覆盖优先级**
    - **Validates: Requirements 2.1, 2.2, 2.3**
  - [ ]* 2.3 编写属性测试：凭证验证拒绝无效输入
    - **Property 3: 凭证验证拒绝无效输入**
    - **Validates: Requirements 2.4, 2.5**

- [ ] 3. 实现 CredentialScrubber 模块
  - [ ] 3.1 创建 `credential-scrubber.ts`，实现 `CredentialScrubber` 类
  ...
  - [ ]* 3.3 编写属性测试：Artifact 文件凭证清洗
    - **Property 7: Artifact 文件凭证清洗**
    - **Validates: Requirements 6.4**

- [ ] 4. 实现 AI Task Presets 模块
  - [ ] 4.1 创建 `ai-task-presets.ts`，定义 `AITaskPreset` 接口和 `AI_TASK_PRESETS` 常量 map，实现 `getAITaskPreset(taskType)` 函数
    - 四种预设：text-generation、code-generation、data-analysis、image-understanding
    - 未知类型回退到 text-generation
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6_
  - [ ]* 4.2 编写属性测试：AI 任务预设映射
...
- [ ] 6. 扩展 DockerRunner 支持 AI 容器
  - [ ] 6.1 修改 `docker-runner.ts` 的 `buildContainerOptions()` 方法，增加 AI 感知逻辑
  ...
  - [ ] 6.2 修改 `DockerRunner.run()` 方法，AI Job 完成后调用 `CredentialScrubber` 清洗 artifacts 目录和日志
    - _Requirements: 6.4, 6.5_
  - [ ] 6.3 修改 `DockerRunner.emitCompleted()` 和 `emitFailed()`，AI Job 的事件 payload 中包含 aiTaskType、aiModel、aiResult 摘要（contentPreview 截断到 200 字符），事件内容经过 CredentialScrubber 清洗
    - _Requirements: 5.1, 5.3, 6.2_
  - [ ]* 6.4 编写属性测试：AI 镜像选择正确性
    - **Property 1: AI 镜像选择正确性**
    - **Validates: Requirements 1.3, 1.4, 7.1**
  - [ ]* 6.5 编写属性测试：非 AI Job 行为不变
    - **Property 8: 非 AI Job 行为不变**
    - **Validates: Requirements 7.1**
  - [ ]* 6.6 编写属性测试：AI 完成事件 contentPreview 截断
    - **Property 9: AI 完成事件 contentPreview 截断**
    - **Validates: Requirements 5.3**

- [ ] 7. 扩展 MockRunner 支持 AI 模拟
  - [ ] 7.1 修改 `mock-runner.ts`，当 `payload.aiEnabled` 为 true 时生成模拟 AI 响应
    - 返回固定的 MOCK_AI_RESULT（content、usage、model）
    - 写入 ai-result.json artifact
    - 事件 payload 包含 aiResult 摘要
    - _Requirements: 7.4_
  - [ ]* 7.2 编写属性测试：Mock 模式 AI 响应一致性
    - **Property 10: Mock 模式 AI 响应一致性**
    - **Validates: Requirements 7.4**

- [ ] 8. 实现 AI Bridge 容器内模块
  - [ ] 8.1 创建 `ai-bridge/` 目录，实现 `index.js`（Node.js 模块）
