...
- [ ] 1. 扩展数据层：SnapshotRecord 类型与 SnapshotStore
  ...
  - [x] 1.2 扩展 `client/src/lib/browser-runtime-storage.ts`，新增 snapshots object store
...
- [ ] 2. 实现快照序列化与校验
  ...
  - [x] 2.2 创建 `client/src/lib/snapshot-serializer.ts`，封装 Worker 通信和回退逻辑
...
- [ ] 4. 实现快照调度器
  ...
  - [x] 4.2 在 `client/src/runtime/browser-runtime.ts` 中集成 SnapshotScheduler
...
- [ ] 5. 实现自动恢复检测与恢复逻辑
  ...
  - [x] 5.2 在 `client/src/runtime/browser-runtime.ts` 启动流程中集成恢复检测
...
- [ ] 7. 实现会话导出/导入
  ...
  - [x] 7.2 在 `client/src/lib/session-export.ts` 中实现导入功能
...
- [ ] 8. 实现 UI 组件
  ...
  - [x] 8.2 在 `client/src/components/Scene3D.tsx` 中添加恢复状态提示
    - 恢复加载时显示"正在恢复上一次任务…"覆盖层
    - _Requirements: 7.2_

  - [x] 8.3 在配置面板或工作流面板中添加 Export/Import Session 按钮
    - Export Session 按钮调用 exportSession()
    - Import Session 按钮打开文件选择器，调用 importSession(file)
    - _Requirements: 3.1, 4.1_

  - [x] 8.4 在 Tasks 页面添加"历史会话"标签
...
- [ ] 9. 集成与连接
  ...
  - [x] 9.2 在 browser-runtime 中连接快照调度器与 Mission 生命周期
