...
- [ ] 1. 扩展共享类型定义
  - [ ] 1.1 扩展 AgentInvokeOptions 和新增 MultimodalContext 接口
    ...
    - _Requirements: 5.1_
  - [ ] 1.2 扩展 AgentRecord 接口新增 capabilities 字段
    - 修改 `shared/workflow-runtime.ts`
    - 在 `AgentRecord` 接口中新增 `capabilities?: string[]` 可选字段
    - _Requirements: 6.2_
  - [ ] 1.3 扩展 STATUS_BUBBLES 和 animateWorker 新增 listening/speaking 状态
...
- [ ] 2. 实现 Voice Provider 配置与服务端语音服务
  - [ ] 2.1 新增 Voice Provider 配置模块
  ...
  - [ ] 2.3 实现 synthesizeSpeech 和 recognizeSpeech 函数
...
- [ ] 6. 扩展 Agent Invoke 层多模态上下文注入
  - [ ] 6.1 修改 composeAgentMessages 函数支持 MultimodalContext
    - 修改 `shared/runtime-agent.ts`
...
- [ ] 9. 集成 WorkflowPanel TTS 播放
  - [ ] 9.1 在 WorkflowPanel 中新增 TTS 播放按钮
    - 修改 `client/src/components/WorkflowPanel.tsx`
    - 在 Agent 回复内容区域新增播放按钮，复用 ChatPanel 的 TTS 播放逻辑
    - _Requirements: 3.4_

- [ ] 10. 扩展动态组织多模态能力标签
  - [ ] 10.1 扩展 inferTaskProfile 检测多模态关键词
  ...
  - [ ] 10.3 扩展 buildPlannerPrompt 包含 Agent 能力标签
    - 修改 `server/core/dynamic-organization.ts`
...
- [ ] 11. 环境变量与文档更新
  - [ ] 11.1 在 .env.example 中新增 TTS_* 和 STT_* 环境变量模板
    - 修改 `.env.example`
    - 新增 TTS_API_URL、TTS_API_KEY、TTS_MODEL、TTS_VOICE、STT_API_URL、STT_API_KEY、STT_MODEL
    - _Requirements: 7.1, 7.2_
  - [ ] 11.2 更新 .kiro/steering/project-overview.md 模块清单
