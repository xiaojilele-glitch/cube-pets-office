...
本任务将为 Mission 补齐端到端取消能力，覆盖 shared contracts、server 任务路由、MissionRuntime、lobster-executor 取消接口、Docker 运行时停止逻辑，以及前端任务详情页的取消交互。
...
- [x] 1. 扩展共享合同与状态枚举
  - [x] 1.1 更新 `shared/mission/contracts.ts`
    ...
    - 如采用独立事件类型，则补充 `MISSION_EVENT_TYPES.cancelled`
    - _Requirements: 4.1.1, 4.1.2, 1.2.3_
  - [x] 1.2 更新 `shared/mission/socket.ts`
...
- [x] 2. 实现 Mission 侧取消能力
  - [x] 2.1 在 `server/tasks/mission-store.ts` 中新增 `markCancelled()`
    - 写入 `status = cancelled`
    - 清理 `waitingFor` 与 `decision`
    - 记录取消原因与时间
    - 追加事件并持久化
    - _Requirements: 2.1.3, 4.1.2, 1.2.1, 1.2.2_
  - [x] 2.2 在 `server/tasks/mission-runtime.ts` 中新增 `cancelMission()`
    - 封装 store 操作
    - 广播 `mission.record.cancelled`
    - _Requirements: 2.1.4, 4.1.4_
  - [x] 2.3 为 Mission cancel 编写单元测试
    - 普通取消
    - waiting Mission 取消
    - 重复取消幂等
...
- [x] 4. 实现 executor cancel API
  - [x] 4.1 更新 `shared/executor/api.ts`
    - 明确 cancel 路由请求/响应类型
    - _Requirements: 3.1.1_
  - [x] 4.2 在 `services/lobster-executor/src/service.ts` 中新增 `cancel()`
    - queued job 直接取消
    - waiting job 直接取消
    - running job 转发给运行时控制
    - 已终态幂等返回
    - _Requirements: 3.1.2, 3.1.3, 3.1.4, 3.1.6_
  - [x] 4.3 在 `services/lobster-executor/src/app.ts` 中实现 `POST /api/executor/jobs/:id/cancel`
    - 去掉 `501`
    - 返回结构化结果
    - _Requirements: 3.1.1_
  - [x] 4.4 为 executor cancel 编写服务层测试
    - 不存在 job
    - queued job
    - waiting job
    - 已终态 job
...
- [x] 5. 打通 Docker 运行中任务停止
  - [x] 5.1 扩展 `StoredJobRecord` 保存取消上下文
    - 记录 `cancelRequested`
    - 保存运行中容器标识或控制句柄
    - _Requirements: 3.1.5_
  - [x] 5.2 在 `services/lobster-executor/src/docker-runner.ts` 中实现运行中 cancel
    - 优先 stop
    - 超时后 kill
    - 发出 `job.cancelled`
    - 保留现有 artifacts 和日志文件
    - _Requirements: 3.1.5, 3.2.1, 3.2.2, 3.2.3_
  - [x] 5.3 为 Docker cancel 编写测试
    - stop 成功
    - stop 超时后 kill
    - cancel 过程中日志追加
    - _Requirements: 3.1.5, 3.2.1, 3.2.3_

- [x] 6. 调整 executor callback 到 Mission 状态映射
  - [x] 6.1 更新 `server/index.ts`
    - `job.cancelled` -> `mission.cancelled`
    - 不再与 `failed` 复用同一路径
    - _Requirements: 4.2.1, 4.2.2, 4.2.3_
  - [x] 6.2 编写事件映射测试
    - `job.cancelled` 事件后 Mission 进入 `cancelled`
    - waiting Mission 收到取消后清空决策态
    - _Requirements: 4.2.1, 4.2.3_

- [x] 7. 增加前端取消交互
  - [x] 7.1 扩展 `client/src/lib/mission-client.ts` 或任务客户端
    - 新增 `cancelMission(taskId, payload)`
    - _Requirements: 5.1.1, 5.1.2_
  - [x] 7.2 扩展 `client/src/lib/tasks-store.ts`
    - 暴露 cancel action
    - 维护提交中状态
    - 处理 Socket 回写
    - _Requirements: 5.1.3, 5.1.4, 5.2.2_
  - [x] 7.3 更新 `client/src/components/tasks/TaskDetailView.tsx`
    - 增加取消按钮
    - 增加确认对话框与原因输入
    - 终态后隐藏继续执行类按钮
