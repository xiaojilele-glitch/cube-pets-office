...
- [ ] 1. 定义共享类型契约和数据库 Schema 扩展
  - [ ] 1.1 在 `shared/skill-contracts.ts` 中定义 SkillDefinition、SkillRecord、SkillBinding、SkillBindingConfig、SkillExecutionMetrics、SkillAuditLog、SkillContext、SideEffect、CanaryConfig 等类型接口
    - 确保与现有 `WorkflowSkillBinding`（shared/organization-schema.ts）兼容
    - _Requirements: 1.1, 6.1, 7.1, 9.1_
  - [ ] 1.2 扩展 `server/db/index.ts`，新增 skills、skill_metrics、skill_audit_log 表和对应的 CRUD 方法
...
- [ ] 2. 实现 SkillRegistry 核心模块
  - [ ] 2.1 创建 `server/core/skill-registry.ts`，实现 registerSkill 方法
  ...
  - [ ] 2.3 实现 resolveSkills 方法，包含依赖解析和循环检测
  ...
  - [ ] 2.5 实现 enableSkill、disableSkill 方法和审计日志
    - 更新 SkillRecord.enabled 字段
    - 写入 skill_audit_log 记录（包含 operator、reason、timestamp）
    - _Requirements: 5.1, 5.2, 5.3, 5.4_
  - [ ] 2.6 实现 getSkillVersions、querySkills 和 resolveMcpForSkill 方法
...
- [ ] 4. 实现 SkillActivator 模块
  - [ ] 4.1 创建 `server/core/skill-activator.ts`，实现 activateSkills 方法
  ...
  - [ ] 4.2 实现 buildSkillPromptSection 方法
...
- [ ] 5. 实现 SkillContext 上下文隔离
  - [ ] 5.1 创建 `server/core/skill-context.ts`，实现 SkillContext 工厂函数
...
- [ ] 6. 实现 SkillMonitor 性能监控模块
  - [ ] 6.1 创建 `server/core/skill-monitor.ts`，实现 recordMetrics 和 getSkillMetrics 方法
...
- [ ] 8. 集成到现有工作流系统
  - [ ] 8.1 修改 `server/core/dynamic-organization.ts`，将 SKILL_LIBRARY 迁移到 SkillRegistry
    - 在 resolveSkills 函数中调用 skillRegistry.resolveSkills 替代硬编码查找
    - 保持 SKILL_LIBRARY 作为种子数据，启动时自动注册到 SkillRegistry
    - _Requirements: 2.1, 2.2, 2.5_
  - [ ] 8.2 修改 `shared/runtime-agent.ts` 的 buildAgentSystemPrompt，注入 Skill prompt
    - 在系统提示中追加 Skill prompt 片段（通过 SkillActivator.buildSkillPromptSection）
    - _Requirements: 3.1, 3.2, 3.3_
  - [ ] 8.3 修改 `server/core/workflow-engine.ts`，在执行阶段调用 SkillActivator
...
- [ ] 9. 实现 Skill API 路由
  - [ ] 9.1 创建 `server/routes/skills.ts`，实现 Skill 管理 API
  ...
  - [ ] 9.2 在 `server/routes/workflows.ts` 中新增节点 Skill 查询端点
    - GET /api/workflows/:id/nodes/:nodeId/skills — 返回节点的 SkillBinding 列表
    - _Requirements: 10.4_
  - [ ] 9.3 在 `server/index.ts` 中注册 skills 路由
    - _Requirements: 10.4_

- [ ] 10. 实现前端 Skill 展示组件
  - [ ] 10.1 创建 `client/src/components/SkillCard.tsx` 组件
    - 展示 name、summary、category、version、enabled 状态
    - 点击展开详细信息（prompt、MCP 依赖、性能指标）
    - _Requirements: 10.2, 10.3_
  - [ ] 10.2 在 `client/src/components/WorkflowPanel.tsx` 的 OrgView 中集成 SkillCard
...
- [ ] 11. 种子数据迁移和最终集成
  - [ ] 11.1 创建种子数据脚本，将现有 SKILL_LIBRARY 中的 8 个 Skill 注册到数据库
