...
- [ ] 1. 定义共享遥测类型
  - [ ] 1.1 创建 `shared/telemetry.ts`，定义 LLMCallRecord、AgentTimingRecord、TelemetryAlert、TelemetrySnapshot、AgentTimingSummary、MissionStageTiming、MissionTelemetrySummary、TelemetryBudget 接口
...
- [ ] 2. 实现服务端遥测存储
  - [ ] 2.1 创建 `server/core/telemetry-store.ts`，实现 TelemetryStore 类
...
- [ ] 4. 实现 LLM 调用和 Agent 埋点
  - [ ] 4.1 修改 `server/core/llm-client.ts`，在 `callProvider` 函数中添加遥测埋点
    - 调用前记录 startTime，调用后构造 LLMCallRecord 并写入 telemetryStore
    - 失败时同样记录，包含 error 字段
    - 从 response.usage 提取 token 数据，使用 estimateCost 计算费用
    - _Requirements: 1.1, 1.2, 1.3_

  - [ ] 4.2 修改 `server/core/agent.ts`，在 `sharedAgentDependencies.llmProvider` 中添加 Agent 计时埋点
...
- [ ] 5. 实现遥测 REST API 和 Socket 推送
  - [ ] 5.1 创建 `server/routes/telemetry.ts`，实现 GET /api/telemetry/live 和 GET /api/telemetry/history 路由
    - live 返回 telemetryStore.getSnapshot()
    - history 返回 telemetryStore.getHistory()
    - _Requirements: 4.1, 4.2, 4.3_

  - [ ] 5.2 在 `server/index.ts` 中注册遥测路由
    - _Requirements: 4.1, 4.2_

  - [ ] 5.3 修改 `server/core/socket.ts`，新增 `emitTelemetryUpdate` 函数
    - 实现 500ms 节流逻辑
    - 在 connection 事件中向新客户端发送当前快照
    - _Requirements: 5.1, 5.2, 5.3_

  - [ ] 5.4 在 TelemetryStore 的 recordLLMCall 和 recordAgentTiming 中调用 emitTelemetryUpdate
    - _Requirements: 5.1_
...
- [ ] 7. 实现前端遥测 Store 和仪表盘组件
  - [ ] 7.1 创建 `client/src/lib/telemetry-store.ts`，实现 Zustand store
  ...
  - [ ] 7.2 创建 `client/src/components/TelemetryDashboard.tsx`
  ...
  - [ ] 7.3 修改 `client/src/components/Scene3D.tsx`，在 Canvas 外层添加仪表盘浮窗图标
    - 右上角像素风格图标按钮
    - 点击切换 dashboardOpen
    - 存在预警时显示红色圆点徽标
    - _Requirements: 7.1, 7.2, 7.3_

  - [ ] 7.4 修改 `client/src/pages/Home.tsx`，集成 TelemetryDashboard 组件
    - 在 isSceneReady 条件块中渲染 TelemetryDashboard
    - 初始化 Socket 监听和 REST 数据加载
    - _Requirements: 6.1_

- [ ] 8. 实现纯前端模式遥测支持
  - [ ] 8.1 创建 `client/src/lib/browser-telemetry-store.ts`
    - 在 IndexedDB 中新增 telemetry object store
    - 实现 saveTelemetrySnapshot / loadTelemetrySnapshot 函数
    - _Requirements: 9.2, 9.3_

  - [ ] 8.2 修改 `client/src/lib/browser-llm.ts`，在 callBrowserLLM 中添加遥测埋点
...
- [ ] 9. 实现 3D 场景 Agent 预警图标
  - [ ] 9.1 修改 `client/src/components/three/PetWorkers.tsx`（或相关 3D 组件），在存在 agent_slow 预警时在对应 Agent 头顶显示警告图标
