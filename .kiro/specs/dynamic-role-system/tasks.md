...
- [ ] 1. 定义共享类型和角色模板 Schema
  - [ ] 1.1 创建 `shared/role-schema.ts`，定义 RoleTemplate、RoleChangeLogEntry、RoleOperationLog、RolePerformanceRecord、AgentRoleRecommendation、RoleSwitchTrace、PhaseAssignment、RoleConstraintError、RoleUsageSummary、AgentRoleDistribution 等类型
...
- [ ] 2. 实现 RoleRegistry 角色模板注册表
  - [ ] 2.1 创建 `server/core/role-registry.ts`，实现 RoleRegistry 类
...
- [ ] 3. 实现 RoleConstraintValidator 约束校验器
  - [ ] 3.1 创建 `server/core/role-constraint-validator.ts`，实现 RoleConstraintValidator 类
...
- [ ] 4. 扩展 Agent 类支持角色加载/卸载
  - [ ] 4.1 修改 `server/core/agent.ts`，新增 AgentRoleState 和角色管理方法
  ...
  - [ ] 4.2 修改 `shared/runtime-agent.ts`，扩展 RuntimeAgentConfig 新增 currentRoleId 可选字段
    - _Requirements: 2.1_
...
- [ ] 6. 实现 RolePerformanceTracker 绩效追踪器
  - [ ] 6.1 创建 `server/core/role-performance-tracker.ts`，实现 RolePerformanceTracker 类
...
- [ ] 7. 实现 RoleMatcher 角色匹配器
  - [ ] 7.1 创建 `server/core/role-matcher.ts`，实现 RoleMatcher 类
...
- [ ] 8. 实现 Phase-level 角色切换编排
  - [ ] 8.1 扩展 `shared/executor/contracts.ts`，在 ExecutionPlanStep 中新增 assignments?: PhaseAssignment[] 字段
    - _Requirements: 5.1_
  - [ ] 8.2 修改 `server/core/workflow-engine.ts`，在阶段切换逻辑中集成角色切换
  ...
  - [ ] 8.3 修改 `server/core/mission-orchestrator.ts`，记录角色切换轨迹到 Mission 事件流
...
- [ ] 9. Checkpoint - 确保工作流集成测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [ ] 10. 实现 RoleAnalyticsService 分析与告警
  - [ ] 10.1 创建 `server/core/role-analytics.ts`，实现 RoleAnalyticsService 类
...
- [ ] 11. 实现后端 API 扩展
  - [ ] 11.1 修改 `server/routes/agents.ts`，扩展 GET /api/agents/:id 响应
    - 新增 currentRole（角色名 + 加载时间）和 roleHistory（最近 20 条切换记录）字段
    - _Requirements: 8.1, 8.5_
  - [ ] 11.2 创建 `server/routes/analytics.ts`，实现 GET /api/analytics/roles 端点
    - 返回 roleUsageSummary 和 agentRoleDistribution
    - _Requirements: 7.5_
  - [ ] 11.3 扩展 Socket.IO 事件，新增 agent.roleChanged 事件推送
...
- [ ] 12. 集成动态组织生成模块
  - [ ] 12.1 修改 `server/core/dynamic-organization.ts`，在组织生成后自动注册角色模板到 RoleRegistry
    - LLM 生成的角色定义 source 标记为 "generated"
    - 人工预定义的角色模板 source 标记为 "predefined"
    - _Requirements: 1.4_

- [ ] 13. 实现前端角色状态展示
  - [ ] 13.1 创建 `client/src/lib/role-store.ts`，实现 Zustand store
    - 订阅 WebSocket agent.roleChanged 事件
    - 维护 agentRoles Map（currentRole + roleHistory）
    - _Requirements: 8.1, 8.5_
  - [ ] 13.2 扩展 Agent 详情面板，展示 currentRole 和 roleHistory
    - 显示当前角色名称和加载时间
    - 显示最近 20 次角色切换记录
    - _Requirements: 8.1_
  - [ ] 13.3 实现多角色绩效雷达图组件
    ...
    - _Requirements: 8.2_
  - [ ] 13.4 扩展 3D 场景中 Agent 的角色视觉标识
    - 根据 currentRole 动态更新颜色、图标、头顶标签
    - 角色切换时添加过渡动画
    - _Requirements: 8.3_
  - [ ] 13.5 扩展工作流面板 Mission 时间轴
    - 展示各 Agent 的角色切换节点
    - 不同角色用不同颜色标注执行时段
    - _Requirements: 8.4_
