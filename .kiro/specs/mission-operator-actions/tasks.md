...
- [ ] 2. 建立后端操作动作服务
  - [ ] 2.1 创建 `server/tasks/mission-operator-service.ts`
    - 集中处理动作可用性校验
...
- [ ] 3. 增加操作动作 API
  - [ ] 3.1 在 `server/routes/tasks.ts` 中新增 `POST /:id/operator-actions`
...
- [ ] 4. 扩展 MissionRuntime / MissionStore
  - [ ] 4.1 增加 operatorState / blocker / attempt 持久化支持
    - Mission snapshot 正确保存与恢复
    - _Requirements: 1.1.3, 1.1.4, 3.1.4, 4.1.2_

  - [ ] 4.2 广播动作导致的 Mission 更新
    - 复用 `mission_event`
    - 确保前端 1 秒内收到状态变化
    - _Requirements: 6.1.3, 6.1.4_

- [ ] 5. 扩展 executor 控制能力
  - [ ] 5.1 更新 `shared/executor/api.ts`
    - 增加 pause / resume 路由常量与请求响应类型
    - _Requirements: 2.1.3, 2.2.3_

  - [ ] 5.2 在 `services/lobster-executor/src/service.ts` 中增加 pause/resume
    - queued job: 控制进入执行
    - running job: 调用 Docker pause/unpause
    - _Requirements: 2.1.3, 2.2.3_

  - [ ] 5.3 在 `services/lobster-executor/src/app.ts` 中暴露 pause/resume API
    - _Requirements: 2.1.3, 2.2.3_

  - [ ] 5.4 在 `services/lobster-executor/src/docker-runner.ts` 中实现容器 pause/unpause
    - `container.pause()`
    - `container.unpause()`
    - _Requirements: 2.1.3, 2.2.3_

  - [ ] 5.5 编写 executor 控制测试
    - queued pause/resume
...
- [ ] 6. 新增前端操作栏
  - [ ] 6.1 创建 `client/src/components/tasks/OperatorActionBar.tsx`
    - 动态渲染可用动作
    - 每个动作独立 loading
    - _Requirements: 7.1.1, 7.1.2, 7.1.4_

  - [ ] 6.2 接入 blocker 卡片与最新动作摘要
    - 展示 blocker reason / createdAt
    - 展示 latest operator action
    - _Requirements: 3.1.4, 6.1.4_

  - [ ] 6.3 将 `OperatorActionBar` 集成到 `TaskDetailView.tsx`
    - 放在头部主信息区
    - 与现有决策面板、执行面板协同
    - _Requirements: 7.1.1, 7.2.1, 7.2.2, 7.2.3_

  - [ ] 6.4 扩展 `client/src/lib/tasks-store.ts`
...
- [ ] 7. 最终验证
  - [ ] 7.1 验证 pause -> resume 闭环
  - [ ] 7.2 验证 mark-blocked -> resume 闭环
  - [ ] 7.3 验证 failed/cancelled -> retry 闭环
  - [ ] 7.4 验证 terminate 复用 cancel 闭环
