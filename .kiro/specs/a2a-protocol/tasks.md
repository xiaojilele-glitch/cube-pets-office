...
- [ ] 1. 定义 A2A 协议类型和工具函数
  - [ ] 1.1 在 `shared/a2a-protocol.ts` 中定义所有 A2A 协议类型
  ...
  - [ ] 1.2 在 `shared/a2a-protocol.ts` 中实现序列化/反序列化和工具函数
  ...
  - [ ]* 1.4 编写 A2ASession 序列化往返和上下文验证属性测试
...
- [ ] 2. 扩展 ExternalAgentNode 类型
  - [ ] 2.1 在 `shared/organization-schema.ts` 中定义 ExternalAgentNode 接口
  ...
  - [ ]* 2.2 编写 ExternalAgentNode 快照兼容性属性测试
    - 实现 `arbitraryExternalAgentNode` 生成器
    - **Property 13: ExternalAgentNode 快照兼容性**
    - **Validates: Requirements 6.3**

- [ ] 3. Checkpoint - 确保协议类型层测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [ ] 4. 实现框架适配器
  - [ ] 4.1 定义 FrameworkAdapter 接口并实现 CrewAI 适配器
...
- [ ] 5. 实现 A2A Client
  - [ ] 5.1 在 `server/core/a2a-client.ts` 中实现 A2AClient 类
...
- [ ] 6. 实现 A2A Server
  - [ ] 6.1 在 `server/core/a2a-server.ts` 中实现 A2AServer 类
...
- [ ] 7. Checkpoint - 确保 Client/Server 核心测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [ ] 8. 扩展 MessageBus A2A 路由
  - [ ] 8.1 在 `server/core/message-bus.ts` 中扩展 A2A 路由
  ...
  - [ ]* 8.2 编写 MessageBus A2A 路由属性测试
    - **Property 14: A2A 消息路由与元数据正确性**
    - **Validates: Requirements 7.1, 7.3**

- [ ] 9. 实现 A2A API 路由
  - [ ] 9.1 在 `server/routes/a2a.ts` 中实现所有 A2A 端点
  ...
  - [ ] 9.2 在 `server/index.ts` 中注册 A2A 路由到 `/api/a2a`
    - 导入 a2a 路由并挂载
    - _Requirements: 3.1_
  - [ ]* 9.3 编写 API 路由单元测试
...
- [ ] 10. Checkpoint - 确保 API 层测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [ ] 11. 实现 3D 场景跨框架可视化
  - [ ] 11.1 创建 `client/src/lib/a2a-store.ts` A2A 状态 Store
    - 定义 A2AState 接口（activeSessions、a2aMessages）
    - 实现 Zustand store，监听 Socket.IO `a2a_message` 事件
    - _Requirements: 8.4_
  - [ ] 11.2 创建 `client/src/components/three/CrossFrameworkParticles.tsx` 粒子流组件
    - 渲染菱形粒子 + 渐变色轨迹（区别于跨 Pod 圆形粒子）
    - 框架颜色映射：CrewAI 蓝色、LangGraph 紫色、Claude 橙色、Custom 灰色
    - 活跃会话显示框架类型标签
    - 完成时绿色脉冲，失败时红色脉冲
    - _Requirements: 8.1, 8.2, 8.3, 8.5_
  - [ ] 11.3 在 `client/src/components/Scene3D.tsx` 中集成 CrossFrameworkParticles
    - 导入并渲染 CrossFrameworkParticles 组件
    - _Requirements: 8.1_

- [ ] 12. 集成动态组织和工作流引擎
  - [ ] 12.1 扩展 `server/core/dynamic-organization.ts` 支持 ExternalAgentNode
    - 在组织生成逻辑中识别 "@external-xxx" 引用模式
    - 为识别到的外部 Agent 创建 ExternalAgentNode 并纳入组织快照
    - _Requirements: 6.2_
  - [ ] 12.2 扩展工作流引擎支持 ExternalAgentNode 任务分配
