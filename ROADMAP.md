...
- [ ] **降低启动门槛**：把本地运行门槛从“需要启动 Vite + Node 服务端并配置 `.env`”降到“安装依赖后直接打开前端”
- [ ] **更贴合项目本质**：项目核心竞争力是 3D 可视化、多智能体协作过程展示与可玩性，不是重服务端基础设施
- [ ] **更利于传播与开源增长**：纯前端 demo 更适合分享、试用、Fork 和二次改造
- [ ] **与当前实现方向一致**：项目已经从 MySQL 主导转为本地 JSON + 文件工作空间，继续向浏览器本地 runtime 演进是自然下一步

这条主线的目标状态：

1. **默认纯前端运行**
   - [ ] 工作流编排可直接在浏览器中执行
   - [ ] 不再强依赖本地 Node 服务端和 `/api` 代理
   - [ ] 配置改为浏览器本地保存，并支持导出 / 导入

2. **保留可选高级模式**
   - [ ] 保留当前服务端版作为 advanced mode
   - [ ] 对不支持浏览器直连的模型服务，允许接入极简代理
   - [ ] 纯前端模式与服务端模式尽量复用同一套 workflow 核心逻辑

3. **主线程只负责 UI，运行时迁入 Worker**
   - [ ] 将 workflow engine 从服务端抽象为可复用 runtime 层
   - [ ] 优先迁移到 Zustand + Web Worker，避免阻塞主线程和 3D 渲染
   - [ ] 用 Worker 事件总线替代当前 Socket.IO 的本地联动职责
...
2. **Phase PF-2：浏览器存储替换服务端本地文件**
   ...
   - [ ] 将 `sessions / memory / reports / SOUL / HEARTBEAT` 从文件系统语义映射到浏览器本地存储
...
4. **Phase PF-4：模型调用前端化**
   - [ ] 优先支持可浏览器直连的模型服务
   - [ ] 设置页明确提示“默认仅适合本地使用，密钥保存在浏览器端”
   - [ ] 为不适合浏览器直连的提供商保留“可选代理 URL”能力

5. **Phase PF-5：产品化收口**
   - [ ] 增加“纯前端模式 / 高级模式”切换说明
   - [ ] 默认启动路径优先纯前端模式，减少首次体验成本
   - [ ] 在确认纯前端链路稳定前，不删除现有服务端实现
