  ...
  return (
    <div className="flex gap-3 py-2">
      ...
      <div className="flex flex-col items-center pt-1">
        <div className={`h-2.5 w-2.5 rounded-full ${cfg.bg} ring-2 ring-white`} />
      ...
      <div className="min-w-0 flex-1 pb-2">
        <div className="flex flex-wrap items-center gap-1.5">
          <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold ${cfg.color} ${cfg.bg}`}>
            {cfg.label}
          </span>
          <span className="text-[10px] font-medium text-gray-500">{entry.agentId}</span>
          <span className="text-[10px] text-gray-400">·</span>
          <span className="text-[10px] text-gray-400">{entry.stage}</span>
        </div>
        <p className="mt-0.5 text-xs leading-relaxed text-gray-700">{entry.content}</p>
      </div>
    </div>
  );
}
...
export function MemoryTimeline() {
  ...
  const sorted = [...entries].sort((a, b) => a.timestampOffset - b.timestampOffset);
  ...
  return (
    <div className="px-3 py-2">
      ...
      <div>
        {sorted.map((entry, i) => (
          <MemoryEntry key={`${entry.agentId}-${entry.kind}-${i}`} entry={entry} />
        ))}
      </div>
    </div>
  );
