...
import type { EventStreamDiff, MetricsComparison } from '@/lib/replay/comparison';
...
export function ComparisonView() {
  ...
  const [compTimeline, setCompTimeline] = useState<ExecutionTimeline | null>(null);

  useEffect(() => {
    if (!comparisonMissionId) return;
    const store = new BrowserReplayStore();
    store.getTimeline(comparisonMissionId).then(setCompTimeline).catch(console.error);
  }, [comparisonMissionId]);

  const comparison = useMemo(() => new ReplayComparison(), []);

  const diff: EventStreamDiff | null = useMemo(
    () => (timeline && compTimeline ? comparison.diffEventStreams(timeline, compTimeline) : null),
    [comparison, timeline, compTimeline],
  );

  const metrics: MetricsComparison | null = useMemo(
    () => (timeline && compTimeline ? comparison.compareMetrics(timeline, compTimeline) : null),
    [comparison, timeline, compTimeline],
  );
  ...
  return (
    <div className="space-y-4 p-4 text-xs">
      <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={stopComparison} className="text-[10px] text-white/50">
          Close
        </Button>
      </div>
        ...
        <table className="w-full text-[10px]">
          <thead>
            <tr className="text-white/40">
              <th className="text-left">Metric</th><th>A</th><th>B</th><th>Diff</th>
            </tr>
          </thead>
          <tbody className="text-white/70">
            <MetricRow label="Duration" a={`${metrics.a.totalDuration}ms`} b={`${metrics.b.totalDuration}ms`} diff={`${metrics.durationDiff > 0 ? '+' : ''}${metrics.durationDiff}ms`} />
            <MetricRow label="LLM Calls" a={String(metrics.a.llmMetrics.callCount)} b={String(metrics.b.llmMetrics.callCount)} diff={`${metrics.llmCallCountDiff > 0 ? '+' : ''}${metrics.llmCallCountDiff}`} />
            <MetricRow label="Concurrency" a={String(metrics.a.concurrency.maxConcurrentAgents)} b={String(metrics.b.concurrency.maxConcurrentAgents)} diff={`${metrics.concurrencyDiff > 0 ? '+' : ''}${metrics.concurrencyDiff}`} />
          </tbody>
        </table>
      </div>
...
function TimelineSummary({ label, timeline }: { label: string; timeline: ExecutionTimeline }) {
  return (
...
function MetricRow({ label, a, b, diff }: { label: string; a: string; b: string; diff: string }) {
  return (
    <tr>
