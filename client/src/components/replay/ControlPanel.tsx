...
import type { ReplayEngine, ReplayState, PlaybackSpeed } from '@/lib/replay/replay-engine';
...
export function ControlPanel({ engine, timeline }: ControlPanelProps) {
  ...
  const handleFilterKeyword = useCallback(
    (kw: string) => {
      setKeyword(kw);
      engine.setFilters({ ...engine.getState().filters, keyword: kw || undefined });
    },
    [engine],
  );
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {/* Play / Pause */}
      <Button variant="ghost" size="icon-sm" onClick={handlePlayPause} className="text-white">
        {state === 'playing' ? <Pause className="size-4" /> : <Play className="size-4" />}
      </Button>
      <Button variant="ghost" size="icon-sm" onClick={handleStop} className="text-white/70">
        ...
        {PLAYBACK_SPEEDS.map((s) => (
          <button
            ...
            className={`rounded px-1.5 py-0.5 text-[10px] font-semibold transition-colors ${
              speed === s ? 'bg-white/20 text-white' : 'text-white/50 hover:text-white/80'
            }`}
          >
            {s}x
          </button>
      ...
      <div className="flex gap-1 text-[10px]">
        <button onClick={() => togglePanel('costTracker')} className="rounded bg-white/10 px-2 py-0.5 text-white/60 hover:text-white">Cost</button>
        <button onClick={() => togglePanel('performance')} className="rounded bg-white/10 px-2 py-0.5 text-white/60 hover:text-white">Perf</button>
        <button onClick={() => togglePanel('dataLineage')} className="rounded bg-white/10 px-2 py-0.5 text-white/60 hover:text-white">Lineage</button>
        <button onClick={toggleDemoMode} className="rounded bg-white/10 px-2 py-0.5 text-white/60 hover:text-white">Demo</button>
      </div>
    </div>
  );
}
