...
import type { ExecutionEvent, LineageNode } from '../../../../shared/replay/contracts';
...
export function DataLineageGraph({ events }: DataLineageGraphProps) {
  ...
  const graph = useMemo(() => tracker.buildLineageGraph(events), [tracker, events]);
  ...
  const svgWidth = Math.max(200, (Math.ceil(Math.sqrt(graph.nodes.length)) + 1) * 100);
  const svgHeight = Math.max(120, (Math.ceil(graph.nodes.length / Math.ceil(Math.sqrt(graph.nodes.length))) + 1) * 60);

  return (
    <div className="h-full overflow-auto rounded-lg border border-white/10 bg-[#1a1a2e]/95 backdrop-blur">
      <p className="px-3 pt-2 text-[11px] font-semibold text-white/80">Data Lineage</p>
      <svg width={svgWidth} height={svgHeight} className="min-h-full min-w-full">
        <defs>
        ...
        {graph.edges.map((edge, i) => {
          ...
          const isHighlighted = highlighted.has(edge.from) && highlighted.has(edge.to);
          return (
            <line
              key={`e-${i}`}
        ...
        {graph.nodes.map((node) => {
          ...
          return (
            <g key={node.id} onClick={() => handleNodeClick(node.id)} className="cursor-pointer">
              ...
              />
              <text x={pos.x} y={pos.y + 22} textAnchor="middle" fontSize={8} fill="#aaa">
