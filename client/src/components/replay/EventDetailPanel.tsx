    ...
    <div className="space-y-4 p-4 text-xs">
      ...
      {event.metadata && (
        <div className="space-y-1 text-white/50">
          {event.metadata.cost != null && (
            <p>Cost: ${event.metadata.cost.toFixed(4)}</p>
          )}
      ...
      {related.length > 0 && (
        <div>
          <p className="mb-1 text-[10px] font-semibold text-white/60">
            Related Events
          </p>
    ...
    <>
      ...
      <p className="mt-1 whitespace-pre-wrap text-white/70">
        {typeof data.messageContent === "string"
          ? data.messageContent
          : JSON.stringify(data.messageContent, null, 2)}
      </p>
    </>
  );
}
...
function DecisionDetail({ data }: { data: DecisionEventData }) {
  return (
    <>
      ...
      <p className="mt-1 text-white/50">
        Result: {JSON.stringify(data.decisionResult)}
      </p>
    </>
  );
}
...
function ResourceDetail({ data }: { data: ResourceAccessEventData }) {
  return (
    <>
      <Row
        label="Resource"
        value={`${data.resourceType}: ${data.resourceId}`}
      />
      ...
      {data.permissionCheck && (
        <Row
          label="Permission"
          value={data.permissionCheck.passed ? "Passed" : "Denied"}
        />
      )}
    </>
  );
