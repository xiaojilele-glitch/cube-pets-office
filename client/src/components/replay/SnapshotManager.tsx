        ...
        <div className="absolute right-0 top-full z-50 mt-1 w-72 rounded-lg border border-white/10 bg-[#1a1a2e] p-3 shadow-xl">
          ...
          <div className="mb-3 space-y-1.5">
            ...
            <Input
              ...
              className="h-7 border-white/10 bg-white/5 text-[11px] text-white placeholder:text-white/30"
            />
            <Button size="sm" onClick={handleCreate} className="w-full text-[11px]">
              Create Snapshot
            </Button>
          </div>
          ...
          <div className="max-h-40 space-y-1 overflow-y-auto">
            ...
            {snapshots.map((s) => (
              ...
              >
                ...
                <span className="ml-2 text-white/40">{new Date(s.createdAt).toLocaleTimeString()}</span>
              </button>
            ))}
          </div>
          ...
          <div className="mt-2 flex gap-2">
            <Button variant="outline" size="sm" onClick={handleExport} className="flex-1 text-[10px]">
              <Download className="mr-1 size-3" /> Export
            </Button>
            <Button variant="outline" size="sm" onClick={handleImport} className="flex-1 text-[10px]">
