...
export function PerformancePanel({ timeline }: PerformancePanelProps) {
  ...
  const metrics = useMemo(() => analyzer.calculateMetrics(timeline), [analyzer, timeline]);
  const bottlenecks = useMemo(() => analyzer.detectBottlenecks(timeline), [analyzer, timeline]);

  return (
    <div className="rounded-lg border border-white/10 bg-[#1a1a2e]/95 p-3 text-xs backdrop-blur">
      <p className="mb-2 text-[11px] font-semibold text-white/80">Performance</p>

      {/* Key metrics */}
      <div className="mb-3 grid grid-cols-2 gap-2">
        <Metric label="Duration" value={`${(metrics.totalDuration / 1000).toFixed(1)}s`} />
        <Metric label="LLM Calls" value={String(metrics.llmMetrics.callCount)} />
        <Metric label="Avg Response" value={`${metrics.llmMetrics.avgResponseTime.toFixed(0)}ms`} />
        <Metric label="Max Concurrent" value={String(metrics.concurrency.maxConcurrentAgents)} />
      </div>
      ...
      {metrics.stageMetrics.length > 0 && (
        <div className="mb-2">
          ...
          {metrics.stageMetrics.map((s) => (
            <div key={s.stageKey} className="flex items-center justify-between py-0.5">
              <span className={`text-[10px] ${s.isBottleneck ? 'font-semibold text-red-400' : 'text-white/60'}`}>
