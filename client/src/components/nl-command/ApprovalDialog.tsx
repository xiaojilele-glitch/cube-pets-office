  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white p-4">
      <div className="text-sm font-medium text-stone-800">
        Plan Approval —{" "}
        <span className="text-indigo-600">{plan.planId.slice(0, 8)}</span>
      </div>
      ...
      {approval && (
        <div className="text-xs text-stone-500">
          ...
          {approval.requiredApprovers.length} · Status:{" "}
      ...
      {onSubmit && (
        <>
          <div className="flex gap-2">
            {(["approved", "rejected", "revision_requested"] as const).map(
              d => (
                <button
                  key={d}
          ...
          />
          <GlowButton onClick={handleSubmit} className="self-end rounded-md">
            Submit
          </GlowButton>
        </>
      )}
