...
export function RiskHeatMap({ plan }: RiskHeatMapProps) {
  ...
  const { grid, placed } = useMemo(() => {
    const g: { count: number; level: string; labels: string[] }[][] = Array.from({ length: 5 }, () =>
      Array.from({ length: 5 }, () => ({ count: 0, level: "low", labels: [] })),
    );
    ...
    for (const r of risks) {
      ...
      if (levels.indexOf(r.level) > levels.indexOf(cell.level)) cell.level = r.level;
    }
    return { grid: g, placed: risks.length };
  }, [risks]);

  if (risks.length === 0) {
    return <div className="p-4 text-sm text-stone-400">No risk data available.</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-3 text-xs text-stone-500">
        ...
        <span className="ml-auto">Overall: <span className="font-medium" style={{ color: LEVEL_COLORS[plan.riskAssessment?.overallRiskLevel ?? "low"] }}>{plan.riskAssessment?.overallRiskLevel ?? "—"}</span></span>
      </div>
      <div className="flex gap-1">
        <div className="flex flex-col justify-between pr-1 text-[10px] text-stone-400" style={{ height: CELL * 5 }}>
          <span>High</span>
          <span>Impact</span>
          <span>Low</span>
        </div>
        <div>
              ...
              {row.map((cell, ci) => (
                <div
                  ...
                  style={{
                    width: CELL,
                    height: CELL,
                    backgroundColor: cell.count > 0 ? LEVEL_COLORS[cell.level] : "#f1f5f9",
                    opacity: cell.count > 0 ? 0.7 + Math.min(cell.count, 3) * 0.1 : 1,
                  }}
          ...
          <div className="mt-0.5 flex justify-between text-[10px] text-stone-400" style={{ width: CELL * 5 + 4 * 2 }}>
            <span>Low</span>
