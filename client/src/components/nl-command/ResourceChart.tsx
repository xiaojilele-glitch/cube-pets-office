...
export function ResourceChart({ plan }: ResourceChartProps) {
  const { agents, maxTime, minTime, legend } = useMemo(() => {
    ...
    if (entries.length === 0) return { agents: [], maxTime: 1, minTime: 0, legend: [] };
    ...
    // Group by agentType
    const grouped = new Map<string, { taskId: string; left: number; width: number; count: number }[]>();
    ...
    const types = [...typeSet];
    const legendItems = types.map((t, i) => ({ type: t, color: COLORS[i % COLORS.length] }));
    ...
    return { agents: agentRows, maxTime: max, minTime: min, legend: legendItems };
  }, [plan]);

  if (agents.length === 0) {
    return <div className="p-4 text-sm text-stone-400">No resource allocation data available.</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="overflow-x-auto rounded border border-stone-200">
        {agents.map((a) => (
          <div key={a.type} className="flex items-center border-b border-stone-100" style={{ height: BAR_HEIGHT + 8 }}>
            <div className="w-28 shrink-0 truncate px-2 text-xs text-stone-600">{a.type}</div>
            <div className="relative flex-1" style={{ height: BAR_HEIGHT }}>
              {a.segments.map((s, i) => (
                <div
                  key={i}
                  className="absolute top-0 rounded opacity-80"
                  style={{ left: `${s.left}%`, width: `${s.width}%`, height: BAR_HEIGHT, backgroundColor: a.color, minWidth: 4 }}
      ...
      <div className="text-xs text-stone-400">
        Total agents: {plan.resourceAllocation?.totalAgents ?? 0} · Peak concurrency: {plan.resourceAllocation?.peakConcurrency ?? 0}
      </div>
    </div>
  );
}
