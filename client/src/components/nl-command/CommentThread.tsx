...
export function CommentThread({ comments, onAdd }: CommentThreadProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {comments.length === 0 && (
        <div className="py-4 text-center text-sm text-stone-400">No comments yet.</div>
      )}

      {comments.map((c) => (
        <div key={c.commentId} className="rounded-lg border border-stone-200 px-3 py-2">
          ...
          <div className="mt-1 text-xs text-stone-600 whitespace-pre-wrap">{c.content}</div>
          {c.mentions.length > 0 && (
          ...
          {c.versions.length > 1 && (
            ...
            >
              {expandedVersions.has(c.commentId) ? "Hide" : "Show"} {c.versions.length - 1} edit(s)
            </button>
          )}
          {expandedVersions.has(c.commentId) &&
            c.versions.slice(0, -1).map((v, i) => (
              <div key={i} className="mt-1 border-l-2 border-stone-200 pl-2 text-[10px] text-stone-400">
