...
export function AgentRolePanel({ agentId }: { agentId: string }) {
  ...
  const roleInfo: AgentRoleInfo = agentRoles.get(agentId) || {
    currentRole: null,
    roleHistory: [],
  };
          ...
          <div className="mt-2">
            <div className="flex items-center gap-2">
              <span
                ...
                style={{
                  backgroundColor: getRoleColor(roleInfo.currentRole.roleName),
                }}
              />
            ...
            <p className="mt-1 text-[10px] text-[#8B7355]">
              {t(locale, "加载时间", "Loaded at")}:{" "}
              {formatTime(locale, roleInfo.currentRole.loadedAt)}
            </p>
          </div>
        ) : (
          ...
          <div className="mt-2 max-h-[280px] space-y-1.5 overflow-y-auto">
            {roleInfo.roleHistory.map((entry, i) => (
              <div
                ...
                className="rounded-lg bg-[#F8F4F0] px-3 py-2"
              >
                <div className="flex items-center gap-1.5 text-[11px]">
                  ...
                  <span className="font-medium text-[#5A4A3A]">
                    {entry.fromRole || t(locale, "无", "None")}
                  </span>
