...
export type TaskHubClarificationTopic = "outcome" | "timeline" | "constraints";
...
const EXACT_PREVIEW_TEXT = [
  ...
  {
    zh: "尽量保持零停机，或者至少做到对用户无感交付。",
    en: "Maintain zero-downtime or user-transparent delivery.",
  },
  {
    zh: "需要预留一条明确可执行的回滚路径。",
    en: "Keep an explicit rollback path available.",
  },
  {
    zh: "执行过程需要遵守已提出的预算与成本边界。",
    en: "Respect the stated budget and execution-cost boundaries.",
  },
  {
    zh: "交付节奏需要对齐这条指令里的时间要求。",
    en: "Keep delivery aligned with the requested timeline.",
  },
  {
    zh: "成功标准需要在任务工作台里进一步确认。",
    en: "Success criteria should be confirmed inside the task workspace.",
  },
  {
    zh: "当前目标已经足够明确，可以直接进入执行。",
    en: "The requested outcome is clear enough to open execution immediately.",
  },
  {
    zh: "在操作人补充明确时间点之前，时间安排暂时保持弹性。",
    en: "Timeline is flexible until the operator adds a deadline.",
  },
  {
    zh: "这条指令已经带有明确的时间预期。",
    en: "Timeline expectations are already embedded in the command.",
  },
  {
    zh: "这是从指令里推断出的主要执行目标。",
    en: "Primary execution target inferred from the command.",
  },
  {
    zh: "在指令补充完整之前，范围漂移的风险会更高。",
    en: "Scope drift is likely until the command is clarified.",
  },
  {
    zh: "执行质量取决于任务简报是否能持续对齐原始指令。",
    en: "Execution quality depends on keeping the task brief aligned with the command.",
  },
  {
    zh: "优先在任务详情里确认范围、阻塞点和操作人决策。",
    en: "Use the task detail panel to confirm scope, blockers, and operator decisions.",
  },
  { zh: "锁定范围与验收口径", en: "Lock scope and acceptance" },
  {
    zh: "把这条指令整理成可执行的任务简报，并确认成功标准。",
    en: "Translate the command into an execution-ready brief and confirm success criteria.",
  },
  {
    zh: "推进主执行链路，并持续把阻塞信息同步回任务详情。",
    en: "Carry out the main workstream and keep the task detail panel updated with blockers.",
  },
  { zh: "复核结果并准备操作人交接", en: "Review outcome and operator handoff" },
  {
    zh: "核对结果、整理剩余风险，并准备下一步操作人动作。",
    en: "Verify the result, summarize remaining risks, and prepare the next operator action.",
  },
  {
    zh: "复核交付物，并确认下一步操作动作。",
    en: "Review deliverables and confirm the next operator action.",
  },
  { zh: "简报已对齐", en: "Brief aligned" },
  { zh: "操作人交接就绪", en: "Operator handoff ready" },
  {
    zh: "回退到一个更小范围的交付切片。",
    en: "Fall back to a smaller scoped delivery slice.",
  },
  {
    zh: "当时间压力或阻塞压力在执行中持续升高时触发。",
    en: "Timeline or blocker pressure rises during execution.",
  },
  {
    zh: "裁掉非关键工作，把当前可见任务继续稳定推进。",
    en: "Trim non-critical work and keep the operator-visible task active.",
  },
  { zh: "范围缩小，但恢复速度更快。", en: "Lower scope, faster recovery." },
  {
    zh: "让任务继续留在队列里，把次级范围延后处理。",
    en: "Keep the mission open in the task queue and defer secondary scope.",
  },
  {
    zh: "用操作动作去暂停、重试或标记阻塞，而不是丢失上下文。",
    en: "Use operator actions to pause, retry, or mark blocked without losing context.",
  },
  {
    zh: "如果执行质量回退，就暂停任务并从最近一个稳定的操作检查点恢复。",
    en: "If execution quality regresses, pause the mission and resume from the last stable operator checkpoint.",
  },
...
export function localizeTaskHubText(value: string, locale: AppLocale): string {
  ...
  const forwardPatterns: Array<[RegExp, (detail: string) => string]> = [
    [/^Clarified outcome:\s*(.+)$/gm, detail => `已补充交付结果：${detail}`],
    [/^Clarified timeline:\s*(.+)$/gm, detail => `已补充时间要求：${detail}`],
    [
      /^Clarified constraints:\s*(.+)$/gm,
      detail => `已补充约束边界：${detail}`,
    ],
  ];

  const reversePatterns: Array<[RegExp, (detail: string) => string]> = [
    [/^已补充交付结果：\s*(.+)$/gm, detail => `Clarified outcome: ${detail}`],
    [/^已补充时间要求：\s*(.+)$/gm, detail => `Clarified timeline: ${detail}`],
    [
      /^已补充约束边界：\s*(.+)$/gm,
      detail => `Clarified constraints: ${detail}`,
    ],
  ];

  for (const [pattern, replacer] of isZh ? forwardPatterns : reversePatterns) {
    next = next.replace(pattern, (_match, detail: string) => replacer(detail));
  }
