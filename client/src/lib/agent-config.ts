...
export const DEPARTMENT_SOFT_COLORS: Record<
  AgentVisualConfig["department"],
  string
> = {
...
export function getAgentConfig(agentId?: string | null): AgentVisualConfig {
  return (
    AGENT_VISUAL_MAP[agentId || DEFAULT_AGENT_ID] ||
    AGENT_VISUAL_MAP[DEFAULT_AGENT_ID]
  );
}
...
export function getAgentIdleText(
  agentId: string | null | undefined,
  locale: AppLocale
): string {
...
export function getAgentChatRole(
  agentId: string | null | undefined,
  locale: AppLocale
): string {
...
export function createGuestVisualConfig(
  ...
  return {
    ...
    department:
      (node.departmentId as AgentVisualConfig["department"]) || "meta",
    role: "worker",
