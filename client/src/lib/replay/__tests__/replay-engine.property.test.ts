...
import { REPLAY_EVENT_TYPES, PLAYBACK_SPEEDS } from '../../../../../shared/replay/contracts';
...
const sortedEventsArb = (min = 2, max = 30): fc.Arbitrary<ExecutionEvent[]> =>
  fc.array(executionEventArb, { minLength: min, maxLength: max }).map((events) =>
    [...events]
...
describe('Property 15: Filter correctness', () => {
  ...
  it('filtering by agentIds returns only matching events', () => {
    ...
    fc.assert(
      fc.property(
        sortedEventsArb(5, 40),
        fc.subarray(['agent-a', 'agent-b', 'agent-c', 'agent-d'], { minLength: 1 }),
        (events, filterAgents) => {
          const engine = new ReplayEngine(makeTimeline(events));
          ...
          for (const e of filtered) {
            const matches = agentSet.has(e.sourceAgent) || (e.targetAgent != null && agentSet.has(e.targetAgent));
    ...
    fc.assert(
      fc.property(
        ...
        (events, filterTypes, filterAgents) => {
          ...
          for (const e of filtered) {
            expect(typeSet.has(e.eventType)).toBe(true);
            const agentMatch = agentSet.has(e.sourceAgent) || (e.targetAgent != null && agentSet.has(e.targetAgent));
...
describe('Property 32: Interactive replay pauses at decision nodes', () => {
  ...
  it('with interactive mode, engine state machine supports pause at decision events', () => {
    ...
    const events: ExecutionEvent[] = [
      makeEvent({ eventId: 'e1', timestamp: 1000, eventType: 'AGENT_STARTED', sourceAgent: 'agent-a' }),
      makeEvent({ eventId: 'e2', timestamp: 2000, eventType: 'DECISION_MADE', sourceAgent: 'agent-a' }),
      makeEvent({ eventId: 'e3', timestamp: 3000, eventType: 'AGENT_STOPPED', sourceAgent: 'agent-a' }),
    ];
    const tl = makeTimeline(events);
    const engine = new ReplayEngine(tl);
    ...
    fc.assert(
      fc.property(
        ...
        (events, speed, seekFraction) => {
          ...
          // Seek to a position
          const seekTime = tl.startTime + seekFraction * (tl.endTime - tl.startTime);
    ...
    state: fc.record({
      ...
      filters: fc.record({
        eventTypes: fc.option(
          fc.subarray([...REPLAY_EVENT_TYPES] as ReplayEventType[], { minLength: 0 }),
          { nil: undefined },
        ),
        agentIds: fc.option(
  ...
  it('JSON.stringify then JSON.parse produces deeply equal snapshot', () => {
    ...
    fc.assert(
      fc.property(snapshotArb, (snapshot) => {
        ...
        expect(parsed.state.eventCursorIndex).toBe(snapshot.state.eventCursorIndex);
        expect(parsed.state.speed).toBe(snapshot.state.speed);
        expect(parsed.state.filters).toEqual(snapshot.state.filters);
        expect(parsed.state.cameraPosition).toEqual(snapshot.state.cameraPosition);
