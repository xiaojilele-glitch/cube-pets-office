...
describe("ReplayEngine", () => {
  ...
  describe("speed control (Req 7.2)", () => {
    ...
    it.each([0.5, 1, 2, 4, 8] as PlaybackSpeed[])(
      "accepts valid speed %s",
      speed => {
  ...
  describe("filtering (Req 7.3, 7.4)", () => {
    ...
    it("filters by eventType", () => {
      ...
      expect(
        filtered.every(
          e =>
            e.eventType === "AGENT_STARTED" || e.eventType === "AGENT_STOPPED"
        )
      ).toBe(true);
    });
    ...
    it("filters by agentId (sourceAgent)", () => {
      ...
      expect(
        filtered.every(
          e => e.sourceAgent === "agent-2" || e.targetAgent === "agent-2"
        )
      ).toBe(true);
    });
