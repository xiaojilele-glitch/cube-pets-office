...
describe('CostTracker', () => {
  ...
  describe('calculateCumulativeCost', () => {
    ...
    it('groups costs by agent', () => {
      const events: ExecutionEvent[] = [
        makeEvent({ eventId: '1', timestamp: 100, sourceAgent: 'a', metadata: { cost: 1 } }),
        makeEvent({ eventId: '2', timestamp: 200, sourceAgent: 'b', metadata: { cost: 2 } }),
        makeEvent({ eventId: '3', timestamp: 300, sourceAgent: 'a', metadata: { cost: 3 } }),
      ];
    ...
    it('groups costs by operation type (eventType)', () => {
      const events: ExecutionEvent[] = [
        makeEvent({ eventId: '1', timestamp: 100, eventType: 'CODE_EXECUTED', metadata: { cost: 1 } }),
        makeEvent({ eventId: '2', timestamp: 200, eventType: 'DECISION_MADE', metadata: { cost: 2 } }),
      ];

      const summary = tracker.calculateCumulativeCost(events, 200);
      expect(summary.byOperationType).toEqual({ CODE_EXECUTED: 1, DECISION_MADE: 2 });
    });
  ...
  describe('generateOptimizationSuggestions', () => {
    it('suggests optimizing the top-cost agent', () => {
      ...
      expect(suggestions.some((s) => s.includes('agent') || s.includes('Agent'))).toBe(true);
    });
