...
import type { ExecutionEvent, ExecutionTimeline } from '../../../../../shared/replay/contracts';
...
describe('PerformanceAnalyzer', () => {
  ...
  describe('calculateMetrics', () => {
    ...
    it('computes stage metrics grouped by metadata.stageKey', () => {
      const events = [
        makeEvent({ eventId: '1', timestamp: 100, metadata: { stageKey: 'plan' } }),
        makeEvent({ eventId: '2', timestamp: 300, metadata: { stageKey: 'plan' } }),
        makeEvent({ eventId: '3', timestamp: 400, metadata: { stageKey: 'exec' } }),
        makeEvent({ eventId: '4', timestamp: 500, metadata: { stageKey: 'exec' } }),
      ];
    ...
    it('counts DECISION_MADE events as LLM calls', () => {
      const events = [
        makeEvent({ eventId: '1', timestamp: 100, eventType: 'DECISION_MADE', eventData: { executionTime: 50 } }),
        makeEvent({ eventId: '2', timestamp: 200, eventType: 'DECISION_MADE', eventData: { executionTime: 150 } }),
  ...
  describe('detectBottlenecks', () => {
    it('marks stages with duration > 2x average as bottlenecks', () => {
      const events = [
        makeEvent({ eventId: '1', timestamp: 100, metadata: { stageKey: 'fast1' } }),
        makeEvent({ eventId: '2', timestamp: 110, metadata: { stageKey: 'fast1' } }),
        makeEvent({ eventId: '3', timestamp: 200, metadata: { stageKey: 'fast2' } }),
        makeEvent({ eventId: '4', timestamp: 210, metadata: { stageKey: 'fast2' } }),
        makeEvent({ eventId: '5', timestamp: 300, metadata: { stageKey: 'slow' } }),
        makeEvent({ eventId: '6', timestamp: 600, metadata: { stageKey: 'slow' } }),
      ];
      const tl = makeTimeline(events);
    ...
    it('returns empty when no stages exceed 2x average', () => {
      const events = [
        makeEvent({ eventId: '1', timestamp: 100, metadata: { stageKey: 'a' } }),
        makeEvent({ eventId: '2', timestamp: 200, metadata: { stageKey: 'a' } }),
        makeEvent({ eventId: '3', timestamp: 300, metadata: { stageKey: 'b' } }),
        makeEvent({ eventId: '4', timestamp: 400, metadata: { stageKey: 'b' } }),
      ];
      const tl = makeTimeline(events);
      const bottlenecks = analyzer.detectBottlenecks(tl);
  ...
  describe('analyzeConcurrency', () => {
    it('tracks concurrent agents from AGENT_STARTED/STOPPED events', () => {
      const events = [
        makeEvent({ eventId: '1', timestamp: 100, eventType: 'AGENT_STARTED', sourceAgent: 'a' }),
        makeEvent({ eventId: '2', timestamp: 200, eventType: 'AGENT_STARTED', sourceAgent: 'b' }),
        makeEvent({ eventId: '3', timestamp: 300, eventType: 'AGENT_STOPPED', sourceAgent: 'a' }),
        makeEvent({ eventId: '4', timestamp: 400, eventType: 'AGENT_STOPPED', sourceAgent: 'b' }),
      ];
      const tl = makeTimeline(events);
    ...
    it('ensures activeAgents >= 0 at all timeline points', () => {
      const events = [
        makeEvent({ eventId: '1', timestamp: 100, eventType: 'AGENT_STARTED', sourceAgent: 'a' }),
        makeEvent({ eventId: '2', timestamp: 200, eventType: 'AGENT_STOPPED', sourceAgent: 'a' }),
        makeEvent({ eventId: '3', timestamp: 250, eventType: 'AGENT_STOPPED', sourceAgent: 'b' }),
      ];
      const tl = makeTimeline(events);
      const profile = analyzer.analyzeConcurrency(tl);
  ...
  describe('comparePerformance', () => {
    it('computes diffs between two timelines', () => {
      ...
      const tlB = makeTimeline([
        makeEvent({ eventId: '3', timestamp: 100, eventType: 'DECISION_MADE', eventData: { executionTime: 100 } }),
        makeEvent({ eventId: '4', timestamp: 200, eventType: 'DECISION_MADE', eventData: { executionTime: 200 } }),
