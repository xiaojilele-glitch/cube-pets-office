 ...
 */
export default function CommandCenterPage({ className }: { className?: string }) {
  ...
  const handleSubmitCommand = useCallback(
    async (text: string) => {
      await submitCommand({ commandText: text, userId: "current-user", priority: "medium" });
    },
    [submitCommand],
  );
      ...
      <div className="mx-auto flex w-full max-w-[1680px] flex-1 flex-col px-3 py-3 md:px-5 md:py-4">
        ...
        {error && (
          <div className="mt-3 flex items-center justify-between rounded-xl border border-rose-200 bg-rose-50/80 px-4 py-2 text-sm text-rose-800">
            <span>{error}</span>
            <button type="button" className="ml-3 text-rose-600 underline" onClick={clearError}>
              Dismiss
            </button>
          </div>
        )}
        ...
        <section className="mt-3 shrink-0 rounded-2xl border border-stone-200/80 bg-white/78 p-4 shadow-sm backdrop-blur">
          ...
          {currentDialog && currentDialog.status === "active" && (
            <div className="mt-3">
              <ClarificationPanel dialog={currentDialog} onAnswer={handleClarificationAnswer} />
            </div>
          )}
        </section>
        ...
        <div className="mt-3 grid min-h-0 flex-1 gap-3 lg:grid-cols-2">
          ...
          <section className="flex flex-col rounded-2xl border border-stone-200/80 bg-white/78 p-4 shadow-sm backdrop-blur">
            <h2 className="mb-3 text-sm font-semibold text-stone-700">Execution Plan</h2>
            {currentPlan ? (
              <div className="flex-1 space-y-2 overflow-auto text-sm text-stone-600">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-stone-400">Plan {currentPlan.planId}</span>
                </div>
                <div>
                  <span className="font-medium">Missions:</span> {currentPlan.missions.length}
                </div>
                <div>
                  <span className="font-medium">Tasks:</span> {currentPlan.tasks.length}
                </div>
                {currentPlan.timeline && (
                  <div>
                    <span className="font-medium">Timeline:</span>{" "}
                    {currentPlan.timeline.startDate} → {currentPlan.timeline.endDate}
                  </div>
                )}
                {currentPlan.costBudget && (
                  <div>
                    <span className="font-medium">Budget:</span>{" "}
                    {currentPlan.costBudget.totalBudget} {currentPlan.costBudget.currency}
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={cn(
                        ...
                        currentPlan.riskAssessment.overallRiskLevel === "critical"
                          ? "bg-red-100 text-red-700"
                          : currentPlan.riskAssessment.overallRiskLevel === "high"
                            ? "bg-orange-100 text-orange-700"
                            : currentPlan.riskAssessment.overallRiskLevel === "medium"
          ...
          <section className="flex flex-col rounded-2xl border border-stone-200/80 bg-white/78 p-4 shadow-sm backdrop-blur">
            <h2 className="mb-3 text-sm font-semibold text-stone-700">Real-time Monitoring</h2>
            {dashboard ? (
              <div className="flex-1 space-y-3 overflow-auto text-sm">
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-xl border border-stone-100 bg-stone-50/60 p-3">
                    <div className="text-xs text-stone-400">Active Commands</div>
                ...
                {alerts.length > 0 && (
                  <div>
                    <h3 className="mb-1 text-xs font-medium text-stone-500">Recent Alerts</h3>
              ...
              <div className="flex flex-1 items-center justify-center text-sm text-stone-400">
                {loading ? "Loading dashboard..." : "No monitoring data available."}
              </div>
            )}
          </section>
        </div>
        ...
        <section className="mt-3 shrink-0 rounded-2xl border border-stone-200/80 bg-white/78 p-4 shadow-sm backdrop-blur">
          <h2 className="mb-2 text-sm font-semibold text-stone-700">Decision Support</h2>
          {currentPlan?.riskAssessment ? (
            <div className="space-y-2 text-sm text-stone-600">
              ...
              <ul className="space-y-1">
                {currentPlan.riskAssessment.risks.slice(0, 3).map((risk) => (
                  <li key={risk.id} className="rounded-lg border border-stone-100 bg-stone-50/60 px-3 py-2 text-xs">
