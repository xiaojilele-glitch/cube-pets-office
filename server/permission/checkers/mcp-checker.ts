...
export class McpChecker implements ResourceChecker {
  checkConstraints(action: Action, resource: string, constraints: PermissionConstraints): boolean {
    ...
    if (constraints.endpoints && constraints.endpoints.length > 0) {
      const toolAllowed = constraints.endpoints.some((allowed) => allowed === toolId || allowed === "*");
    ...
    if (operation && constraints.methods && constraints.methods.length > 0) {
      const opAllowed = constraints.methods.some((m) => m === operation || m === "*");
    ...
    if (constraints.parameterConstraints) {
      ...
      for (const [, regexStr] of Object.entries(constraints.parameterConstraints)) {
        try {
          new RegExp(regexStr);
        } catch {
          return false;
...
function parseMcpResource(resource: string): { toolId: string; operation: string | null } {
  const colonIdx = resource.indexOf(":");
  if (colonIdx === -1) return { toolId: resource, operation: null };
  return { toolId: resource.slice(0, colonIdx), operation: resource.slice(colonIdx + 1) };
}
