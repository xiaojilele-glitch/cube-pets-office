...
describe('phase1 message bus hardening', () => {
  ...
  it('allows direct-report communication on the correct stage', async () => {
    ...
    const result = await messageBus.send('ceo', 'pixel', 'focus on gameplay', 'wf-1', 'direction');
  ...
  it('rejects skip-level messaging', async () => {
    const { messageBus, MessageBusValidationError } = await import('../core/message-bus.js');
