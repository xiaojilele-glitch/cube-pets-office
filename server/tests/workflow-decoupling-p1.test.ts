...
function arbMissionRecord(): fc.Arbitrary<MissionRecord> {
  return fc.tuple(
    ...
    fc.option(fc.array(arbMessageLogEntry, { minLength: 0, maxLength: 15 }), { nil: undefined }),
...
const CORE_REQUIRED_KEYS: (keyof MissionRecord)[] = [
  'id', 'kind', 'title', 'status', 'progress', 'stages', 'createdAt', 'updatedAt', 'events',
];
  ...
  it('enrichment fields do not affect core required fields', () => {
    fc.assert(
      fc.property(
        arbCoreMissionRecord,
        fc.option(arbOrganization, { nil: undefined }),
        fc.option(fc.array(arbWorkPackage, { minLength: 0, maxLength: 5 }), { nil: undefined }),
        fc.option(fc.array(arbMessageLogEntry, { minLength: 0, maxLength: 10 }), { nil: undefined }),
        fc.option(fc.array(arbAgentCrewMember, { minLength: 0, maxLength: 5 }), { nil: undefined }),
    ...
    fc.assert(
      fc.property(arbMissionRecord(), (record) => {
        ...
        if (record.workPackages !== undefined) {
          ...
          const validStatuses = ['pending', 'running', 'passed', 'failed', 'verified'];
        ...
        if (record.agentCrew !== undefined) {
          ...
          const validStatuses = ['idle', 'working', 'thinking', 'done', 'error'];
