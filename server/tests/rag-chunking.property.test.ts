...
const arbSourceType: fc.Arbitrary<SourceType> = fc.constantFrom(
  ...SOURCE_TYPES
);
    ...
    fc.assert(
      fc.property(arbCodeContent, arbChunkMetadata, (content, metadata) => {
        ...
        const hasSignature = chunks.some(
          c =>
            c.metadata.functionSignature !== undefined &&
            c.metadata.functionSignature !== null
        );
