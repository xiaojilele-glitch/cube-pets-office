...
// ─── Helpers for Properties 15-23 ────────────────────────────

function makeConfig(overrides?: Partial<AutonomyConfig>): AutonomyConfig {
  return { ...DC, ...overrides };
}
function makeCostMonitor(config = DC): CostMonitor { return new CostMonitor(config); }
function makePM(config = DC): CapabilityProfileManager { return new CapabilityProfileManager(config); }
function makeEngine(pm?: CapabilityProfileManager, cm?: CostMonitor, config = DC): CompetitionEngine {
  return new CompetitionEngine(pm ?? makePM(config), cm ?? makeCostMonitor(config), config);
}
const mockLLM: LLMProvider = { review: async () => '' };
function makeJudge(config = DC): JudgeAgent { return new JudgeAgent(mockLLM, config); }

const arbCompTask = (ov?: Partial<CompetitionTaskRequest>): fc.Arbitrary<CompetitionTaskRequest> =>
  fc.record({
    taskId: fc.string({ minLength: 1, maxLength: 8 }),
    requiredSkills: fc.array(fc.string({ minLength: 1, maxLength: 6 }), { minLength: 1, maxLength: 3 }),
    requiredSkillWeights: fc.constant(new Map<string, number>()),
    priority: fc.constantFrom('critical' as const, 'high' as const, 'normal' as const, 'low' as const),
    qualityRequirement: fc.constantFrom('high' as const, 'normal' as const, 'low' as const),
    dataSecurityLevel: fc.constantFrom('sensitive' as const, 'normal' as const),
...
describe('Property 15: 竞争模式触发条件', () => {
  it('should trigger iff any condition is met', () => {
    fc.assert(fc.property(arbCompTask(), fc.double({ min: 0, max: 1, noNaN: true }), (task, bestFitness) => {
      const engine = makeEngine();
      const result = engine.shouldTrigger(task, bestFitness);
      const uncertainty = engine.computeUncertainty(task, bestFitness);
      const expected = task.priority === 'critical' || task.qualityRequirement === 'high' || uncertainty > 0.7 || task.manualCompetition === true;
  ...
  it('returns false when no conditions met', () => {
    const task: CompetitionTaskRequest = {
      taskId: 't1', requiredSkills: ['a'], requiredSkillWeights: new Map(),
      priority: 'normal', qualityRequirement: 'normal', dataSecurityLevel: 'normal',
      estimatedDurationMs: 10000, manualCompetition: false, historicalFailRate: 0, descriptionAmbiguity: 0,
    };
    expect(makeEngine().shouldTrigger(task, 1.0)).toBe(false);
  });
});
...
// Validates: Requirements 4.3
describe('Property 17: 多样性优先参赛者选择', () => {
  it('returns exactly N agents with seed as highest fitness, all fitness >= 0.5', () => {
    fc.assert(fc.property(
      fc.integer({ min: 2, max: 4 }),
      fc.array(fc.record({
        id: fc.stringMatching(/^[a-z]{1,6}$/),
        confidence: fc.double({ min: 0.6, max: 1, noNaN: true }),
        skills: fc.dictionary(fc.constantFrom('js', 'py', 'go', 'rust', 'sql'), fc.double({ min: 0.3, max: 1, noNaN: true }), { minKeys: 1, maxKeys: 3 }),
      }), { minLength: 6, maxLength: 10 }),
      (n, agents) => {
        const seen = new Set<string>();
        const unique = agents.filter(a => { if (seen.has(a.id)) return false; seen.add(a.id); return true; });
        if (unique.length < n) return;

        const pm = makePM();
        for (const a of unique) {
          const p = pm.initProfile(a.id, Object.keys(a.skills));
          for (const [k, v] of Object.entries(a.skills)) p.skillVector.set(k, v);
          p.confidenceScore = a.confidence;
        }

        const engine = makeEngine(pm);
        const result = engine.selectContestants(unique.map(a => a.id), n);
...
describe('Property 18: 外部 Agent 安全校验', () => {
  it('blocks external agents on sensitive tasks, allows otherwise', () => {
    fc.assert(fc.property(fc.boolean(), fc.constantFrom('sensitive' as const, 'normal' as const), (isExternal, secLevel) => {
      const pm = makePM();
      pm.initProfile('a1', isExternal ? ['external', 'coding'] : ['coding']);
      const engine = makeEngine(pm);
      const task: CompetitionTaskRequest = {
        taskId: 't1', requiredSkills: ['coding'], requiredSkillWeights: new Map(),
        priority: 'normal', qualityRequirement: 'normal', dataSecurityLevel: secLevel,
        estimatedDurationMs: 10000, manualCompetition: false, historicalFailRate: 0, descriptionAmbiguity: 0,
      };
      const result = engine.checkDataSecurity('a1', task);
      if (secLevel === 'sensitive' && isExternal) expect(result).toBe(false);
...
describe('Property 19: 竞争 deadline 计算', () => {
  it('deadline = min(estimatedDurationMs * 1.5, maxDeadlineMs)', () => {
    fc.assert(fc.property(
      ...
      fc.integer({ min: 1000, max: 1_000_000 }),
      (estimatedMs, maxDeadlineMs) => {
        const config = makeConfig({ competition: { ...DC.competition, maxDeadlineMs } });
        const engine = makeEngine(undefined, undefined, config);
        const deadline = engine.computeDeadline(estimatedMs);
        expect(deadline).toBeCloseTo(Math.min(estimatedMs * 1.5, maxDeadlineMs), 5);
      },
    ), { numRuns: 200 });
  });
});
...
describe('Property 20: 裁判加权评分公式', () => {
  it('totalWeighted = 0.35*c + 0.30*q + 0.20*e + 0.15*n, in [0,1]', () => {
    fc.assert(fc.property(arb01, arb01, arb01, arb01, (c, q, e, n) => {
      const judge = makeJudge();
      const input: JudgingScore[] = [{ agentId: 'a1', correctness: c, quality: q, efficiency: e, novelty: n, totalWeighted: 0 }];
      const [result] = judge.computeWeightedScores(input);
      const expected = 0.35 * c + 0.30 * q + 0.20 * e + 0.15 * n;
      expect(result.totalWeighted).toBeCloseTo(Math.min(Math.max(expected, 0), 1), 10);
      expect(result.totalWeighted).toBeGreaterThanOrEqual(0);
      expect(result.totalWeighted).toBeLessThanOrEqual(1);
    }), { numRuns: 200 });
...
describe('Property 21: 竞争结果能力画像回写', () => {
  it('1st +0.05 (cap 1.0), last -0.03 (floor 0.0), middle unchanged', () => {
    fc.assert(fc.property(fc.integer({ min: 3, max: 5 }), fc.double({ min: 0, max: 1, noNaN: true }), (count, baseSkill) => {
      const pm = makePM();
      const agents: string[] = [];
      for (let i = 0; i < count; i++) {
        const id = `ag-${i}`;
      ...
      pm.applyCompetitionReward(agents[0], 0.05);
      pm.applyCompetitionReward(agents[agents.length - 1], -0.03);
      expect(pm.getProfile(agents[0])!.skillVector.get('coding')!).toBeCloseTo(Math.min(before[0] + 0.05, 1.0), 10);
      expect(pm.getProfile(agents[agents.length - 1])!.skillVector.get('coding')!).toBeCloseTo(Math.max(before[before.length - 1] - 0.03, 0.0), 10);
      for (let i = 1; i < count - 1; i++) {
        expect(pm.getProfile(agents[i])!.skillVector.get('coding')!).toBeCloseTo(before[i], 10);
      }
    }), { numRuns: 200 });
  });
});
...
describe('Property 22: Judge 置信度下调', () => {
  it('N overrides → confidence decreases by N*0.1, alert when < 0.5', () => {
    fc.assert(fc.property(fc.integer({ min: 1, max: 15 }), (n) => {
      const judge = makeJudge();
      expect(judge.getJudgeConfidenceScore()).toBe(1.0);
      const alerts: string[] = [];
      const origWarn = console.warn;
      console.warn = (msg: string) => { alerts.push(msg); };
      for (let i = 0; i < n; i++) judge.onJudgmentOverridden();
      console.warn = origWarn;
      const expected = Math.max(1.0 - n * 0.1, 0);
      expect(judge.getJudgeConfidenceScore()).toBeCloseTo(expected, 10);
      if (expected < 0.5) {
  ...
  it('merge when |top1-top2|/max < 0.05, no merge when >= 0.05', () => {
    fc.assert(fc.property(
      ...
      (s1, s2) => {
        const judge = makeJudge();
        const scores: JudgingScore[] = [
          { agentId: 'a1', correctness: 0, quality: 0, efficiency: 0, novelty: 0, totalWeighted: s1 },
          { agentId: 'a2', correctness: 0, quality: 0, efficiency: 0, novelty: 0, totalWeighted: s2 },
        ];
        const result = judge.checkMergeRequired(scores);
        const maxVal = Math.max(s1, s2);
        const relDiff = Math.abs(s1 - s2) / maxVal;
        expect(result).toBe(relDiff < 0.05);
      },
    ), { numRuns: 200 });
  });
  it('returns false for single contestant', () => {
    const scores: JudgingScore[] = [{ agentId: 'a1', correctness: 1, quality: 1, efficiency: 1, novelty: 1, totalWeighted: 1 }];
    expect(makeJudge().checkMergeRequired(scores)).toBe(false);
  });
});
