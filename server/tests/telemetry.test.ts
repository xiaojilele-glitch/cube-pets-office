...
const arbAlertType = fc.constantFrom(
  "agent_slow",
  "token_over_budget"
) as fc.Arbitrary<TelemetryAlert["type"]>;
...
const arbTelemetrySnapshot: fc.Arbitrary<TelemetrySnapshot> = fc.record({
  ...
  agentTimings: fc.array(arbAgentTimingSummary, {
    minLength: 0,
    maxLength: 10,
  }),
  missionStageTimings: fc.array(arbMissionStageTiming, {
    minLength: 0,
    maxLength: 10,
  }),
...
const arbLLMCallRecord: fc.Arbitrary<LLMCallRecord> = fc.record({
  ...
  workflowId: fc.option(fc.string({ minLength: 1, maxLength: 20 }), {
    nil: undefined,
  }),
    ...
    fc.assert(
      fc.property(
        ...
        records => {
          ...
          const expectedTokensIn = records.reduce(
            (sum, r) => sum + r.tokensIn,
            0
          );
          const expectedTokensOut = records.reduce(
            (sum, r) => sum + r.tokensOut,
            0
          );
...
const arbAgentTimingRecord = (
  agentId: string
): fc.Arbitrary<AgentTimingRecord> =>
  fc.record({
...
describe("Property 3: 滑动窗口平均值正确性", () => {
  ...
  it("avgDurationMs equals the arithmetic mean of the last min(N, 20) records for a given agent", async () => {
    ...
    fc.assert(
      fc.property(
        fc.array(arbAgentTimingRecord("test-agent"), {
          minLength: 1,
          maxLength: 60,
        }),
        records => {
          ...
          const windowRecords = records.slice(
            -Math.min(records.length, WINDOW_SIZE)
          );
          const expectedAvg =
            windowRecords.reduce((sum, r) => sum + r.durationMs, 0) /
...
const arbMissionTelemetrySummary: fc.Arbitrary<MissionTelemetrySummary> =
  fc.record({
    ...
    stageTimings: fc.array(arbMissionStageTiming, {
      minLength: 0,
      maxLength: 5,
    }),
  });
  ...
  it("when sliding window average durationMs > 30000, an agent_slow alert with matching agentId exists", async () => {
    ...
    fc.assert(
      fc.property(
        ...
        fc.array(fc.nat({ max: 120_000 }), { minLength: 1, maxLength: 40 }),
        (agentId, durations) => {
          const store = new TelemetryStore();
          store.resetCurrentMission();
          ...
          const windowRecords = durations.slice(
            -Math.min(durations.length, WINDOW_SIZE)
          );
          const avg =
            windowRecords.reduce((s, d) => s + d, 0) / windowRecords.length;
