...
import type { AutonomyConfig, JudgingScore } from '../../shared/autonomy-types.js';
...
function makeConfig(ov?: Partial<AutonomyConfig>): AutonomyConfig { return { ...DC, ...ov }; }
function makePM(c = DC) { return new CapabilityProfileManager(c); }
function makeCM(c = DC) { return new CostMonitor(c); }
...
function makeJA(c = DC) { return new JudgeAgent(mockLLM, c); }

const arbTask = (ov?: Partial<CompetitionTaskRequest>): fc.Arbitrary<CompetitionTaskRequest> =>
  fc.record({
...
describe('Property 15: 竞争模式触发条件', () => {
  it('should trigger iff any condition is met', () => {
    fc.assert(fc.property(arbTask(), arb01, (task, bestFitness) => {
      const ce = makeCE();
      const result = ce.shouldTrigger(task, bestFitness);
      const unc = ce.computeUncertainty(task, bestFitness);
      const expected = task.priority === 'critical' || task.qualityRequirement === 'high' || unc > 0.7 || task.manualCompetition === true;
  ...
  it('returns <= N agents, no duplicates, all have profiles', () => {
    fc.assert(fc.property(
      ...
      (n, agents) => {
        ...
        const uniq = agents.filter(a => { if (seen.has(a.id)) return false; seen.add(a.id); return true; });
        ...
        const result = makeCE(pm).selectContestants(uniq.map(a => a.id), n);
...
describe('Property 18: 外部 Agent 安全校验', () => {
  it('blocks external agents on sensitive tasks, allows otherwise', () => {
    fc.assert(fc.property(fc.boolean(), fc.constantFrom('sensitive' as const, 'normal' as const), (isExt, secLvl) => {
      const pm = makePM();
      pm.initProfile('a1', isExt ? ['external', 'coding'] : ['coding']);
      const task: CompetitionTaskRequest = {
        taskId: 't1', requiredSkills: ['coding'], requiredSkillWeights: new Map(),
        priority: 'normal', qualityRequirement: 'normal', dataSecurityLevel: secLvl,
        estimatedDurationMs: 10000, manualCompetition: false, historicalFailRate: 0, descriptionAmbiguity: 0,
      };
...
describe('Property 19: 竞争 deadline 计算', () => {
  it('deadline = min(estimatedDurationMs * 1.5, maxDeadlineMs)', () => {
    fc.assert(fc.property(
      ...
      (estMs, maxMs) => {
        const cfg = makeConfig({ competition: { ...DC.competition, maxDeadlineMs: maxMs } });
...
describe('Property 20: 裁判加权评分公式', () => {
  it('totalWeighted = 0.35*c + 0.30*q + 0.20*e + 0.15*n, in [0,1]', () => {
    fc.assert(fc.property(arb01, arb01, arb01, arb01, (c, q, e, n) => {
      ...
      const exp = 0.35 * c + 0.30 * q + 0.20 * e + 0.15 * n;
...
describe('Property 21: 竞争结果能力画像回写', () => {
  it('1st +0.05 (cap 1.0), last -0.03 (floor 0.0), middle unchanged', () => {
    fc.assert(fc.property(fc.integer({ min: 3, max: 5 }), fc.double({ min: 0, max: 1, noNaN: true }), (cnt, base) => {
      const pm = makePM();
      const ids: string[] = [];
      for (let i = 0; i < cnt; i++) { ids.push(`ag${i}`); const p = pm.initProfile(`ag${i}`, ['coding']); p.skillVector.set('coding', base); }
      const before = ids.map(id => pm.getProfile(id)!.skillVector.get('coding')!);
      pm.applyCompetitionReward(ids[0], 0.05);
      pm.applyCompetitionReward(ids[ids.length - 1], -0.03);
      expect(pm.getProfile(ids[0])!.skillVector.get('coding')!).toBeCloseTo(Math.min(before[0] + 0.05, 1.0), 10);
      expect(pm.getProfile(ids[ids.length - 1])!.skillVector.get('coding')!).toBeCloseTo(Math.max(before[cnt - 1] - 0.03, 0.0), 10);
      for (let i = 1; i < cnt - 1; i++) expect(pm.getProfile(ids[i])!.skillVector.get('coding')!).toBeCloseTo(before[i], 10);
    }), { numRuns: 200 });
  });
});
...
describe('Property 22: Judge 置信度下调', () => {
  it('N overrides → confidence decreases by N*0.1, alert when < 0.5', () => {
    fc.assert(fc.property(fc.integer({ min: 1, max: 15 }), (n) => {
      ...
      console.warn = (m: string) => { alerts.push(m); };
      ...
      if (exp < 0.5) expect(alerts.some(a => a.includes('JUDGE_RELIABILITY_LOW'))).toBe(true);
    }), { numRuns: 100 });
  });
});
