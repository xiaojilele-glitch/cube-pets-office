...
import { AlertEngine, type AlertContext, type OnAlertCallback } from '../../core/nl-command/alert-engine.js';

const __test_dirname = dirname(fileURLToPath(import.meta.url));
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_alert_engine__/nl-audit.json');
...
function makeRule(overrides: Partial<AlertRule> = {}): AlertRule {
  return {
    ...
    condition: overrides.condition ?? { metric: 'cost', operator: 'gt', threshold: 100 },
...
describe('AlertEngine', () => {
  ...
  let notified: Alert[];
  const onAlert: OnAlertCallback = (alert) => { notified.push(alert); };
  ...
  describe('evaluate()', () => {
    it('should trigger alert when metric exceeds gt threshold', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'cost', operator: 'gt', threshold: 100 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { cost: 150 } }));
    ...
    it('should NOT trigger alert when metric does not exceed gt threshold', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'cost', operator: 'gt', threshold: 100 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { cost: 50 } }));
      expect(alerts).toHaveLength(0);
    });

    it('should NOT trigger for gt when value equals threshold', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'cost', operator: 'gt', threshold: 100 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { cost: 100 } }));
      expect(alerts).toHaveLength(0);
    });

    it('should trigger for lt operator', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'progress', operator: 'lt', threshold: 50 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { progress: 30 } }));
      expect(alerts).toHaveLength(1);
    });

    it('should trigger for eq operator', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'errors', operator: 'eq', threshold: 0 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { errors: 0 } }));
      expect(alerts).toHaveLength(1);
    });

    it('should trigger for gte operator at boundary', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'cost', operator: 'gte', threshold: 100 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { cost: 100 } }));
      expect(alerts).toHaveLength(1);
    });

    it('should trigger for lte operator at boundary', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'progress', operator: 'lte', threshold: 10 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { progress: 10 } }));
      expect(alerts).toHaveLength(1);
    });

    it('should skip disabled rules', async () => {
      engine.registerRule(makeRule({ enabled: false }));
      const alerts = await engine.evaluate(makeContext({ metrics: { cost: 999 } }));
      expect(alerts).toHaveLength(0);
    });

    it('should skip rules when metric is missing from context', async () => {
      engine.registerRule(makeRule({ condition: { metric: 'cost', operator: 'gt', threshold: 100 } }));
      const alerts = await engine.evaluate(makeContext({ metrics: { progress: 50 } }));
      expect(alerts).toHaveLength(0);
    });

    it('should evaluate multiple rules and trigger matching ones', async () => {
      engine.registerRule(makeRule({
        ruleId: 'r1', type: 'COST_EXCEEDED',
        condition: { metric: 'cost', operator: 'gt', threshold: 100 },
      }));
      engine.registerRule(makeRule({
        ruleId: 'r2', type: 'TASK_DELAYED',
        condition: { metric: 'delay', operator: 'gt', threshold: 5 },
      }));
      engine.registerRule(makeRule({
        ruleId: 'r3', type: 'RISK_ESCALATED',
        condition: { metric: 'risk', operator: 'gt', threshold: 80 },
      }));

      const alerts = await engine.evaluate(makeContext({
        metrics: { cost: 200, delay: 3, risk: 90 },
      }));
      expect(alerts).toHaveLength(2);
      expect(alerts.map((a) => a.type).sort()).toEqual(['COST_EXCEEDED', 'RISK_ESCALATED']);
    });

    it('should carry correct alert fields', async () => {
      engine.registerRule(makeRule({
        ruleId: 'r1', type: 'ERROR_OCCURRED', priority: 'critical',
        condition: { metric: 'errors', operator: 'gt', threshold: 0 },
      }));
      const alerts = await engine.evaluate(makeContext({
        metrics: { errors: 3 }, entityId: 'task-42', entityType: 'task',
      }));
      expect(alerts).toHaveLength(1);
      const a = alerts[0];
    ...
    it('should support all 5 alert types', async () => {
      const types: Array<AlertRule['type']> = [
        'TASK_DELAYED', 'COST_EXCEEDED', 'RISK_ESCALATED', 'ERROR_OCCURRED', 'APPROVAL_REQUIRED',
      ];
      ...
      types.forEach((_, i) => { metrics[`m${i}`] = 1; });
  ...
  describe('deduplication', () => {
    ...
    it('should allow same type but different entityId', async () => {
      ...
      const a1 = await engine.evaluate(makeContext({ metrics: { cost: 200 }, entityId: 'task-1' }));
      const a2 = await engine.evaluate(makeContext({ metrics: { cost: 200 }, entityId: 'task-2' }));
      expect(a1).toHaveLength(1);
      expect(a2).toHaveLength(1);
    });

    it('should allow different type on same entityId', async () => {
      engine.registerRule(makeRule({ ruleId: 'r1', type: 'COST_EXCEEDED', condition: { metric: 'cost', operator: 'gt', threshold: 100 } }));
      engine.registerRule(makeRule({ ruleId: 'r2', type: 'TASK_DELAYED', condition: { metric: 'delay', operator: 'gt', threshold: 5 } }));

      const alerts = await engine.evaluate(makeContext({
        metrics: { cost: 200, delay: 10 }, entityId: 'task-1',
      }));
      expect(alerts).toHaveLength(2);

      // Second call: both deduped
      const alerts2 = await engine.evaluate(makeContext({
        metrics: { cost: 200, delay: 10 }, entityId: 'task-1',
      }));
  ...
  describe('notify()', () => {
    ...
    it('should work without onAlert callback', async () => {
      ...
      const alerts = await silentEngine.evaluate(makeContext({ metrics: { cost: 200 } }));
      expect(alerts).toHaveLength(1);
    });

    it('should use updated callback after setOnAlert', async () => {
      const newNotified: Alert[] = [];
      engine.setOnAlert((a) => { newNotified.push(a); });
      engine.registerRule(makeRule());
      await engine.evaluate(makeContext({ metrics: { cost: 200 } }));
  ...
  describe('audit trail integration', () => {
    it('should record audit entry for each triggered alert', async () => {
      engine.registerRule(makeRule());
      await engine.evaluate(makeContext({ metrics: { cost: 200 }, entityId: 'task-99' }));

      const entries = await auditTrail.query({ operationType: 'alert_triggered' });
