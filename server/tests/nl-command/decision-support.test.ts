...
const TEST_AUDIT_PATH = resolve(
  __test_dirname,
  "../../../data/__test_decision_support__/nl-audit.json"
);
...
describe("DecisionSupportEngine", () => {
  ...
  describe("analyzeRisks()", () => {
    ...
    it("should fallback to plan risk assessment when LLM returns invalid JSON", async () => {
      ...
      expect(result.overallRiskLevel).toBe(
        ...
        suggestions: [
          {
            ...
            description: "Switch to GPT-3.5",
            estimatedImpact: "Save 30%",
          },
          {
            title: "Batch tasks",
            description: "Combine similar tasks",
            estimatedImpact: "Save 10%",
          },
        ],
      });
      const provider = makeMockProvider(llmResponse);
    ...
    it("should return resource suggestions from LLM", async () => {
      const llmResponse = JSON.stringify({
        suggestions: [
          {
            title: "Add agent",
            description: "Add one more TS agent",
            estimatedImpact: "Reduce duration by 20%",
          },
        ],
      });
      const provider = makeMockProvider(llmResponse);
  ...
  describe("generateOptimizationReport()", () => {
    ...
    it("should flag poor duration accuracy in recommendations", async () => {
      ...
      expect(
        report.recommendations.some(r => r.toLowerCase().includes("duration"))
      ).toBe(true);
    });
  ...
  describe("LLM retry behavior", () => {
    it("should retry on temporary errors", async () => {
      ...
      const provider: ILLMProvider = {
        ...
        healthCheck: vi
          .fn()
          .mockResolvedValue({ healthy: true, provider: "mock" }),
    ...
    it("should throw after max retries exhausted", async () => {
      ...
      await expect(engine.analyzeRisks(makePlan())).rejects.toThrow(
        "always fails"
      );
