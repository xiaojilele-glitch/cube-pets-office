...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_mission_decomposer__/nl-audit.json');
...
function makeAnalysis(overrides: Partial<CommandAnalysis> = {}): CommandAnalysis {
  return {
    ...
    objectives: overrides.objectives ?? ['Improve architecture', 'Reduce latency'],
    ...
    generate: vi.fn(async (_messages: LLMMessage[], _options?: LLMGenerateOptions): Promise<LLMGenerateResult> => {
      ...
      if (callIndex % 2 === 1) {
            ...
            missions: [
              {
                ...
                objectives: ['Define service boundaries', 'Design API contracts'],
              ...
              },
              {
                ...
                objectives: ['Implement payment flow', 'Implement refund flow'],
                constraints: [],
        ...
        return {
          content: JSON.stringify({
            dependencies: [
              { fromMissionId: 'mission-2', toMissionId: 'mission-1', type: 'depends_on', description: 'Implementation needs architecture' },
              { fromMissionId: 'mission-3', toMissionId: 'mission-2', type: 'blocks', description: 'Testing needs implementation' },
            ],
          }),
          usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
          latencyMs: 50,
...
describe('MissionDecomposer', () => {
  ...
  describe('decompose()', () => {
    ...
    it('should generate missions with all required fields (Req 3.2, 3.3)', async () => {
      ...
      for (const mission of result.missions) {
        ...
        expect(['critical', 'high', 'medium', 'low']).toContain(mission.priority);
      }
    });
    ...
    it('should identify dependency relationships between missions (Req 3.4)', async () => {
      ...
      expect(result.dependencies).toHaveLength(2);
      const dep1 = result.dependencies.find(d => d.fromMissionId === 'mission-2');
      expect(dep1).toBeDefined();
      expect(dep1!.toMissionId).toBe('mission-1');
      expect(dep1!.type).toBe('depends_on');

      const dep2 = result.dependencies.find(d => d.fromMissionId === 'mission-3');
    ...
    it('should record a success audit entry (Req 3.6)', async () => {
      ...
      const entries = await auditTrail.query({ operationType: 'decomposition_completed' });
    ...
    it('should throw CyclicDependencyError and record failure audit on cyclic dependencies (Req 3.4)', async () => {
        ...
        generate: vi.fn(async () => {
          if ((cyclicProvider.generate as any).mock.calls.length <= 1) {
            return {
              content: JSON.stringify({
                missions: [
                  { missionId: 'm-a', title: 'A', description: '', objectives: [], constraints: [], estimatedDuration: 60, estimatedCost: 100, priority: 'medium' },
                  { missionId: 'm-b', title: 'B', description: '', objectives: [], constraints: [], estimatedDuration: 60, estimatedCost: 100, priority: 'medium' },
                ],
              }),
              usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
      ...
      await expect(cyclicDecomposer.decompose(makeCommand())).rejects.toThrow(CyclicDependencyError);
    ...
    it('should throw when LLM returns empty mission list', async () => {
      ...
      await expect(emptyDecomposer.decompose(makeCommand())).rejects.toThrow('LLM returned empty or invalid mission list');
    });
    ...
    it('should skip dependencies with invalid mission IDs', async () => {
        ...
        generate: vi.fn(async () => {
          ...
          if (callIdx === 1) {
            return {
              content: JSON.stringify({
                missions: [
                  { missionId: 'mx', title: 'X', description: '', objectives: [], constraints: [], estimatedDuration: 30, estimatedCost: 10, priority: 'medium' },
                  { missionId: 'my', title: 'Y', description: '', objectives: [], constraints: [], estimatedDuration: 30, estimatedCost: 10, priority: 'medium' },
                ],
              }),
              usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
              latencyMs: 50, model: 'mock', provider: 'mock',
          ...
          return {
            content: JSON.stringify({
              dependencies: [
                { fromMissionId: 'mx', toMissionId: 'nonexistent', type: 'blocks' },
      ...
      const d = new MissionDecomposer({ llmProvider: invalidDepProvider, model: 'mock', auditTrail });
    ...
    it('should return no dependencies for a single mission', async () => {
      ...
      const singleProvider: ILLMProvider = {
        ...
        generate: vi.fn(async () => {
          ...
          return {
            content: JSON.stringify({
              missions: [
                { missionId: 'solo', title: 'Solo mission', description: '', objectives: [], constraints: [], estimatedDuration: 60, estimatedCost: 50, priority: 'medium' },
              ],
            }),
            usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
            latencyMs: 50, model: 'mock', provider: 'mock',
      ...
      const d = new MissionDecomposer({ llmProvider: singleProvider, model: 'mock', auditTrail });
      const result = await d.decompose(makeCommand());
  ...
  describe('generateOrganization()', () => {
    it('should trigger organization generation callback (Req 14.1)', async () => {
      const orgCallback = vi.fn(async (_missionId: string, _complexity: number) => ({
      ...
      const result = await decomposerWithOrg.generateOrganization('mission-1', 5);
