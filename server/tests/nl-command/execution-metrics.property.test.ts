...
import type { NLExecutionPlan, TimelineEntry } from '../../../shared/nl-command/contracts.js';
...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_exec_metrics_prop__/nl-audit.json');
...
const planWithExpectedMetricsArb: fc.Arbitrary<{
  ...
  .map(([entries, totalBudget]) => {
    ...
    entries.forEach((e, i) => {
      ...
      taskCosts[e.entityId] = i === entries.length - 1
    ...
    const plan: NLExecutionPlan = {
      ...
      commandId: 'cmd-prop',
      status: 'completed',
      missions: [{ missionId: 'm1', title: 'M1', description: 'd', objectives: ['o'], constraints: [], estimatedDuration: 60, estimatedCost: totalBudget, priority: 'medium' }],
      tasks: entries.map((e) => ({ taskId: e.entityId, title: e.entityId, description: 'd', objectives: ['o'], constraints: [], estimatedDuration: e.duration, estimatedCost: taskCosts[e.entityId], requiredSkills: ['ts'], priority: 'medium' as const })),
      timeline: { startDate: '2026-01-01', endDate: '2026-12-31', criticalPath: [], milestones: [], entries },
      resourceAllocation: { entries: [], totalAgents: 1, peakConcurrency: 1 },
      riskAssessment: { risks: [], overallRiskLevel: 'low' },
      costBudget: { totalBudget, missionCosts: { m1: totalBudget }, taskCosts, agentCosts: {}, modelCosts: {}, currency: 'USD' },
...
const emptyTimelinePlanArb: fc.Arbitrary<NLExecutionPlan> = fc
  ...
  .map((totalBudget) => ({
    ...
    missions: [],
    tasks: [],
    timeline: { startDate: '2026-01-01', endDate: '2026-01-01', criticalPath: [], milestones: [], entries: [] },
    resourceAllocation: { entries: [], totalAgents: 0, peakConcurrency: 0 },
    riskAssessment: { risks: [], overallRiskLevel: 'low' as const },
    costBudget: { totalBudget, missionCosts: {}, taskCosts: {}, agentCosts: {}, modelCosts: {}, currency: 'USD' },
    contingencyPlan: { alternatives: [], degradationStrategies: [], rollbackPlan: '' },
    createdAt: Date.now(),
    updatedAt: Date.now(),
  }));
...
describe('Property 21: execution metrics collection and deviation calculation', () => {
  ...
  beforeEach(() => {
    ...
    engine = new DecisionSupportEngine({ llmProvider: makeMockProvider() as any, model: 'test', auditTrail });
  });
  ...
  it('durationDeviation SHALL equal (actualDuration - plannedDuration) / plannedDuration', () => {
    fc.assert(
      fc.asyncProperty(planWithExpectedMetricsArb, async ({ plan, expectedPlannedDuration, expectedActualDuration }) => {
        const metrics = await engine.collectExecutionData(plan);
        const expectedDeviation = expectedPlannedDuration === 0
          ? 0
          : (expectedActualDuration - expectedPlannedDuration) / expectedPlannedDuration;
  ...
  it('costDeviation SHALL equal (actualCost - plannedCost) / plannedCost', () => {
    fc.assert(
      fc.asyncProperty(planWithExpectedMetricsArb, async ({ plan, expectedPlannedCost, expectedActualCost }) => {
        const metrics = await engine.collectExecutionData(plan);
        const expectedDeviation = expectedPlannedCost === 0
          ? 0
          : (expectedActualCost - expectedPlannedCost) / expectedPlannedCost;
    ...
    fc.assert(
      fc.asyncProperty(emptyTimelinePlanArb, async (plan) => {
        ...
        if (plan.costBudget.totalBudget === 0) {
          ...
          const expected = (0 - plan.costBudget.totalBudget) / plan.costBudget.totalBudget;
