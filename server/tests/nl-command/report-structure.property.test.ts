...
const TEST_AUDIT_PATH = resolve(
  __test_dirname,
  "../../../data/__test_report_prop__/nl-audit.json"
);
...
const planArb: fc.Arbitrary<NLExecutionPlan> = fc
  ...
  .map(({ missions, tasks, risks, overallRiskLevel, planStatus }) => {
    const totalBudget =
      missions.reduce((s, m) => s + m.estimatedCost, 0) || 100;
    ...
    fc.assert(
      fc.property(planArb, plan => {
        ...
        expect(["low", "medium", "high", "critical"]).toContain(
          report.riskAnalysis.overallRiskLevel
        );
  ...
  it("progressAnalysis mission/task counts SHALL match plan input", () => {
    fc.assert(
      fc.property(planArb, plan => {
        ...
        expect(report.progressAnalysis.totalMissions).toBe(
          plan.missions.length
        );
        expect(report.progressAnalysis.totalTasks).toBe(plan.tasks.length);
        expect(report.progressAnalysis.completedMissions).toBeLessThanOrEqual(
          plan.missions.length
        );
        expect(report.progressAnalysis.completedTasks).toBeLessThanOrEqual(
          plan.tasks.length
        );
        expect(report.progressAnalysis.overallProgress).toBeGreaterThanOrEqual(
          0
        );
