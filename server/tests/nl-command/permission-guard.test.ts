...
describe('PermissionGuard', () => {
  ...
  describe('default role permissions', () => {
    it('admin should have all permissions', () => {
      ...
      expect(perms).toEqual(expect.arrayContaining(['view', 'create', 'edit', 'approve', 'execute', 'cancel']));
      expect(perms).toHaveLength(6);
    });

    it('manager should have view, create, edit, approve', () => {
      const perms = guard.getPermissions('manager');
      expect(perms).toEqual(expect.arrayContaining(['view', 'create', 'edit', 'approve']));
    ...
    it('operator should have view, create, edit, execute', () => {
      ...
      expect(perms).toEqual(expect.arrayContaining(['view', 'create', 'edit', 'execute']));
      expect(perms).toHaveLength(4);
  ...
  describe('entity-level overrides', () => {
    ...
    it('should scope overrides to entityType', () => {
      ...
      expect(guard.checkPermission('u1', 'viewer', 'cancel', 'mission')).toBe(true);
    ...
    it('should scope overrides to entityType + entityId', () => {
      guard.setOverride('u1', { grant: ['approve'], deny: [] }, 'task', 'task-42');

      // Exact match → granted
      expect(guard.checkPermission('u1', 'viewer', 'approve', 'task', 'task-42')).toBe(true);
      // Same entityType, different entityId → not granted
      expect(guard.checkPermission('u1', 'viewer', 'approve', 'task', 'task-99')).toBe(false);
    ...
    it('should apply overrides in priority order (global < entityType < entityType+entityId)', () => {
      ...
      guard.setOverride('u1', { grant: ['execute'], deny: [] }, 'mission', 'mission-1');

      // viewer + global grant → true
      expect(guard.checkPermission('u1', 'viewer', 'execute')).toBe(true);
      // viewer + global grant + mission deny → false
      expect(guard.checkPermission('u1', 'viewer', 'execute', 'mission')).toBe(false);
      // viewer + global grant + mission deny + mission-1 grant → true
      expect(guard.checkPermission('u1', 'viewer', 'execute', 'mission', 'mission-1')).toBe(true);
    });
  ...
  describe('getPermissions() with userId and overrides', () => {
    it('should return effective permissions including overrides', () => {
      ...
      expect(perms).toEqual(expect.arrayContaining(['view', 'execute', 'cancel']));
    });
