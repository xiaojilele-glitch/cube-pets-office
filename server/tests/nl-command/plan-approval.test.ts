...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_plan_approval__/nl-audit.json');
...
function makePlan(overrides: Partial<NLExecutionPlan> = {}): NLExecutionPlan {
  return {
    ...
    costBudget: { totalBudget: 0, missionCosts: {}, taskCosts: {}, agentCosts: {}, modelCosts: {}, currency: 'CNY' },
...
describe('PlanApproval', () => {
  ...
  describe('createApprovalRequest()', () => {
    ...
    it('should accept custom approvers', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice', 'bob']);
  ...
  describe('submitApproval()', () => {
    it('should record an approval decision', async () => {
      ...
      const updated = await approval.submitApproval(req.requestId, 'alice', 'approved', 'LGTM');
    ...
    it('should throw for unknown request', async () => {
      await expect(approval.submitApproval('nonexistent', 'alice', 'approved')).rejects.toThrow(
    ...
    it('should throw for non-required approver', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice']);
      await expect(approval.submitApproval(req.requestId, 'bob', 'approved')).rejects.toThrow(
    ...
    it('should throw for duplicate submission', async () => {
      ...
      await expect(approval.submitApproval(req.requestId, 'alice', 'rejected')).rejects.toThrow(
        'already submitted',
      );
    });
  });
  ...
  describe('approval status computation', () => {
    it('should remain pending until all approvers submit', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice', 'bob']);
      const afterAlice = await approval.submitApproval(req.requestId, 'alice', 'approved');
    ...
    it('should transition to approved when all approve', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice', 'bob']);
      await approval.submitApproval(req.requestId, 'alice', 'approved');
      const final = await approval.submitApproval(req.requestId, 'bob', 'approved');
      expect(final.status).toBe('approved');
    });

    it('should transition to rejected if any approver rejects', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice', 'bob']);
      const afterReject = await approval.submitApproval(req.requestId, 'alice', 'rejected', 'Too risky');
      expect(afterReject.status).toBe('rejected');
    });

    it('should transition to revision_requested if any approver requests revision', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice', 'bob']);
      const afterRevision = await approval.submitApproval(req.requestId, 'alice', 'revision_requested', 'Needs changes');
      expect(afterRevision.status).toBe('revision_requested');
    });

    it('rejected takes precedence over revision_requested', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice', 'bob', 'charlie']);
      await approval.submitApproval(req.requestId, 'alice', 'revision_requested');
      const afterReject = await approval.submitApproval(req.requestId, 'bob', 'rejected');
      expect(afterReject.status).toBe('rejected');
    });

    it('single approver approved → status approved', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['admin']);
      const result = await approval.submitApproval(req.requestId, 'admin', 'approved');
  ...
  describe('isApprovalComplete()', () => {
    ...
    it('should return true for approved request', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice']);
      const updated = await approval.submitApproval(req.requestId, 'alice', 'approved');
      expect(approval.isApprovalComplete(updated)).toBe(true);
    });

    it('should return true for rejected request', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice']);
      const updated = await approval.submitApproval(req.requestId, 'alice', 'rejected');
      expect(approval.isApprovalComplete(updated)).toBe(true);
    });

    it('should return true for revision_requested', async () => {
      const req = await approval.createApprovalRequest(makePlan(), ['alice']);
      const updated = await approval.submitApproval(req.requestId, 'alice', 'revision_requested');
      expect(approval.isApprovalComplete(updated)).toBe(true);
    });
  });
  ...
  describe('audit trail integration', () => {
    ...
    it('should record approval_completed when status finalizes', async () => {
      ...
      const entries = await auditTrail.query({ operationType: 'approval_completed' });
      expect(entries.length).toBeGreaterThanOrEqual(1);
    });
  });
