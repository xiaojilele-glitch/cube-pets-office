...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_task_decomposer__/nl-audit.json');
    ...
    generate: vi.fn(async (_messages: LLMMessage[], _options?: LLMGenerateOptions): Promise<LLMGenerateResult> => {
      ...
      if (callIndex % 2 === 1) {
            ...
            tasks: [
              {
                ...
                description: 'Design the REST API schema for the payment module',
                objectives: ['Define endpoints', 'Define request/response types'],
                constraints: [{ type: 'quality', description: 'RESTful design' }],
              ...
              },
              {
                ...
                description: 'Write integration tests for the payment service',
        ...
        return {
          content: JSON.stringify({
            dependencies: [
              { fromTaskId: 'task-2', toTaskId: 'task-1', type: 'depends_on', description: 'Service needs API schema' },
              { fromTaskId: 'task-3', toTaskId: 'task-2', type: 'blocks', description: 'Tests need service implementation' },
            ],
          }),
          usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
          latencyMs: 50,
...
function makeMission(overrides: Partial<DecomposedMission> = {}): DecomposedMission {
  return {
    missionId: overrides.missionId ?? 'mission-1',
    title: overrides.title ?? 'Refactor payment module',
    description: overrides.description ?? 'Refactor the payment module for better architecture',
...
describe('TaskDecomposer', () => {
  ...
  describe('decompose()', () => {
    ...
    it('should throw and record failure audit on cyclic dependencies', async () => {
        ...
        generate: vi.fn(async () => {
          ...
          if ((cyclicProvider.generate as any).mock.calls.length <= 1) {
            return {
              content: JSON.stringify({
                tasks: [
                  { taskId: 't-a', title: 'A', description: '', objectives: [], constraints: [], estimatedDuration: 30, estimatedCost: 10, requiredSkills: [], priority: 'medium' },
                  { taskId: 't-b', title: 'B', description: '', objectives: [], constraints: [], estimatedDuration: 30, estimatedCost: 10, requiredSkills: [], priority: 'medium' },
                ],
              }),
              usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
      ...
      await expect(cyclicDecomposer.decompose(makeMission(), makeContext())).rejects.toThrow(CyclicDependencyError);
    ...
    it('should throw when LLM returns empty task list', async () => {
      ...
      await expect(emptyDecomposer.decompose(makeMission(), makeContext())).rejects.toThrow('LLM returned empty or invalid task list');
    });
    ...
    it('should skip dependencies with invalid task IDs', async () => {
        ...
        generate: vi.fn(async () => {
          ...
          if (callIdx === 1) {
            return {
              content: JSON.stringify({
                tasks: [
                  { taskId: 'task-x', title: 'X', description: '', objectives: [], constraints: [], estimatedDuration: 30, estimatedCost: 10, requiredSkills: [], priority: 'medium' },
                  { taskId: 'task-y', title: 'Y', description: '', objectives: [], constraints: [], estimatedDuration: 30, estimatedCost: 10, requiredSkills: [], priority: 'medium' },
                ],
              }),
              usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
              latencyMs: 50, model: 'mock', provider: 'mock',
          ...
          return {
            content: JSON.stringify({
              dependencies: [
                { fromTaskId: 'task-x', toTaskId: 'task-nonexistent', type: 'blocks' },
                { fromTaskId: 'task-x', toTaskId: 'task-x', type: 'blocks' }, // self-reference
                { fromTaskId: 'task-y', toTaskId: 'task-x', type: 'depends_on' },
              ],
            }),
            usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
            latencyMs: 50, model: 'mock', provider: 'mock',
      ...
      const d = new TaskDecomposer({ llmProvider: invalidDepProvider, model: 'mock', auditTrail });
    ...
    it('should return no dependencies for a single task', async () => {
      ...
      const singleTaskProvider: ILLMProvider = {
        ...
        generate: vi.fn(async () => {
          ...
          return {
            content: JSON.stringify({
              tasks: [
                { taskId: 'task-only', title: 'Only task', description: '', objectives: [], constraints: [], estimatedDuration: 60, estimatedCost: 50, requiredSkills: [], priority: 'medium' },
              ],
            }),
            usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
            latencyMs: 50, model: 'mock', provider: 'mock',
      ...
      const d = new TaskDecomposer({ llmProvider: singleTaskProvider, model: 'mock', auditTrail });
      const result = await d.decompose(makeMission(), makeContext());
