...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_plan_adjustment__/nl-audit.json');
...
function makePlan(overrides: Partial<NLExecutionPlan> = {}): NLExecutionPlan {
  ...
  return {
    ...
    timeline: overrides.timeline ?? {
      ...
      milestones: [],
      entries: [
        { entityId: 'm-1', entityType: 'mission', startTime: now - 1800_000, endTime: now + 1800_000, duration: 60, isCriticalPath: true },
        { entityId: 't-1', entityType: 'task', startTime: now - 1800_000, endTime: now + 1800_000, duration: 30, isCriticalPath: false },
      ],
    },
    resourceAllocation: overrides.resourceAllocation ?? { entries: [], totalAgents: 1, peakConcurrency: 1 },
    riskAssessment: overrides.riskAssessment ?? { risks: [], overallRiskLevel: 'low' },
    ...
    contingencyPlan: overrides.contingencyPlan ?? { alternatives: [], degradationStrategies: [], rollbackPlan: '' },
    createdAt: overrides.createdAt ?? now,
    updatedAt: overrides.updatedAt ?? now,
  };
}

function makeChange(overrides: Partial<AdjustmentChange> = {}): AdjustmentChange {
  return {
...
describe('PlanAdjustmentManager', () => {
  ...
  describe('proposeAdjustment()', () => {
    it('should create a proposed adjustment with correct fields', async () => {
      ...
      const adj = await manager.proposeAdjustment('plan-1', 'Task delayed', changes, DEFAULT_IMPACT);
    ...
    it('should allow approvalRequired=false', async () => {
      const adj = await manager.proposeAdjustment('plan-1', 'Minor fix', [makeChange()], DEFAULT_IMPACT, false);
      expect(adj.approvalRequired).toBe(false);
    });

    it('should record an audit entry', async () => {
      await manager.proposeAdjustment('plan-1', 'reason', [makeChange()], DEFAULT_IMPACT);
      const entries = await auditTrail.query({ operationType: 'adjustment_proposed' });
  ...
  describe('applyAdjustment()', () => {
    it('should apply changes to the plan and update status (no approval required)', async () => {
      ...
      const changes: AdjustmentChange[] = [
        { entityId: 't-1', entityType: 'task', field: 'estimatedDuration', oldValue: 30, newValue: 60 },
      ];

      const adj = await manager.proposeAdjustment('plan-1', 'extend task', changes, DEFAULT_IMPACT, false);
    ...
    it('should apply mission changes', async () => {
      const plan = makePlan();
      const changes: AdjustmentChange[] = [
        { entityId: 'm-1', entityType: 'mission', field: 'estimatedCost', oldValue: 100, newValue: 200 },
      ];

      const adj = await manager.proposeAdjustment('plan-1', 'cost increase', changes, DEFAULT_IMPACT, false);
    ...
    it('should require approval when approvalRequired=true', async () => {
      const plan = makePlan();
      const adj = await manager.proposeAdjustment('plan-1', 'reason', [makeChange()], DEFAULT_IMPACT, true);

      await expect(manager.applyAdjustment(adj.adjustmentId, plan)).rejects.toThrow(
        'requires approval',
      );
    });
    ...
    it('should throw for unknown adjustment', async () => {
      const plan = makePlan();
      await expect(manager.applyAdjustment('nonexistent', plan)).rejects.toThrow('not found');
    });

    it('should throw for already applied adjustment', async () => {
      const plan = makePlan();
      const adj = await manager.proposeAdjustment('plan-1', 'reason', [makeChange()], DEFAULT_IMPACT, false);
      await manager.applyAdjustment(adj.adjustmentId, plan);
      await expect(manager.applyAdjustment(adj.adjustmentId, plan)).rejects.toThrow('already applied');
    });
    ...
    it('should throw for rejected adjustment', async () => {
      ...
      await expect(manager.applyAdjustment(adj.adjustmentId, plan)).rejects.toThrow('rejected');
    });
  ...
  describe('approveAdjustment()', () => {
    it('should transition status to approved', async () => {
      const adj = await manager.proposeAdjustment('plan-1', 'reason', [makeChange()], DEFAULT_IMPACT);
    ...
    it('should throw for non-proposed adjustment', async () => {
      const adj = await manager.proposeAdjustment('plan-1', 'reason', [makeChange()], DEFAULT_IMPACT);
      manager.approveAdjustment(adj.adjustmentId);
      expect(() => manager.approveAdjustment(adj.adjustmentId)).toThrow('Can only approve proposed');
    });

    it('should throw for unknown adjustment', () => {
      expect(() => manager.approveAdjustment('nonexistent')).toThrow('not found');
    });
  });
  ...
  describe('rejectAdjustment()', () => {
    ...
    it('should throw for non-proposed adjustment', async () => {
      const adj = await manager.proposeAdjustment('plan-1', 'reason', [makeChange()], DEFAULT_IMPACT);
      manager.rejectAdjustment(adj.adjustmentId);
      expect(() => manager.rejectAdjustment(adj.adjustmentId)).toThrow('Can only reject proposed');
    });
  });
    ...
    it('should flag delayed tasks when actual progress < expected', () => {
      ...
      const plan = makePlan({
        timeline: {
          ...
          milestones: [],
          entries: [
            { entityId: 't-1', entityType: 'task', startTime: now - 3600_000, endTime: now + 3600_000, duration: 120, isCriticalPath: false },
          ],
        },
      });
    ...
    it('should flag tasks past their end time with incomplete progress', () => {
      ...
      const plan = makePlan({
        timeline: {
          ...
          milestones: [],
          entries: [
            { entityId: 't-1', entityType: 'task', startTime: now - 7200_000, endTime: now - 3600_000, duration: 60, isCriticalPath: false },
          ],
        },
      });
    ...
    it('should not flag tasks that have not started yet', () => {
      ...
      const plan = makePlan({
        timeline: {
          ...
          milestones: [],
          entries: [
            { entityId: 't-1', entityType: 'task', startTime: now + 3600_000, endTime: now + 7200_000, duration: 60, isCriticalPath: false },
          ],
        },
      });
  ...
  describe('listAdjustments()', () => {
    it('should return all adjustments for a plan', async () => {
      await manager.proposeAdjustment('plan-1', 'reason1', [makeChange()], DEFAULT_IMPACT);
      await manager.proposeAdjustment('plan-1', 'reason2', [makeChange()], DEFAULT_IMPACT);
      await manager.proposeAdjustment('plan-2', 'other', [makeChange()], DEFAULT_IMPACT);
