...
const dagArb = (
  minNodes = 1,
  maxNodes = 12
): fc.Arbitrary<{ nodeIds: string[]; edges: Edge[] }> =>
 ...
 */
const cyclicGraphArb: fc.Arbitrary<{ nodeIds: string[]; edges: Edge[] }> = fc
  .integer({ min: 2, max: 8 })
  .chain(n => {
    ...
    fc.assert(
      fc.property(
        fc
          ...
          .map(n => Array.from({ length: n }, (_, i) => `iso-${i}`)),
        nodeIds => {
  ...
  it("for any cyclic graph, topoSortWithGroups SHALL throw CyclicDependencyError", () => {
    fc.assert(
      fc.property(cyclicGraphArb, ({ nodeIds, edges }) => {
        expect(() => topoSortWithGroups(nodeIds, edges)).toThrow(
          CyclicDependencyError
        );
      }),
      { numRuns: 20 }
