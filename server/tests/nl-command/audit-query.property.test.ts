...
const TEST_AUDIT_PATH = resolve(
  __dirname,
  "../../../data/test-audit-query-prop.json"
);
...
const entityIdArb = fc.constantFrom(
  "ent-1",
  "ent-2",
  "ent-3",
  "ent-4",
...
function matchesFilter(entry: AuditEntry, filter: AuditQueryFilter): boolean {
  ...
  if (filter.endTime !== undefined && entry.timestamp > filter.endTime)
    return false;
  if (filter.operator !== undefined && entry.operator !== filter.operator)
    return false;
  if (
    filter.operationType !== undefined &&
    entry.operationType !== filter.operationType
  )
    return false;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ entries, filter }) => {
          ...
          const expectedSorted = expectedMatching.sort(
          ...
          expect(result.map(e => e.entryId)).toEqual(
            expectedPage.map(e => e.entryId)
          );
        }
      ),
      { numRuns: 20 }
