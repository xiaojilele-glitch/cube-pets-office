...
const TEST_AUDIT_PATH = resolve(
  __test_dirname,
  "../../../data/__test_comment_audit__/nl-audit.json"
);
...
describe("CommentManager", () => {
  ...
  describe("editComment()", () => {
    ...
    it("should re-parse mentions after edit", async () => {
      const c = await manager.addComment(
        "task-1",
        "task",
        "user-1",
        "Hello @alice"
  ...
  describe("parseMentions()", () => {
    ...
    it("should extract multiple mentions", () => {
      expect(manager.parseMentions("@alice @bob @charlie")).toEqual([
        "alice",
        "bob",
        "charlie",
      ]);
    ...
    it("should deduplicate mentions", () => {
      expect(manager.parseMentions("@alice @bob @alice")).toEqual([
        "alice",
        "bob",
      ]);
    });
    ...
    it("should handle mentions with hyphens and underscores", () => {
      expect(manager.parseMentions("@user-1 @user_2")).toEqual([
        "user-1",
        "user_2",
      ]);
    });
    ...
    it("should handle mentions at start, middle, and end", () => {
      expect(manager.parseMentions("@start middle @mid end @end")).toEqual([
        "start",
        "mid",
        "end",
      ]);
