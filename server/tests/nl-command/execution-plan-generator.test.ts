...
const TEST_AUDIT_PATH = resolve(
  __test_dirname,
  "../../../data/__test_exec_plan_gen__/nl-audit.json"
);
              ...
              alternatives: [
                {
                  ...
                  trigger: "Timeline exceeded by 50%",
                  action: "Cut non-critical tasks",
  ...
  return {
    ...
    dependencies:
      missions.length > 1
        ? [
            {
              fromMissionId: missions[1].missionId,
              toMissionId: missions[0].missionId,
...
function makeTaskDecomposition(
  missionId: string,
  ...
  return {
    ...
    dependencies:
      tasks.length > 1
        ? [
            {
              fromTaskId: tasks[1].taskId,
              toTaskId: tasks[0].taskId,
              type: "depends_on" as const,
            },
          ]
        : [],
...
describe("ExecutionPlanGenerator", () => {
  ...
  describe("generate()", () => {
    it("should return a valid NLExecutionPlan with all required fields (Req 5.1)", async () => {
      const missions = [
        makeMission("m-1", 200, 60),
        makeMission("m-2", 300, 90),
      ];
      const tasks = [
        makeTask("t-1", 100, 30, ["backend"]),
        makeTask("t-2", 100, 30, ["frontend"]),
      ];
    ...
    it("should enforce Property 8: totalBudget = sum of missionCosts (Req 5.5)", async () => {
      ...
      const sumMissionCosts = Object.values(
    ...
    it("should generate contingency plan (Req 5.6)", async () => {
      ...
      expect(Array.isArray(plan.contingencyPlan.degradationStrategies)).toBe(
        true
      );
  ...
  describe("computeCriticalPath()", () => {
    it("should return critical path IDs and total duration", async () => {
      const missions = [
        makeMission("m-1", 100, 60),
        makeMission("m-2", 200, 120),
      ];
  ...
  describe("LLM fallback behavior", () => {
    it("should use fallback risk assessment when LLM fails", async () => {
      ...
      const plan = await failGenerator.generate(
        command,
        decomposition,
        taskDecomps
      );
