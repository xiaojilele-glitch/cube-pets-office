...
function filterMissions(
  ...
  return missions.filter(m => {
    if (criteria.priority !== undefined && m.priority !== criteria.priority)
      return false;
    return true;
  });
}
...
const priorityArb = fc.constantFrom<CommandPriority>(
  "critical",
  "high",
  "medium",
  "low"
);
const sortFieldArb = fc.constantFrom<SortField>(
  "title",
  "priority",
  "estimatedDuration",
  "estimatedCost"
...
const missionArb: fc.Arbitrary<DecomposedMission> = fc.record({
  ...
  objectives: fc.array(fc.string({ minLength: 1, maxLength: 20 }), {
    minLength: 1,
    maxLength: 3,
  }),
  ...
  it("filtered result SHALL contain only entries matching all filter criteria", () => {
    fc.assert(
      fc.property(missionListArb, priorityArb, (missions, priority) => {
        ...
        const expectedCount = missions.filter(
        ...
        (missions, field, direction) => {
          ...
          for (let i = 1; i < result.length; i++) {
            ...
            switch (field) {
              ...
              case "priority":
                cmp =
                  PRIORITY_ORDER[prev.priority] - PRIORITY_ORDER[curr.priority];
                break;
              case "estimatedDuration":
  ...
  it("filter then sort SHALL preserve filter invariant and sort order", () => {
    fc.assert(
      fc.property(
        ...
        sortFieldArb,
        sortDirArb,
        (missions, priority, field, direction) => {
