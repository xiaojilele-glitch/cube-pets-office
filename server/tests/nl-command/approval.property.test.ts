...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_approval_prop__/nl-audit.json');
...
const approversArb = fc
  .uniqueArray(fc.stringMatching(/^[a-z]{3,8}$/), { minLength: 1, maxLength: 5 })
  .filter((arr) => arr.length >= 1);

type Decision = 'approved' | 'rejected' | 'revision_requested';
const decisionArb: fc.Arbitrary<Decision> = fc.constantFrom('approved', 'rejected', 'revision_requested');
    ...
    await fc.assert(
      fc.asyncProperty(approversArb, async (approvers) => {
        ...
        for (let i = 0; i < approvers.length - 1; i++) {
          const updated = await approval.submitApproval(req.requestId, approvers[i], 'approved');
    ...
    await fc.assert(
      fc.asyncProperty(approversArb, async (approvers) => {
        ...
        for (const approver of approvers) {
          result = await approval.submitApproval(req.requestId, approver, 'approved');
        }
      ...
      fc.asyncProperty(
        ...
        async (approvers, rawIdx) => {
          ...
          for (let i = 0; i < approvers.length; i++) {
            ...
            const decision: Decision = i === rejectIdx ? 'rejected' : 'approved';
            result = await approval.submitApproval(req.requestId, approvers[i], decision);
          }
      ...
      fc.asyncProperty(
        ...
        async (approvers, decisionStream) => {
          ...
          for (let i = 0; i < approvers.length; i++) {
            ...
            result = await approval.submitApproval(req.requestId, approvers[i], decisions[i]);
          }

          const submitted = result.approvals;
          const hasRejected = submitted.some((d) => d.decision === 'rejected');
          const hasRevision = submitted.some((d) => d.decision === 'revision_requested');
