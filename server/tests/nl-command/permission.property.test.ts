...
const permissionArb: fc.Arbitrary<Permission> = fc.constantFrom(
  "view",
  "create",
  "edit",
  "approve",
...
const permissionOverrideArb: fc.Arbitrary<PermissionOverride> = fc
  ...
  .chain(grant => {
    ...
    return fc
      .subarray(remaining, { minLength: 0 })
      .map(deny => ({ grant, deny }));
  });
    ...
    fc.assert(
      fc.property(
        ...
        (userId, role, permission) => {
          ...
          expect(guard.checkPermission(userId, role, permission)).toBe(
            expected
          );
        }
      ),
      ...
      fc.property(
        ...
        (userId, role, override, entityType, entityId) => {
          ...
          for (const perm of ALL_PERMISSIONS) {
            expect(
              guard.checkPermission(userId, role, perm, entityType, entityId)
            ).toBe(basePerms.has(perm));
          }
        }
      ),
      { numRuns: 20 }
  ...
  it("overrides for one user SHALL NOT affect another user", () => {
    fc.assert(
      fc.property(
        ...
        fc.constantFrom("user-c", "user-d"),
        roleArb,
        permissionArb,
        (targetUser, otherUser, role, permission) => {
          guard.setOverride(targetUser, { grant: [permission], deny: [] });
          const expected = DEFAULT_ROLE_PERMISSIONS[role].includes(permission);
          expect(guard.checkPermission(otherUser, role, permission)).toBe(
            expected
          );
        }
      ),
