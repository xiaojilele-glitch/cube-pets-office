...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_cmd_analyzer__/nl-audit.json');

// ─── Mock LLM Provider ───

function createMockLLMProvider(responseMap?: Record<string, string>): ILLMProvider {
  return {
    name: 'mock',
    generate: vi.fn(async (messages: LLMMessage[], options?: LLMGenerateOptions): Promise<LLMGenerateResult> => {
      ...
      if (responseMap) {
        for (const [key, val] of Object.entries(responseMap)) {
          if (content.includes(key)) {
            return { content: val, usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 }, latencyMs: 50, model: 'mock', provider: 'mock' };
          }
        }
      }
      ...
      return {
        content: JSON.stringify({
          ...
          entities: [{ name: 'payment-module', type: 'module', description: 'Payment processing module' }],
          constraints: [{ type: 'quality', description: 'zero downtime' }],
          objectives: ['Improve payment module architecture'],
          risks: [{ id: 'r-1', description: 'Service disruption', level: 'medium', probability: 0.3, impact: 0.7, mitigation: 'Blue-green deployment' }],
...
function makeCommand(overrides: Partial<StrategicCommand> = {}): StrategicCommand {
  return {
    ...
    commandText: overrides.commandText ?? 'Refactor the payment module with zero downtime',
...
function makeAnalysis(overrides: Partial<CommandAnalysis> = {}): CommandAnalysis {
  return {
    ...
    entities: overrides.entities ?? [{ name: 'payment-module', type: 'module' }],
    constraints: overrides.constraints ?? [{ type: 'quality', description: 'zero downtime' }],
    ...
    clarificationTopics: overrides.clarificationTopics ?? ['deployment strategy'],
  };
}
...
describe('CommandAnalyzer', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should record audit entry after analysis', async () => {
      ...
      const entries = await auditTrail.query({ operationType: 'command_analyzed' });
    ...
    it('should clamp confidence to [0, 1]', async () => {
      ...
      (provider.generate as any).mockResolvedValue({
        content: JSON.stringify({
          intent: 'test', entities: [], constraints: [], objectives: [],
          risks: [], assumptions: [], confidence: 1.5, needsClarification: false,
        }),
        usage: { promptTokens: 10, completionTokens: 5, totalTokens: 15 },
        latencyMs: 50, model: 'mock', provider: 'mock',
      });
      const a = new CommandAnalyzer({ llmProvider: provider, model: 'mock', auditTrail });
  ...
  describe('generateClarificationQuestions()', () => {
    it('should return clarification questions from LLM', async () => {
      const questionsResponse = JSON.stringify({
        questions: [
          { questionId: 'q-1', text: 'Which payment gateway?', type: 'single_choice', options: ['Stripe', 'PayPal'], context: 'Need to know target' },
          { questionId: 'q-2', text: 'What is the timeline?', type: 'free_text' },
        ],
      });
      ...
      const questions = await analyzer.generateClarificationQuestions(command, analysis);
    ...
    it('should record audit entry for clarification questions', async () => {
      ...
      const entries = await auditTrail.query({ operationType: 'clarification_question' });
      expect(entries).toHaveLength(1);
      expect(entries[0].entityId).toBe('cmd-1');
    });
    ...
    it('should return empty array when LLM returns invalid response', async () => {
      ...
      const questions = await analyzer.generateClarificationQuestions(makeCommand(), makeAnalysis());
  ...
  describe('updateAnalysis()', () => {
    it('should return updated analysis with higher confidence', async () => {
      const updatedResponse = JSON.stringify({
        ...
        entities: [{ name: 'payment-module', type: 'module' }, { name: 'Stripe', type: 'service' }],
        constraints: [{ type: 'quality', description: 'zero downtime' }],
      ...
      const updated = await analyzer.updateAnalysis(makeCommand(), makeAnalysis(), answer);
    ...
    it('should record audit entry for clarification answer', async () => {
      ...
      const entries = await auditTrail.query({ operationType: 'clarification_answer' });
      expect(entries).toHaveLength(1);
  ...
  describe('finalize()', () => {
    it('should produce a FinalizedCommand with refined text', async () => {
      const finalizeResponse = JSON.stringify({
        refinedText: 'Refactor the payment module to use Stripe with zero downtime deployment',
        clarificationSummary: 'Confirmed Stripe as payment gateway, zero downtime required',
      });
      (mockProvider.generate as any).mockResolvedValueOnce({
      ...
      const command = makeCommand();
      const analysis = makeAnalysis({ confidence: 0.95, needsClarification: false });
    ...
    it('should record audit entry for finalization', async () => {
      ...
      const entries = await auditTrail.query({ operationType: 'command_finalized' });
      expect(entries).toHaveLength(1);
      expect(entries[0].entityId).toBe('cmd-1');
    });
  });
  ...
  describe('retry logic', () => {
      ...
      const retryProvider: ILLMProvider = {
        ...
        generate: vi.fn(async () => {
          ...
          return {
            content: JSON.stringify({
              intent: 'test', entities: [], constraints: [], objectives: [],
              risks: [], assumptions: [], confidence: 0.8, needsClarification: false,
            }),
            usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
            latencyMs: 50, model: 'mock', provider: 'mock',
          };
    ...
    it('should throw after max retries exhausted', async () => {
      const failProvider: ILLMProvider = {
        name: 'fail-mock',
        generate: vi.fn(async () => { throw new Error('always fails'); }),
        streamGenerate: async function* () { yield ''; },
      ...
      await expect(failAnalyzer.analyze(makeCommand())).rejects.toThrow('always fails');
    ...
    it('should not retry on non-temporary errors', async () => {
      const noRetryProvider: ILLMProvider = {
        name: 'no-retry-mock',
        generate: vi.fn(async () => { throw new Error('auth error'); }),
        streamGenerate: async function* () { yield ''; },
        healthCheck: async () => ({ healthy: false, provider: 'mock' }),
        isTemporaryError: () => false,
      };
      ...
      await expect(noRetryAnalyzer.analyze(makeCommand())).rejects.toThrow('auth error');
