...
const TEST_AUDIT_PATH = resolve(
  __test_dirname,
  "../../../data/__test_clarification_prop__/nl-audit.json"
);

// --- Generators ---

const questionTypeArb: fc.Arbitrary<
    ...
    generate: vi.fn(
      async (
        ...
        if (content.includes("Produce a refined command")) {
          return {
            content: JSON.stringify({
              refinedText: "Refined: " + content.slice(0, 30),
              clarificationSummary: "Clarification completed",
            }),
            usage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 },
            latencyMs: 50,
            model: "mock",
...
function makeCommand(commandId: string): StrategicCommand {
  return {
    commandId,
    commandText: "Refactor the payment module with zero downtime",
    userId: "user-1",
    timestamp: Date.now(),
    status: "clarifying",
    constraints: [],
...
function makeAnalysis(confidence = 0.5): CommandAnalysis {
  return {
    ...
    confidence,
    needsClarification: true,
    clarificationTopics: ["deployment strategy"],
  };
}
...
describe("Property 4: clarification dialog accepts both answer types", () => {
  ...
  it("SHALL store both free-text and selection-based answers in the same dialog", () => {
    fc.assert(
      fc.asyncProperty(
        commandIdArb,
        questionsArb.filter(qs => qs.length >= 2),
        async (commandId, questions) => {
          const dialog = await manager.createDialog(commandId, questions);
          await manager.addAnswer(dialog.dialogId, {
          ...
          const freeStored = dialog.answers.find(
            a => a.questionId === questions[0].questionId
          );
          const selStored = dialog.answers.find(
            a => a.questionId === questions[1].questionId
          );
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (commandId, questions) => {
          ...
          const updatedAnalysis = await analyzer.updateAnalysis(
            command,
            initialAnalysis,
            answer
          );
          const changed =
            updatedAnalysis.confidence !== initialAnalysis.confidence ||
      ...
      fc.asyncProperty(
        ...
        async (commandId, questions) => {
          ...
          for (const q of questions) {
            const answer: ClarificationAnswer = {
              questionId: q.questionId,
              text: "Answer for " + q.questionId,
              selectedOptions: q.options ? [q.options[0]] : undefined,
              timestamp: Date.now(),
            };
            await freshManager.addAnswer(dialog.dialogId, answer);
            currentAnalysis = await analyzer.updateAnalysis(
              command,
              currentAnalysis,
              answer
            );
