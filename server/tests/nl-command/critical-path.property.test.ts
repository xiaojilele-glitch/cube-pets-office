...
const dagArb = (
  minNodes: number,
  maxNodes: number,
): fc.Arbitrary<{ nodeIds: string[]; edges: Edge[]; durations: Map<string, number> }> =>
  fc
    .record({
...
function scheduleEntities(
  ...
  for (let groupIdx = 0; groupIdx < groups.length; groupIdx++) {
    for (const entityId of groups[groupIdx]) {
      ...
      const startTime = depEndTimes.length > 0 ? Math.max(...depEndTimes) : baseTime;
    ...
    fc.assert(
      fc.property(dagArb(1, 8), ({ nodeIds, edges, durations }) => {
        ...
        if (cpEntries.length > 0) {
          const cpDurationSum = cpEntries.reduce((sum, e) => sum + e.duration, 0);
  ...
  it('linear chain critical path includes all nodes', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.integer({ min: 1, max: 60 }), { minLength: 6, maxLength: 6 }),
        (chainLen, durPool) => {
          ...
          const durations = new Map<string, number>();
          nodeIds.forEach((id, i) => durations.set(id, durPool[i % durPool.length]));
          ...
          const totalDur = nodeIds.reduce((s, id) => s + (durations.get(id) ?? 0), 0);
