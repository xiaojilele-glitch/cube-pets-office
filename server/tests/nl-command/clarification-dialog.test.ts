...
const TEST_AUDIT_PATH = resolve(__test_dirname, '../../../data/__test_clarification__/nl-audit.json');

function makeQuestions(): ClarificationQuestion[] {
  return [
    { questionId: 'q-1', text: 'What is the target module?', type: 'free_text' },
    {
      questionId: 'q-2',
...
function makeFreeTextAnswer(questionId: string, text: string): ClarificationAnswer {
...
describe('ClarificationDialogManager', () => {
  ...
  describe('addAnswer()', () => {
    ...
    it('should mark dialog as completed when all questions are answered', async () => {
      ...
      await manager.addAnswer(dialog.dialogId, makeFreeTextAnswer('q-1', 'payment module'));
      expect(dialog.status).toBe('active');

      await manager.addAnswer(dialog.dialogId, makeSelectionAnswer('q-2', 'canary', ['canary']));
    ...
    it('should throw when dialog does not exist', async () => {
      ...
      await expect(manager.addAnswer('nonexistent', answer)).rejects.toThrow('Dialog not found');
    });

    it('should record an audit entry for each answer', async () => {
      const dialog = await manager.createDialog('cmd-1', makeQuestions());
      await manager.addAnswer(dialog.dialogId, makeFreeTextAnswer('q-1', 'payment'));
      await manager.addAnswer(dialog.dialogId, makeSelectionAnswer('q-2', 'canary', ['canary']));
  ...
  describe('isComplete()', () => {
    ...
    it('should return false when only some questions are answered', async () => {
      const dialog = await manager.createDialog('cmd-1', makeQuestions());
      await manager.addAnswer(dialog.dialogId, makeFreeTextAnswer('q-1', 'test'));
