...
const capabilityTag = fc.constantFrom(
  "vision",
  "tts",
  "stt",
  "code",
...
const nonEmptyCapabilities = fc.array(capabilityTag, {
  minLength: 1,
  maxLength: 4,
});

/** Role that is NOT "ceo" (so it passes the filter) */
const nonCeoRole = fc.constantFrom(
  "worker",
  "manager",
  "specialist",
  "analyst"
...
const templateWithCapabilities: fc.Arbitrary<RoleTemplate> = fc
  ...
  .map(
    ([id, title, responsibility, role, name, capabilities]: [
      string,
      string,
      string,
      string,
...
const ceoTemplate: fc.Arbitrary<RoleTemplate> = fc
  ...
  .map(
    ([id, title, responsibility, capabilities]: [
      string,
      string,
      string,
      string[],
    ...
    fc.assert(
      fc.property(mixedTemplates, templates => {
        ...
        const nonCeoWithoutCaps = templates.filter(
          t =>
            t.role !== "ceo" && (!t.capabilities || t.capabilities.length === 0)
        );
