...
const TEST_STORE_DIR = resolve(__test_dirname, '../../data/__test_constraint_unit__');
...
describe('COOLDOWN_ACTIVE', () => {
  it('should return COOLDOWN_ACTIVE when within cooldown period', () => {
    ...
    const result = validator.validate(
      ...
      'target-role',
      makeContext({ lastRoleSwitchAt: lastSwitch, roleSwitchCooldownMs: 60_000 }),
    );
  ...
  it('should pass when cooldown is exactly expired (boundary: elapsed === cooldownMs)', () => {
    ...
    const result = validator.validate(
      makeAgent(),
      'target-role',
      makeContext({ lastRoleSwitchAt: lastSwitch, roleSwitchCooldownMs: cooldownMs }),
    );
...
describe('ROLE_SWITCH_DENIED', () => {
  ...
  it('should pass when target IS in compatibleRoles whitelist', () => {
    ...
    reg.register(makeTemplate({ roleId: 'reviewer', authorityLevel: 'medium' }));
