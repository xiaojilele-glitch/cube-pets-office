...
const arbUniqueSkillIds = (min: number, max: number) =>
  fc.uniqueArray(arbSkillId, { minLength: min, maxLength: max, comparator: (a, b) => a === b });
    ...
    fc.assert(
      fc.property(
        ...
        (id) => {
          ...
          expect(() => registry.resolveSkills([id])).toThrow(CircularDependencyError);
        },
      ),
      { numRuns: 100 },
    );
    ...
    fc.assert(
      fc.property(
        ...
        (ids) => {
          ...
          expect(() => registry.resolveSkills([ids[0]])).toThrow(CircularDependencyError);
        },
      ),
      { numRuns: 100 },
    );
      ...
      fc.property(
        ...
        (ids) => {
          ...
          try {
            ...
            expect(cde.cyclePath[cde.cyclePath.length - 1]).toBe(cde.cyclePath[0]);
          }
        },
      ),
      { numRuns: 100 },
    ...
    fc.assert(
      fc.property(
        ...
        (ids, disableRatio) => {
          ...
          const disableCount = Math.max(1, Math.floor(ids.length * disableRatio));
    ...
    fc.assert(
      fc.property(
        ...
        (ids) => {
          ...
          registry.disableSkill(targetId, '1.0.0', 'test', 'immediate effect test');
