...
const safeSignalArb = (agentId: string): fc.Arbitrary<ReputationSignal> =>
  fc.record({
    ...
    roleId: fc.option(fc.constantFrom("coder", "reviewer", "lead"), {
      nil: undefined,
    }),
...
describe("Property 5: 信誉变更事件完整性", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (seed, signalTemplate) => {
          ...
          expect(typeof latest.dimensionDeltas.collaborationDelta).toBe(
            "number"
          );
  ...
  it("adjustReputation generates a ReputationChangeEvent with reason admin_adjust", () => {
    fc.assert(
      fc.property(dimensionKeyArb, adjustDeltaArb, (dimension, delta) => {
        ...
        expect(latest.oldOverallScore).toBe(oldOverall);
        expect(latest.newOverallScore).toBe(
          service.getReputation(agentId)!.overallScore
        );
  ...
  it("inactivity decay generates a ReputationChangeEvent with reason inactivity_decay", () => {
    fc.assert(
      fc.property(fc.integer({ min: 500, max: 800 }), initialScore => {
        ...
        const decayEvent = eventsAfter.find(
          e => e.reason === "inactivity_decay" && e.agentId === agentId
        );
