...
function makeRecord(overrides: Partial<CostRecord> = {}): CostRecord {
  ...
  return {
    ...
    unitPriceOut: 0.0006,
    actualCost:
      overrides.actualCost ?? estimateCost(model, tokensIn, tokensOut),
...
describe("CostTracker", () => {
  ...
  describe("getSnapshot", () => {
    ...
    it("aggregates totals from multiple records", () => {
      tracker.recordCall(
        makeRecord({ tokensIn: 100, tokensOut: 50, actualCost: 0.01 })
      );
      tracker.recordCall(
        makeRecord({ tokensIn: 200, tokensOut: 80, actualCost: 0.02 })
      );
  ...
  describe("finalizeMission", () => {
    ...
    it("includes top 5 agents in the summary", () => {
      for (let i = 0; i < 7; i++) {
        tracker.recordCall(
          makeRecord({ agentId: `agent-${i}`, actualCost: 0.01 * (i + 1) })
        );
      }
  ...
  describe("downgrade logic (Task 4.1)", () => {
    ...
    it("triggers soft downgrade when token_warning alert is generated", () => {
      ...
      tracker.recordCall(
        makeRecord({ tokensIn: 500, tokensOut: 400, actualCost: 0.001 })
      );
    ...
    it("pauses non-critical agents during hard downgrade", () => {
      ...
      tracker.recordCall(
        makeRecord({ agentId: "critical-1", actualCost: 0.1 })
      );
  ...
  describe("budget percentages (Task 3.1)", () => {
    ...
    it("computes tokenUsedPercent correctly", () => {
      ...
      tracker.recordCall(
        makeRecord({ tokensIn: 300, tokensOut: 200, actualCost: 0.001 })
      );
    ...
    it("caps tokenUsedPercent at 1.0", () => {
      ...
      tracker.recordCall(
        makeRecord({ tokensIn: 80, tokensOut: 80, actualCost: 0.001 })
      );
    ...
    it("returns 0 for tokenUsedPercent when maxTokens is 0", () => {
      ...
      tracker.recordCall(
        makeRecord({ tokensIn: 500, tokensOut: 500, actualCost: 0.001 })
      );
  ...
  describe("checkAlerts (Task 3.1)", () => {
    ...
    it("does not duplicate alerts of the same type", () => {
      ...
      const costWarnings = tracker
        .getAlerts()
        .filter(a => a.type === "cost_warning");
    ...
    it("does not generate alerts when below thresholds", () => {
      ...
      tracker.recordCall(
        makeRecord({ tokensIn: 10, tokensOut: 5, actualCost: 0.001 })
      );
    ...
    it("each alert has a unique id and timestamp", () => {
      ...
      tracker.recordCall(
        makeRecord({ tokensIn: 80, tokensOut: 80, actualCost: 0.02 })
      );
...
describe("CostTracker persistence (Task 6.1)", () => {
  ...
  it("loads history from existing file", () => {
    ...
    tracker1.recordCall(
      makeRecord({ tokensIn: 200, tokensOut: 100, actualCost: 0.02 })
    );
  ...
  it("restores downgradePolicy from file", () => {
    const tracker1 = createTracker();
    tracker1.setDowngradePolicy({
      enabled: false,
      lowCostModel: "gpt-4o-mini",
      criticalAgentIds: ["c1", "c2"],
    });
  ...
  it("starts with empty history when file is corrupted", () => {
    ...
    expect(tracker.getHistory()).toHaveLength(0);
    expect(tracker.getBudget()).toEqual(
      expect.objectContaining({ maxCost: 1.0 })
    );
    expect(warnSpy).toHaveBeenCalledWith(
