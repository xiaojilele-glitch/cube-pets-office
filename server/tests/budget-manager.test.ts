...
describe("BudgetManager", () => {
  ...
  describe("updateBudget", () => {
    ...
    it("should throw when budget does not exist", () => {
      expect(() => mgr.updateBudget("nonexistent", { name: "x" })).toThrow(
        "预算不存在"
      );
    });
    ...
    it("should fallback invalid totalBudget on update", () => {
      ...
      expect(r.budget.totalBudget).toBe(
        DEFAULT_BUDGET_TEMPLATES[0].defaultBudget
      );
    });
  });
  ...
  describe("validateHierarchy", () => {
    it("should return true when child fits within parent budget", () => {
      const parent = mgr.createBudget(
        makeBudgetInput({ totalBudget: 100, level: "DEPARTMENT" })
      );
  ...
  describe("reconcile", () => {
    it("should return budget, actual cost, and variance", () => {
      const b = mgr.createBudget(
        makeBudgetInput({ totalBudget: 100, usedBudget: 30 })
      );
