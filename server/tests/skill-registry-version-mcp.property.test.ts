...
const arbMcpId = fc.string({ minLength: 1, maxLength: 16 }).map(
  s =>
    s
      .replace(/[^a-z0-9-]/gi, "a")
      .toLowerCase()
...
function registerMockMcp(mcpId: string) {
  mockMcpLibrary.set(mcpId, {
    ...
    connection: {
      transport: "stdio",
      endpoint: `http://localhost/{agentId}/{workflowId}`,
    },
    tools: [`tool-${mcpId}`],
  });
}
    ...
    fc.assert(
      fc.property(arbId, arbUniqueMcpIds(1, 6), (skillId, mcpIds) => {
        ...
        const skill = registry.registerSkill(
          makeSkillDef(skillId, { requiredMcp: mcpIds })
        );
  ...
  it("resolveMcpForSkill returns empty array when requiredMcp is empty", () => {
    fc.assert(
      fc.property(arbId, skillId => {
        ...
        mockMcpLibrary = new Map();
        const skill = registry.registerSkill(
          makeSkillDef(skillId, { requiredMcp: [] })
        );
        const bindings = registry.resolveMcpForSkill(skill, "agent-1", "wf-1");
    ...
    fc.assert(
      fc.property(
        ...
        (skillId, validCandidates, invalidCandidates) => {
          ...
          const skill = registry.registerSkill(
            makeSkillDef(skillId, { requiredMcp: allMcpIds })
          );
          const bindings = registry.resolveMcpForSkill(
            skill,
            "agent-1",
            "wf-1"
          );
          ...
          const bindingIds = new Set(bindings.map(b => b.id));
          for (const mcpId of validCandidates)
            expect(bindingIds.has(mcpId)).toBe(true);
          for (const mcpId of invalidIds)
            expect(bindingIds.has(mcpId)).toBe(false);
        }
      ),
      { numRuns: 100 }
    );
  ...
  it("sequential enable/disable operations each produce their own audit log entry", () => {
    fc.assert(
      fc.property(
        arbId,
        arbSemver,
        fc.array(fc.constantFrom("enable" as const, "disable" as const), {
          minLength: 1,
          maxLength: 6,
        }),
        arbOperator,
      ...
      fc.property(
        ...
        (skillIds, versions) => {
          ...
          for (let i = 0; i < skillIds.length; i++)
            versionMap[skillIds[i]] = versions[i % versions.length];
          ...
          for (const binding of bindings) {
            ...
            expect(binding.resolvedSkill.version).toBe(
              versionMap[binding.skillId]
            );
          }
        }
      ),
