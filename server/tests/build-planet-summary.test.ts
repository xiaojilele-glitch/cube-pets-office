...
describe('buildPlanetSummaryRecord', () => {
  ...
  it('clamps progress to 0-100 range', () => {
    expect(buildPlanetSummaryRecord(makePlanet({ progress: 150 })).progress).toBe(100);
    expect(buildPlanetSummaryRecord(makePlanet({ progress: -10 })).progress).toBe(0);
  });
  ...
  it('sets hasWarnings when mission has error events', () => {
    ...
    const mission = makeMission({
      events: [
        { type: 'log', message: 'Something went wrong', level: 'warn', time: 1500 },
      ],
    });
    const summary = buildPlanetSummaryRecord(planet, mission);
