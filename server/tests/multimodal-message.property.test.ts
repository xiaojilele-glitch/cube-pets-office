...
import type { LLMMessage, LLMMessageContentPart } from '../../shared/workflow-runtime.js';
...
const arbImageUrlPart: fc.Arbitrary<LLMMessageContentPart> = fc.record({
  ...
  image_url: fc.record({
    url: fc.string(),
    detail: fc.option(
      fc.constantFrom('low', 'high', 'auto') as fc.Arbitrary<'low' | 'high' | 'auto'>,
      { nil: undefined },
    ),
  }),
});

const arbContentPart: fc.Arbitrary<LLMMessageContentPart> = fc.oneof(arbTextPart, arbImageUrlPart);
 ...
 */
function convertToChatCompletionsFormat(parts: LLMMessageContentPart[]): LLMMessageContentPart[] {
  return parts;
}
