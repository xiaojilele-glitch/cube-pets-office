...
import type { ReputationProfile, ReputationGrade } from '../../shared/reputation.js';
...
describe('TrustTierEvaluator.evaluateGradeChange', () => {
  it('generates REPUTATION_DOWNGRADE event on grade drop', () => {
    const events = evaluator.evaluateGradeChange('A', 'B', 'agent-1', 'task-42');
  ...
  it('generates both DOWNGRADE and CRITICAL events when dropping to D', () => {
    const events = evaluator.evaluateGradeChange('B', 'D', 'agent-2', 'task-99');
  ...
  it('includes taskId in event detail', () => {
    const events = evaluator.evaluateGradeChange('A', 'C', 'agent-1', 'task-xyz');
