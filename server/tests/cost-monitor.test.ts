...
import type { AutonomyConfig, CompetitionSession } from '../../shared/autonomy-types.js';
...
function makeSession(overrides?: Partial<CompetitionSession>): CompetitionSession {
  return {
    id: 'comp-1',
    taskId: 'task-1',
    contestants: [
      { agentId: 'a1', isExternal: false, result: 'code-a', tokenConsumed: 100, timedOut: false },
      { agentId: 'a2', isExternal: false, result: 'code-b', tokenConsumed: 200, timedOut: false },
    ],
    status: 'completed',
    deadline: 90_000,
    budgetApproved: true,
    ...
    judgingResult: {
      scores: [
        { agentId: 'a1', correctness: 0.9, quality: 0.8, efficiency: 0.7, novelty: 0.6, totalWeighted: 0.79 },
        { agentId: 'a2', correctness: 0.7, quality: 0.6, efficiency: 0.5, novelty: 0.4, totalWeighted: 0.58 },
      ],
      ranking: ['a1', 'a2'],
  ...
  describe('checkCompetitionBudget', () => {
    ...
    it('respects custom budgetRatio', () => {
      const customConfig = makeConfig({
        competition: { defaultContestantCount: 3, maxDeadlineMs: 300_000, budgetRatio: 0.5 },
      });
      const customMonitor = new CostMonitor(customConfig);
      // limit = 1000 * 0.5 = 500
      expect(customMonitor.checkCompetitionBudget(400, 1000).approved).toBe(true);
      expect(customMonitor.checkCompetitionBudget(600, 1000).approved).toBe(false);
    });
  });
  ...
  describe('recordCompetitionCost', () => {
    ...
    it('logs warning when ROI < 1.0', () => {
      ...
      const session = makeSession({
        judgingResult: {
          scores: [
            { agentId: 'a1', correctness: 0.2, quality: 0.1, efficiency: 0.1, novelty: 0.1, totalWeighted: 0.13 },
          ],
          ranking: ['a1'],
