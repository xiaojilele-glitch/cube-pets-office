...
describe("ApiChecker", () => {
  describe("endpoint path matching", () => {
    it("allows matching endpoint pattern", () => {
      ...
      expect(
        checker.checkConstraints("call", "GET /api/v1/users/123", constraints)
      ).toBe(true);
    });
    ...
    it("supports ** for deep path matching", () => {
      ...
      expect(
        checker.checkConstraints("call", "/api/v1/users/123/posts", constraints)
      ).toBe(true);
    });
  ...
  describe("HTTP method validation", () => {
    it("allows matching method", () => {
      ...
      expect(
        checker.checkConstraints("call", "GET /api/data", constraints)
      ).toBe(true);
      expect(
        checker.checkConstraints("call", "POST /api/data", constraints)
      ).toBe(true);
    });
    ...
    it("denies non-matching method", () => {
      ...
      expect(
        checker.checkConstraints("call", "DELETE /api/data", constraints)
      ).toBe(false);
    });
    ...
    it("skips method check when resource has no method prefix", () => {
      ...
      expect(checker.checkConstraints("call", "/api/data", constraints)).toBe(
        true
      );
    });
  });
  ...
  describe("parameter constraints", () => {
    it("validates query parameters against regex", () => {
      ...
      expect(
        checker.checkConstraints("call", "/api/users?userId=123", constraints)
      ).toBe(true);
      expect(
        checker.checkConstraints("call", "/api/users?userId=abc", constraints)
      ).toBe(false);
    });
    ...
    it("passes when constrained param is not present in query", () => {
      ...
      expect(checker.checkConstraints("call", "/api/users", constraints)).toBe(
        true
      );
    });
    ...
    it("denies on invalid regex in constraint", () => {
      ...
      expect(checker.checkConstraints("call", "/api?x=1", constraints)).toBe(
        false
      );
    });
  });
  ...
  describe("combined constraints", () => {
    it("checks method + endpoint + params together", () => {
      ...
      expect(
        checker.checkConstraints(
          "call",
          "POST /api/v1/users/123?limit=50",
          constraints
        )
      ).toBe(false);
      expect(
      ...
      expect(
        checker.checkConstraints(
          "call",
          "GET /api/v1/users/123?limit=99999",
          constraints
        )
      ).toBe(false);
    });
