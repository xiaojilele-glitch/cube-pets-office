...
} from "../core/governance/alert-manager";
...
describe("Property 6: 审计链完整性", () => {
  ...
  const arbAuditAction: fc.Arbitrary<AuditAction> = fc.constantFrom(
    ...ALL_AUDIT_ACTIONS
  );
        ...
        operations => {
          ...
          for (const entry of recorded) {
            ...
            const original = operations.find(
              (_, idx) => recorded[idx].id === entry.id
            )!;
...
describe("Property 22: 预算层级约束", () => {
  ...
  const arbBudgetLevel = fc.constantFrom<BudgetLevel>(
    "ORGANIZATION",
    "DEPARTMENT",
    "PROJECT"
  );
  const arbBudget = fc.double({
    min: 1,
    max: 1_000_000,
    noNaN: true,
    noDefaultInfinity: true,
    ...
    fc.assert(
      fc.property(
        ...
        (parentLevel, parentBudget, childFractions) => {
          ...
          const parent = mgr.createBudget({
            level: parentLevel,
            name: `parent-${parentLevel}`,
            totalBudget: parentBudget,
            usedBudget: 0,
            currency: "USD",
          });
    ...
    fc.assert(
      fc.property(
        ...
        (parentBudget, firstChildFraction) => {
          ...
          if (remaining > 0.001) {
            expect(mgr.validateHierarchy(remaining * 0.5, parent.id)).toBe(
              true
            );
          }
        }
...
describe("Property 23: 预算模板实例化", () => {
  const arbBudgetPeriod: fc.Arbitrary<BudgetPeriod> = fc.constantFrom(
    "MISSION",
    "DAILY",
    "HOURLY"
  );
  const arbAlertThreshold: fc.Arbitrary<AlertThresholdConfig> = fc.record({
    ...
    responseStrategy: fc.constantFrom(
      "LOG",
      "REDUCE_CONCURRENCY",
      "DOWNGRADE_MODEL",
      "PAUSE_TASK"
  ...
  const arbBudgetTemplate: fc.Arbitrary<BudgetTemplate> = fc.record({
    ...
    defaultAlertThresholds: fc.array(arbAlertThreshold, {
      minLength: 1,
      maxLength: 6,
    }),
  });
...
describe("Property 24: 预算修改审批阈值", () => {
  ...
  it("version number should increment on each successive modification", () => {
    fc.assert(
      fc.property(
        arbBudget,
        fc.array(arbNewBudget, { minLength: 1, maxLength: 10 }),
        (originalBudget, updates) => {
          const mgr = new BudgetManager();
          const budget = mgr.createBudget({
            level: "DEPARTMENT",
...
describe("Property 5: alert type and response strategy mapping", () => {
  ...
  const arbResponseStrategy: fc.Arbitrary<AlertResponseStrategy> =
    fc.constantFrom(
      "LOG",
      "REDUCE_CONCURRENCY",
      "DOWNGRADE_MODEL",
  ...
  it("when cost reaches a threshold, alert action matches the threshold responseStrategy", () => {
    fc.assert(
      fc.property(
        arbMissionBudget(
          fc.array(arbThresholdConfig, { minLength: 1, maxLength: 6 })
        ),
        budget => {
          const mgr = new AlertManager();
          mgr._reset();
          const sorted = [...budget.alertThresholds].sort(
  ...
  it("custom thresholds override default thresholds", () => {
    fc.assert(
      fc.property(
        arbMissionBudget(
          fc
            .array(arbThresholdConfig, { minLength: 1, maxLength: 4 })
            .filter(arr => {
        ...
        budget => {
          ...
          for (const alert of alerts) {
            const customTh = budget.alertThresholds.find(
              t => t.percent === alert.threshold
            );
            expect(customTh).toBeDefined();
            expect(alert.action).toBe(customTh!.responseStrategy);
          }
          const customPercents = new Set(
            budget.alertThresholds.map(t => t.percent)
          );
          for (const alert of alerts) {
...
describe("Property 7: 降级链正确性", () => {
  ...
  const arbTerminalModel: fc.Arbitrary<string> = fc.constantFrom(
    ...terminalModels
  );
  ...
  it("downgrading a model in the chain should return the next model in the chain", () => {
    fc.assert(
      fc.property(arbChainModel, arbMissionId, (sourceModel, missionId) => {
        ...
        const effective = mgr.getEffectiveModel(
          missionId,
          sourceModel,
          "any-agent"
        );
    ...
    fc.assert(
      fc.property(
        ...
        (missionId, sourceModel, grayPercent, agentIds) => {
          const mgr = new ModelDowngradeManager();
          const record = mgr.applyDowngrade(
            missionId,
            sourceModel,
            grayPercent
          );
...
describe("Property 9: 降级失败自动回滚", () => {
  ...
  const arbRollbackReason: fc.Arbitrary<string> = fc.string({
    minLength: 1,
    maxLength: 100,
  });
    ...
    fc.assert(
      fc.property(
        ...
        (missionId, sourceModel, agentId, reason) => {
          ...
          const effectiveBefore = mgr.getEffectiveModel(
            missionId,
            sourceModel,
            agentId
          );
          ...
          const effectiveAfter = mgr.getEffectiveModel(
            missionId,
            sourceModel,
            agentId
          );
