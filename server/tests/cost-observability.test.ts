...
describe('Property 2: estimateCost 纯函数正确性', () => {
  it('should compute cost as (tokensIn/1000)*pricing.input + (tokensOut/1000)*pricing.output for any model and non-negative tokens', () => {
    fc.assert(
      fc.property(arbModel, arbTokens, arbTokens, (model, tokensIn, tokensOut) => {
        const pricing = Object.hasOwn(PRICING_TABLE, model) ? PRICING_TABLE[model] : DEFAULT_PRICING;
        const expected = (tokensIn / 1000) * pricing.input + (tokensOut / 1000) * pricing.output;
  ...
  it('should use DEFAULT_PRICING for unknown models', () => {
    fc.assert(
      fc.property(
        fc.string({ minLength: 1, maxLength: 30 }).filter((s) => !Object.hasOwn(PRICING_TABLE, s) && !protoKeys.has(s)),
        arbTokens,
        arbTokens,
...
const arbAgentCostSummary: fc.Arbitrary<AgentCostSummary> = fc.record({
  ...
  totalCost: fc.double({ min: 0, max: 1000, noNaN: true, noDefaultInfinity: true }),
...
const arbCostAlert: fc.Arbitrary<CostAlert> = fc.record({
  ...
  type: fc.constantFrom('cost_warning', 'cost_exceeded', 'token_warning', 'token_exceeded') as fc.Arbitrary<CostAlert['type']>,
...
const arbBudget: fc.Arbitrary<Budget> = fc.record({
  maxCost: fc.double({ min: 0.01, max: 10_000, noNaN: true, noDefaultInfinity: true }),
  maxTokens: fc.nat({ max: 10_000_000 }),
  warningThreshold: fc.double({ min: 0, max: 1, noNaN: true, noDefaultInfinity: true }),
});

/** Arbitrary for DowngradeLevel */
const arbDowngradeLevel: fc.Arbitrary<DowngradeLevel> = fc.constantFrom('none', 'soft', 'hard');
...
const arbCostSnapshot: fc.Arbitrary<CostSnapshot> = fc.record({
  ...
  budgetUsedPercent: fc.double({ min: 0, max: 1, noNaN: true, noDefaultInfinity: true }),
  tokenUsedPercent: fc.double({ min: 0, max: 1, noNaN: true, noDefaultInfinity: true }),
...
function freshTracker(): InstanceType<typeof CostTracker> {
  ...
  return new CostTracker('/tmp/cost-test-' + Math.random().toString(36).slice(2) + '.json');
}
...
const arbCostRecord: fc.Arbitrary<CostRecord> = fc
  ...
  .map((r) => {
    const pricing = Object.hasOwn(PRICING_TABLE, r.model) ? PRICING_TABLE[r.model] : DEFAULT_PRICING;
    return {
  ...
  it('should preserve all required fields after recordCall for any valid CostRecord', () => {
    fc.assert(
      fc.property(arbCostRecord, (record) => {
        ...
        expect(r.actualCost).toBeCloseTo(estimateCost(r.model, r.tokensIn, r.tokensOut), 12);
    ...
    fc.assert(
      fc.property(
        ...
        (records) => {
          ...
          tracker.setBudget({ maxCost: 999999, maxTokens: 999999999, warningThreshold: 0.99 });
          ...
          const expectedTokensOut = records.reduce((s, r) => s + r.tokensOut, 0);
    ...
    fc.assert(
      fc.property(
        ...
        (records) => {
          ...
          // Build expected map
          const expectedMap = new Map<string, { tokensIn: number; tokensOut: number; cost: number; count: number }>();
          for (const r of records) {
            const aid = r.agentId ?? 'unknown';
            const entry = expectedMap.get(aid) ?? { tokensIn: 0, tokensOut: 0, cost: 0, count: 0 };
  ...
  it('should generate correct alert types based on cost/token thresholds', () => {
    fc.assert(
      fc.property(
        ...
        fc.double({ min: 0.1, max: 0.95, noNaN: true, noDefaultInfinity: true }),  // warningThreshold
...
describe('Property 6: 预算百分比正确性', () => {
  it('budgetUsedPercent and tokenUsedPercent should match formula capped at 1.0', () => {
    fc.assert(
      fc.property(
        fc.double({ min: 0.01, max: 100, noNaN: true, noDefaultInfinity: true }),   // maxCost
    ...
    fc.assert(
      fc.property(
        ...
        (records) => {
          ...
          tracker.setBudget({ maxCost: 999999, maxTokens: 999999999, warningThreshold: 0.8 });
          ...
          expect(alertsBefore.filter((a) => a.type === 'cost_exceeded')).toHaveLength(0);
          ...
          if (totalCost > 0) {
            ...
            tracker.setBudget({ maxCost: newMaxCost, maxTokens: 999999999, warningThreshold: 0.8 });

            const alertsAfter = tracker.getAlerts();
            const costExceeded = alertsAfter.filter((a) => a.type === 'cost_exceeded');
    ...
    fc.assert(
      fc.property(
        ...
        (originalModel, lowCostModel) => {
          const tracker = freshTracker();
          tracker.setDowngradePolicy({ enabled: true, lowCostModel, criticalAgentIds: [] });
          // Trigger soft downgrade: set a tiny budget and add a record that exceeds warning threshold
          tracker.setBudget({ maxCost: 0.0001, maxTokens: 1, warningThreshold: 0.5 });
          tracker.recordCall({
            id: 'test-soft',
            timestamp: Date.now(),
            model: 'gpt-4o',
    ...
    fc.assert(
      fc.property(
        ...
        (allAgentIds, criticalAgentIds) => {
          ...
          const tracker = freshTracker();
          tracker.setDowngradePolicy({ enabled: true, lowCostModel: 'glm-4.6', criticalAgentIds: uniqueCritical });
          // Trigger hard downgrade: set budget so cost exceeds maxCost
          tracker.setBudget({ maxCost: 0.0000001, maxTokens: 1, warningThreshold: 0.5 });
    ...
    fc.assert(
      fc.property(
        ...
        (originalModel, agentIds) => {
          ...
          const tracker = freshTracker();
          tracker.setDowngradePolicy({ enabled: true, lowCostModel: 'glm-4.6', criticalAgentIds: [] });
          tracker.setBudget({ maxCost: 0.0000001, maxTokens: 1, warningThreshold: 0.5 });
  ...
  it('should produce equivalent data after persist + load round-trip', () => {
    fc.assert(
      fc.property(
        ...
        (budget, missions) => {
          const tmpFile = join(tmpdir(), `cost-test-${Math.random().toString(36).slice(2)}.json`);
          ...
          try {
            ...
            expect(loadedBudget.warningThreshold).toBeCloseTo(budget.warningThreshold, 10);
            ...
            for (let i = 0; i < expectedLen; i++) {
              expect(loadedHistory[i].missionId).toBe(expectedMissions[i].missionId);
