...
import {
  RolePerformanceTracker,
  type TaskResult,
} from "../core/role-performance-tracker.js";
...
describe("RolePerformanceTracker", () => {
  ...
  describe("updateOnTaskComplete", () => {
    it("creates a new record on first task completion", () => {
      tracker.updateOnTaskComplete(
        "agent-1",
        "coder",
        makeTaskResult({ qualityScore: 90, latencyMs: 500, success: true })
      );

      const record = tracker.getPerformance(
        "agent-1",
        "coder"
    ...
    it("recalculates avgQualityScore from recentTasks", () => {
      tracker.updateOnTaskComplete(
        "a",
        "r",
        makeTaskResult({ qualityScore: 60 })
      );
      tracker.updateOnTaskComplete(
        "a",
        "r",
        makeTaskResult({ qualityScore: 80 })
      );
    ...
    it("recalculates avgLatencyMs from recentTasks", () => {
      tracker.updateOnTaskComplete(
        "a",
        "r",
        makeTaskResult({ latencyMs: 200 })
      );
      tracker.updateOnTaskComplete(
        "a",
        "r",
        makeTaskResult({ latencyMs: 400 })
      );
    ...
    it("recalculates averages from the ring buffer window only", () => {
      ...
      for (let i = 0; i < 50; i++) {
        tracker.updateOnTaskComplete(
          "a",
          "r",
          makeTaskResult({ qualityScore: 50, latencyMs: 100 })
        );
      }
      ...
      for (let i = 0; i < 50; i++) {
        tracker.updateOnTaskComplete(
          "a",
          "r",
          makeTaskResult({ qualityScore: 100, latencyMs: 200 })
        );
      }
      const record = tracker.getPerformance("a", "r") as RolePerformanceRecord;
  ...
  describe("qualityScore clamping", () => {
    it("clamps qualityScore above 100 to 100", () => {
      tracker.updateOnTaskComplete(
        "a",
        "r",
        makeTaskResult({ qualityScore: 150 })
      );
      const record = tracker.getPerformance("a", "r") as RolePerformanceRecord;
    ...
    it("clamps qualityScore below 0 to 0", () => {
      tracker.updateOnTaskComplete(
        "a",
        "r",
        makeTaskResult({ qualityScore: -20 })
      );
      const record = tracker.getPerformance("a", "r") as RolePerformanceRecord;
  ...
  describe("getPerformance", () => {
    ...
    it("returns a copy of the map (not the internal reference)", () => {
      tracker.updateOnTaskComplete("a", "r1", makeTaskResult());
      const map1 = tracker.getPerformance("a") as Map<
        string,
        RolePerformanceRecord
      >;
      const map2 = tracker.getPerformance("a") as Map<
        string,
        RolePerformanceRecord
      >;
  ...
  describe("multi-agent isolation", () => {
    it("tracks performance independently per agent", () => {
      tracker.updateOnTaskComplete(
        "agent-1",
        "coder",
        makeTaskResult({ qualityScore: 90 })
      );
      tracker.updateOnTaskComplete(
        "agent-2",
        "coder",
      ...
      const r1 = tracker.getPerformance(
        "agent-1",
        "coder"
      ) as RolePerformanceRecord;
      const r2 = tracker.getPerformance(
        "agent-2",
        "coder"
      ) as RolePerformanceRecord;
    ...
    it("tracks performance independently per role within same agent", () => {
      ...
      tracker.updateOnTaskComplete(
        "a",
        "reviewer",
        makeTaskResult({ qualityScore: 70 })
      );

      const coder = tracker.getPerformance(
        "a",
        "coder"
      ) as RolePerformanceRecord;
      const reviewer = tracker.getPerformance(
        "a",
        "reviewer"
      ) as RolePerformanceRecord;
