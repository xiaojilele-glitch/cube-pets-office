...
const arbWorkPackage: fc.Arbitrary<MissionWorkPackage> = fc.record({
  ...
  assignee: fc.option(fc.string({ minLength: 1, maxLength: 12 }), {
    nil: undefined,
  }),
  stageKey: fc
...
const arbOrganization: fc.Arbitrary<MissionOrganizationSnapshot> = fc.record({
  departments: fc.array(
    fc.record({
      ...
      managerName: fc.option(fc.string({ minLength: 1, maxLength: 12 }), {
        nil: undefined,
      }),
    }),
...
function arbPlanet(): fc.Arbitrary<MissionPlanetOverviewItem> {
  return fc.record({
    ...
    currentStageLabel: fc.option(fc.string({ minLength: 1, maxLength: 20 }), {
      nil: undefined,
    }),
    waitingFor: fc.option(fc.string({ minLength: 1, maxLength: 30 }), {
      nil: undefined,
    }),
...
function arbMission(): fc.Arbitrary<MissionRecord> {
  return fc.record({
    ...
    workPackages: fc.option(
      fc.array(arbWorkPackage, { minLength: 0, maxLength: 6 }),
      { nil: undefined }
    ),
    messageLog: fc.option(
      fc.array(arbMessageLog, { minLength: 0, maxLength: 6 }),
      { nil: undefined }
    ),
    artifacts: fc.option(
      fc.array(
...
describe("Feature: mission-native-projection, Property 1: Mission 原生摘要完整性", () => {
  it("produces a valid MissionTaskSummary with all fields non-undefined", () => {
    fc.assert(
      fc.property(
        arbPlanet(),
        fc.option(arbMission(), { nil: undefined }),
        (planet, mission) => {
          const summary = buildPlanetSummaryRecord(
            planet,
            mission ?? undefined
          );
