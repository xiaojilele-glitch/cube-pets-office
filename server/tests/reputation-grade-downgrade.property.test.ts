...
function isDowngrade(oldGrade: ReputationGrade, newGrade: ReputationGrade): boolean {
...
describe('Property 13: 信誉等级降级事件', () => {
  ...
  it('any downgrade generates at least one REPUTATION_DOWNGRADE event', () => {
    fc.assert(
      fc.property(downgradePairArb, agentIdArb, taskIdArb, ([oldGrade, newGrade], agentId, taskId) => {
        const events = evaluator.evaluateGradeChange(oldGrade, newGrade, agentId, taskId);
        const downgradeEvents = events.filter((e) => e.type === 'REPUTATION_DOWNGRADE');
  ...
  it('downgrade event detail contains agentId, old grade, new grade, and taskId', () => {
    fc.assert(
      fc.property(downgradePairArb, agentIdArb, taskIdArb, ([oldGrade, newGrade], agentId, taskId) => {
        const events = evaluator.evaluateGradeChange(oldGrade, newGrade, agentId, taskId);
        const downgradeEvent = events.find((e) => e.type === 'REPUTATION_DOWNGRADE')!;
  ...
  it('downgrade to D generates both REPUTATION_DOWNGRADE and AGENT_REPUTATION_CRITICAL', () => {
    fc.assert(
      fc.property(downgradeToD_Arb, agentIdArb, taskIdArb, ([oldGrade, newGrade], agentId, taskId) => {
        const events = evaluator.evaluateGradeChange(oldGrade, newGrade, agentId, taskId);
  ...
  it('downgrade to non-D grade generates only REPUTATION_DOWNGRADE (no critical alert)', () => {
    ...
    fc.assert(
      fc.property(downgradeToNonD, agentIdArb, taskIdArb, ([oldGrade, newGrade], agentId, taskId) => {
        const events = evaluator.evaluateGradeChange(oldGrade, newGrade, agentId, taskId);
        const types = events.map((e) => e.type);
        expect(types).toContain('REPUTATION_DOWNGRADE');
  ...
  it('no events generated when grade stays the same or upgrades', () => {
    fc.assert(
      fc.property(nonDowngradePairArb, agentIdArb, taskIdArb, ([oldGrade, newGrade], agentId, taskId) => {
        const events = evaluator.evaluateGradeChange(oldGrade, newGrade, agentId, taskId);
  ...
  it('AGENT_REPUTATION_CRITICAL detail contains agentId and taskId', () => {
    fc.assert(
      fc.property(downgradeToD_Arb, agentIdArb, taskIdArb, ([oldGrade, newGrade], agentId, taskId) => {
        const events = evaluator.evaluateGradeChange(oldGrade, newGrade, agentId, taskId);
        const criticalEvent = events.find((e) => e.type === 'AGENT_REPUTATION_CRITICAL')!;
