...
import { QdrantAdapter, createQdrantAdapter } from '../rag/store/qdrant-adapter.js';
...
describe('QdrantAdapter', () => {
  ...
  describe('createCollection', () => {
    it('creates collection and 4 payload indexes', async () => {
      ...
      expect(indexUrls.every((u) => u === `${BASE_URL}/collections/rag_proj1/index`)).toBe(true);
      ...
      expect(fieldNames).toEqual(['agentId', 'codeLanguage', 'sourceType', 'timestamp']);

      // keyword indexes
      const keywordFields = indexBodies.filter((b: any) => b.field_schema === 'keyword');
      expect(keywordFields).toHaveLength(3);

      // integer index for timestamp
      const intFields = indexBodies.filter((b: any) => b.field_schema === 'integer');
  ...
  describe('upsert', () => {
    ...
    it('handles multiple records in a single batch', async () => {
      ...
      const body = JSON.parse(lastFetchCall(globalThis.fetch as any)[1].body as string);
  ...
  describe('search', () => {
    it('sends search request and returns mapped hits', async () => {
      const mockResult = {
        result: [
          { id: 'c1', score: 0.95, payload: { content: 'hello', sourceType: 'task_result' } },
          { id: 'c2', score: 0.80, payload: { content: 'world', sourceType: 'document' } },
        ],
      };
    ...
    it('builds keyword match filter for string values', async () => {
      ...
      const sourceFilter = body.filter.must.find((m: any) => m.key === 'sourceType');
      expect(sourceFilter.match).toEqual({ value: 'code_snippet' });

      const agentFilter = body.filter.must.find((m: any) => m.key === 'agentId');
    ...
    it('skips null/undefined filter values', async () => {
      ...
      await adapter.search('rag_proj1', [0.1], {
        topK: 5,
        filter: { sourceType: 'task_result', agentId: undefined, codeLanguage: null },
      });
  ...
  describe('timeout', () => {
    it('throws timeout error when request exceeds timeoutMs', async () => {
      ...
      await expect(adapter.upsert('rag_proj1', [
