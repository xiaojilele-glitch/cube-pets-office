...
import type { AutonomyConfig, AssessmentResult } from '../../shared/autonomy-types.js';
...
function makeAssessmentResult(overrides?: Partial<AssessmentResult>): AssessmentResult {
  return {
    agentId: 'agent-1',
    taskId: 'task-1',
    fitnessScore: 0.5,
...
describe('TaskAllocator', () => {
  ...
  describe('broadcastAssessment', () => {
    it('returns results for all candidates', async () => {
      ...
      const results = await allocator.broadcastAssessment(makeTask(), ['a1', 'a2'], 200);
      expect(results).toHaveLength(2);
      expect(results.map((r) => r.agentId).sort()).toEqual(['a1', 'a2']);
    });

    it('returns REJECT for unknown agents (profile missing)', async () => {
      const results = await allocator.broadcastAssessment(makeTask(), ['unknown'], 200);
  ...
  describe('selectBestAgent', () => {
    ...
    it('picks ACCEPT with highest fitnessScore as DIRECT_ASSIGN', () => {
      const results: AssessmentResult[] = [
        makeAssessmentResult({ agentId: 'a1', decision: 'ACCEPT', fitnessScore: 0.85 }),
        makeAssessmentResult({ agentId: 'a2', decision: 'ACCEPT', fitnessScore: 0.9 }),
        makeAssessmentResult({ agentId: 'a3', decision: 'ACCEPT_WITH_CAVEAT', fitnessScore: 0.95 }),
      ];
    ...
    it('picks ACCEPT_WITH_CAVEAT when no ACCEPT exists', () => {
      const results: AssessmentResult[] = [
        makeAssessmentResult({ agentId: 'a1', decision: 'ACCEPT_WITH_CAVEAT', fitnessScore: 0.65 }),
        makeAssessmentResult({ agentId: 'a2', decision: 'ACCEPT_WITH_CAVEAT', fitnessScore: 0.75 }),
        makeAssessmentResult({ agentId: 'a3', decision: 'REQUEST_ASSIST', fitnessScore: 0.55 }),
      ];
      const decision = allocator.selectBestAgent(results, task);
      expect(decision).not.toBeNull();
    ...
    it('picks REQUEST_ASSIST as TASKFORCE when no ACCEPT/CAVEAT', () => {
      const results: AssessmentResult[] = [
        makeAssessmentResult({ agentId: 'a1', decision: 'REQUEST_ASSIST', fitnessScore: 0.45 }),
        makeAssessmentResult({ agentId: 'a2', decision: 'REQUEST_ASSIST', fitnessScore: 0.55 }),
        makeAssessmentResult({ agentId: 'a3', decision: 'REJECT_AND_REFER', fitnessScore: 0.2 }),
      ];
      const decision = allocator.selectBestAgent(results, task);
      expect(decision).not.toBeNull();
    ...
    it('returns null when all REJECT', () => {
      const results: AssessmentResult[] = [
        makeAssessmentResult({ agentId: 'a1', decision: 'REJECT_AND_REFER', fitnessScore: 0.1 }),
        makeAssessmentResult({ agentId: 'a2', decision: 'REJECT_AND_REFER', fitnessScore: 0.2 }),
      ];
      const decision = allocator.selectBestAgent(results, task);
  ...
  describe('forceAssign', () => {
    ...
    it('picks agent with highest referral count', () => {
      const results: AssessmentResult[] = [
        makeAssessmentResult({ agentId: 'a1', decision: 'REJECT_AND_REFER', fitnessScore: 0.1, referralList: ['a3'] }),
        makeAssessmentResult({ agentId: 'a2', decision: 'REJECT_AND_REFER', fitnessScore: 0.2, referralList: ['a3', 'a4'] }),
      ];
    ...
    it('picks highest fitnessScore when no referrals', () => {
      const results: AssessmentResult[] = [
        makeAssessmentResult({ agentId: 'a1', decision: 'REJECT_AND_REFER', fitnessScore: 0.3, referralList: [] }),
        makeAssessmentResult({ agentId: 'a2', decision: 'REJECT_AND_REFER', fitnessScore: 0.1, referralList: [] }),
      ];
      const decision = allocator.forceAssign(results, task);
      expect(decision.strategy).toBe('FORCE_ASSIGN');
