...
describe('TaskforceManager', () => {
  ...
  describe('electLead', () => {
    ...
    it('returns the only candidate when there is one', () => {
      const candidates = [makeAssessmentResult({ agentId: 'solo', fitnessScore: 0.3 })];
  ...
  describe('formTaskforce', () => {
    it('creates a session with recruiting status', async () => {
      ...
      const task = { taskId: 'task-1', requiredSkills: ['coding'], requiredSkillWeights: new Map([['coding', 0.8]]) };
    ...
    it('creates a message bus room', async () => {
      ...
      expect(bus.createRoom).toHaveBeenCalledWith(`taskforce:${session.taskforceId}`);
    });
  ...
  describe('processApplications', () => {
    it('filters out applications with fitnessScore < 0.5', async () => {
      ...
      const apps: TaskforceApplication[] = [
        { agentId: 'good', fitnessScore: 0.7, loadFactor: 0.3, estimatedCompletionTime: 1000 },
        { agentId: 'bad', fitnessScore: 0.3, loadFactor: 0.2, estimatedCompletionTime: 500 },
      ];
    ...
    it('filters out applications with loadFactor >= 0.8', async () => {
      ...
      const apps: TaskforceApplication[] = [
        { agentId: 'overloaded', fitnessScore: 0.9, loadFactor: 0.8, estimatedCompletionTime: 1000 },
        { agentId: 'available', fitnessScore: 0.6, loadFactor: 0.5, estimatedCompletionTime: 800 },
      ];
    ...
    it('sorts accepted members by fitnessScore descending', async () => {
      ...
      const apps: TaskforceApplication[] = [
        { agentId: 'mid', fitnessScore: 0.6, loadFactor: 0.3, estimatedCompletionTime: 1000 },
        { agentId: 'top', fitnessScore: 0.9, loadFactor: 0.2, estimatedCompletionTime: 500 },
        { agentId: 'low', fitnessScore: 0.5, loadFactor: 0.4, estimatedCompletionTime: 1500 },
      ];
    ...
    it('assigns role "worker" to all accepted members', async () => {
      ...
      const apps: TaskforceApplication[] = [
        { agentId: 'w1', fitnessScore: 0.7, loadFactor: 0.3, estimatedCompletionTime: 1000 },
      ];
  ...
  describe('dissolveTaskforce', () => {
    ...
    it('destroys the message bus room', async () => {
      ...
      expect(bus.destroyRoom).toHaveBeenCalledWith(`taskforce:${session.taskforceId}`);
    });
  ...
  describe('getActiveTaskforces', () => {
    ...
    it('returns all non-dissolved sessions', async () => {
      ...
      const task1 = { taskId: 't1', requiredSkills: ['coding'], requiredSkillWeights: new Map([['coding', 0.8]]) };
      const task2 = { taskId: 't2', requiredSkills: ['testing'], requiredSkillWeights: new Map([['testing', 0.7]]) };
