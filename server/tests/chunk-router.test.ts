...
import { SOURCE_TYPES, type ChunkRecord, type ChunkMetadata } from '../../shared/rag/contracts.js';
...
describe('ChunkRouter', () => {
  ...
  it('throws on unknown sourceType', () => {
    expect(() => router.route('unknown_type' as any)).toThrow('Unknown sourceType');
  });

  it('throws when strategy is not registered', () => {
    expect(() => router.route('code_snippet')).toThrow('No Chunker registered for strategy');
  ...
  it('uses config override strategy when RAG_CHUNKING_OVERRIDES is set', () => {
    ...
    resetRAGConfigCache();
    getRAGConfig(
      { RAG_CHUNKING_OVERRIDES: JSON.stringify({ code_snippet: { strategy: 'custom_ast' } }) },
      { noCache: false },
    );
