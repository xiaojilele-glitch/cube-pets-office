...
describe("ReputationService", () => {
  ...
  describe("handleTaskCompleted", () => {
    ...
    it("halts update and creates audit entry on anomaly detection", () => {
      ...
      service.handleTaskCompleted(
        makeSignal({ agentId, taskId: "task-anomaly" })
      );
  ...
  describe("getReputationByRole", () => {
    it("returns undefined for unknown agent", () => {
      expect(
        service.getReputationByRole("nonexistent", "coder")
      ).toBeUndefined();
    });
    ...
    it("returns undefined for unknown role", () => {
      ...
      service.initializeProfile(agentId, false);
      expect(
        service.getReputationByRole(agentId, "unknown-role")
      ).toBeUndefined();
    });
  });
  ...
  describe("adjustReputation", () => {
    ...
    it("clamps dimension to [0, 1000]", () => {
      ...
      expect(service.getReputation(agentId)!.dimensions.qualityScore).toBe(
        1000
      );
  ...
  describe("resetReputation", () => {
    it("resets internal agent to 500", () => {
      ...
      // Modify via task
      service.handleTaskCompleted(
        makeSignal({ agentId, taskQualityScore: 95 })
      );
  ...
  describe("getLeaderboard", () => {
    ...
    it("supports ascending order", () => {
      ...
      if (ourEntries.length >= 2) {
        expect(ourEntries[0].overallScore).toBeLessThanOrEqual(
          ourEntries[1].overallScore
        );
      }
    });
    ...
    it("supports sorting by dimension score", () => {
      ...
      const ourEntries = board.filter(e => ids.includes(e.agentId));
      if (ourEntries.length >= 2) {
        expect(ourEntries[0].dimensions.qualityScore).toBeGreaterThanOrEqual(
          ourEntries[1].dimensions.qualityScore
        );
      }
    });
  });
