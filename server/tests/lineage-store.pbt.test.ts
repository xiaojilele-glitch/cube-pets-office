...
const arbSessionId = fc.option(
  fc.string({ minLength: 1, maxLength: 20 }).map((s) => `sess_${s.replace(/[^a-zA-Z0-9_]/g, "x")}`),
  { nil: undefined },
);

const arbAgentId = fc.option(
  fc.string({ minLength: 1, maxLength: 20 }).map((s) => `agent_${s.replace(/[^a-zA-Z0-9_]/g, "x")}`),
  { nil: undefined },
);

const arbDecisionId = fc.option(
  fc.string({ minLength: 1, maxLength: 20 }).map((s) => `dec_${s.replace(/[^a-zA-Z0-9_]/g, "x")}`),
  { nil: undefined },
);
    ...
    await fc.assert(
      fc.asyncProperty(arbUniqueNodes, async (nodes) => {
        ...
        try {
          ...
          for (const nodeType of ["source", "transformation", "decision"] as const) {
      ...
      fc.asyncProperty(
        ...
        async (nodes, cutoff) => {
          ...
          try {
            ...
            const expectedRemaining = nodes.filter((n) => n.timestamp >= cutoff).length;
            const stats = await store.getStats();
