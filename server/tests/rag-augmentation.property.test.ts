...
import type { RetrievalResult, SourceType } from '../../shared/rag/contracts.js';
...
function cleanup() { if (existsSync(LOG_PATH)) unlinkSync(LOG_PATH); }

/* ---- Arbitraries ---- */

const arbRetrievalResult: fc.Arbitrary<RetrievalResult> = fc.record({
  chunkId: fc.stringMatching(/^[a-z_]+:[a-z0-9-]+:\d+$/),
  score: fc.float({ min: Math.fround(0.1), max: Math.fround(1.0), noNaN: true }),
  content: fc.array(
    fc.string({ minLength: 1, maxLength: 8 }).filter(s => s.trim().length > 0),
    { minLength: 5, maxLength: 50 },
  ).map(w => w.join(' ')),
  sourceType: fc.constantFrom('task_result', 'code_snippet', 'document') as fc.Arbitrary<SourceType>,
  sourceId: fc.stringMatching(/^src-[a-z0-9]{3,6}$/),
  metadata: fc.constant({ ingestedAt: new Date().toISOString(), lastAccessedAt: new Date().toISOString(), contentHash: 'abcdef0123456789' }),
  totalCandidates: fc.integer({ min: 1, max: 100 }),
});

function arbResultList(min: number, max: number) {
  return fc.uniqueArray(arbRetrievalResult, { minLength: min, maxLength: max, selector: r => r.chunkId });
}
    ...
    fc.assert(
      fc.property(
        ...
        (results, budget) => {
          ...
          const validStatuses = new Set(['injected', 'pruned', 'below_threshold']);
          ...
          const injected = allocation.chunks.filter(c => c.status === 'injected').length;
          const pruned = allocation.chunks.filter(c => c.status === 'pruned').length;
          const belowThreshold = allocation.chunks.filter(c => c.status === 'below_threshold').length;
          expect(injected + pruned + belowThreshold).toBe(allocation.chunks.length);
        },
      ),
      { numRuns: 20 },
    );
