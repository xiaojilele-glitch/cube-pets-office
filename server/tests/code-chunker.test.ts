...
describe('CodeChunker', () => {
  ...
  it('extracts functionSignature for function chunks', () => {
    ...
    expect(withSig[0].metadata.functionSignature).toContain('function processData');
  });
  ...
  it('extracts imports metadata', () => {
    ...
    const withImports = result.filter(r => r.metadata.imports && r.metadata.imports.length > 0);
