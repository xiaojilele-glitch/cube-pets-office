      ...
      fc.property(
        ...
        (pairs) => {
          ...
          for (const pair of pairs) {
            ...
            const devAtoB = Math.abs(pair.ratingAtoB - pair.otherMembersAvgRating);
            const devBtoA = Math.abs(pair.ratingBtoA - pair.otherMembersAvgRating);
            const highDeviation =
              devAtoB > config.anomaly.collusionDeviationMin &&
              devBtoA > config.anomaly.collusionDeviationMin;

            if (mutualHigh && highDeviation) {
              expectedSuspicious.push({ agentA: pair.agentA, agentB: pair.agentB });
            }
          }
          ...
          expect(result.weight).toBe(
            expectedSuspicious.length > 0 ? config.anomaly.suspiciousWeight : 1.0,
          );
        },
      ),
      { numRuns: 200 },
  ...
  it('returns weight = suspiciousWeight when collusion is detected', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (pairCount) => {
          const pairs: CollabRatingPair[] = Array.from({ length: pairCount }, (_, i) => ({
            agentA: `agent-a-${i}`,
            agentB: `agent-b-${i}`,
      ...
      fc.property(
        fc.array(
          fc.record({
            agentA: agentIdArb,
            agentB: agentIdArb,
            ratingAtoB: fc.integer({ min: 0, max: config.anomaly.collusionRatingMin }),
            ratingBtoA: fc.integer({ min: 0, max: config.anomaly.collusionRatingMin }),
            otherMembersAvgRating: fc.integer({ min: 0, max: 100 }),
          }),
