...
function buildFullMission(
  core: ReturnType<typeof arbCoreMissionRecord['generate']> extends fc.Value<infer T> ? T : never,
  ...
  it('idempotency — applying derivation rules twice yields identical results', () => {
    fc.assert(
      fc.property(
        ...
        fc.option(fc.array(arbWorkPackage, { minLength: 0, maxLength: 8 }), { nil: undefined }),
        fc.option(fc.array(arbMessageLogEntry, { minLength: 0, maxLength: 12 }), { nil: undefined }),
        fc.option(fc.array(arbAgentCrewMember, { minLength: 0, maxLength: 8 }), { nil: undefined }),
    ...
    fc.assert(
      fc.property(
        ...
        (core, basePackages, extraCompleted) => {
          const missionBefore: MissionRecord = { ...core, workPackages: [...basePackages] } as MissionRecord;
    ...
    fc.assert(
      fc.property(
        ...
        (core, baseCrew, extraActive) => {
          const missionBefore: MissionRecord = { ...core, agentCrew: [...baseCrew] } as MissionRecord;
      ...
      fc.property(
        ...
        (core, orgA, orgB, wps, msgs, crew) => {
          const missionA: MissionRecord = {
            ...core, organization: orgA, workPackages: wps, messageLog: msgs, agentCrew: crew,
          } as MissionRecord;
          const missionB: MissionRecord = {
            ...core, organization: orgB, workPackages: wps, messageLog: msgs, agentCrew: crew,
          } as MissionRecord;

          // taskCount, messageCount, activeAgentCount must be identical
          expect(deriveTaskCount(missionA)).toBe(deriveTaskCount(missionB));
          expect(deriveMessageCount(missionA)).toBe(deriveMessageCount(missionB));
          expect(deriveActiveAgentCount(missionA)).toBe(deriveActiveAgentCount(missionB));
        },
      ),
      { numRuns: 100 },
    );
    ...
    fc.assert(
      fc.property(
        ...
        (core, org, wpsA, wpsB, msgs, crew) => {
          const missionA: MissionRecord = {
            ...core, organization: org, workPackages: wpsA, messageLog: msgs, agentCrew: crew,
          } as MissionRecord;
          const missionB: MissionRecord = {
            ...core, organization: org, workPackages: wpsB, messageLog: msgs, agentCrew: crew,
          } as MissionRecord;

          // departmentLabels, messageCount, activeAgentCount must be identical
          expect(deriveDepartmentLabels(missionA)).toEqual(deriveDepartmentLabels(missionB));
          expect(deriveMessageCount(missionA)).toBe(deriveMessageCount(missionB));
          expect(deriveActiveAgentCount(missionA)).toBe(deriveActiveAgentCount(missionB));
        },
      ),
    ...
    fc.assert(
      fc.property(
        ...
        (core, workPackages) => {
          ...
          expect(deriveCompletedTaskCount(mission)).toBeLessThanOrEqual(deriveTaskCount(mission));
        },
      ),
      { numRuns: 100 },
    );
    ...
    fc.assert(
      fc.property(
        ...
        (core, agentCrew) => {
          ...
          expect(deriveActiveAgentCount(mission)).toBeLessThanOrEqual(totalCrew);
        },
      ),
      { numRuns: 100 },
    );
