...
describe('Property 10: 信誉阈值过滤', () => {
  ...
  it('filterByReputationThreshold excludes all candidates with overallScore < threshold', () => {
    fc.assert(
      fc.property(candidateListArb, thresholdArb, (candidates, threshold) => {
        const result = scorer.filterByReputationThreshold(candidates, threshold);
  ...
  it('filterByTaskforceRequirements for lead: requires overallScore >= 600', () => {
    fc.assert(
      fc.property(candidateListArb, (candidates) => {
        ...
        for (const c of result) {
          expect(c.profile.overallScore).toBeGreaterThanOrEqual(cfg.scheduling.leadMinScore);
        }
  ...
  it('filterByTaskforceRequirements for worker: requires overallScore >= 300', () => {
    fc.assert(
      fc.property(candidateListArb, (candidates) => {
        ...
        for (const c of result) {
          expect(c.profile.overallScore).toBeGreaterThanOrEqual(cfg.scheduling.workerMinScore);
        }
  ...
  it('filterByTaskforceRequirements for reviewer: requires qualityScore >= 500', () => {
    fc.assert(
      fc.property(candidateListArb, (candidates) => {
        ...
        const expectedCount = candidates.filter(
          (c) => c.profile.dimensions.qualityScore >= cfg.scheduling.reviewerMinQuality,
        ).length;
        expect(result.length).toBe(expectedCount);
      }),
      { numRuns: 200 },
  ...
  it('filterByTaskforceRequirements never returns candidates that fail the role requirement', () => {
    fc.assert(
      fc.property(
        candidateListArb,
        fc.constantFrom('lead' as const, 'worker' as const, 'reviewer' as const),
        (candidates, role) => {
          ...
          for (const c of candidates) {
            if (!resultIds.has(c.agentId)) {
              ...
              switch (role) {
                case 'lead':
                  expect(c.profile.overallScore).toBeLessThan(cfg.scheduling.leadMinScore);
                  break;
                case 'worker':
                  expect(c.profile.overallScore).toBeLessThan(cfg.scheduling.workerMinScore);
                  break;
                case 'reviewer':
    ...
    fc.assert(
      fc.property(
        ...
        (candidates, threshold, role) => {
          const thresholdResult = scorer.filterByReputationThreshold(candidates, threshold);
          const roleResult = scorer.filterByTaskforceRequirements(candidates, role);
