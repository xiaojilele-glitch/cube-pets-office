...
class InMemoryVectorStore implements VectorStoreAdapter {
  ...
  async search() { return []; }
  async delete(col: string, ids: string[]) { const c = this.collections.get(col); if (c) ids.forEach(id => c.delete(id)); }
...
function makeRow(id: string, projectId: string, tier: 'hot' | 'cold', daysAgo: number): RagChunkMetadataRow {
  const date = new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString();
  return {
    chunk_id: id, source_type: 'task_result', source_id: 'src-1',
    project_id: projectId, chunk_index: 0, content_hash: 'abc123',
    token_count: 100, code_language: null, function_signature: null,
    agent_id: null, ingested_at: date, last_accessed_at: date,
    storage_tier: tier, metadata_json: '{}',
  };
}
...
describe('Property 21: Promoting cold chunk updates tier to hot', () => {
  beforeEach(() => { cleanup(); resetRAGConfigCache(); });
  afterEach(() => cleanup());
...
describe('Property 22: Stale hot chunks get archived with lifecycle logs', () => {
  ...
  it('stale hot chunks get archived and produce lifecycle logs', async () => {
    ...
    const lcManager = new LifecycleManager(vectorStore, metaStore, hotCold, LOG_PATH);
