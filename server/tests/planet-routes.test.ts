...
import type { MissionRecord, MissionStage } from '../../shared/mission/contracts.js';
import { missionToPlanetOverview, buildPlanetInteriorStages, buildPlanetInteriorAgents } from '../routes/planets.js';
...
describe('missionToPlanetOverview', () => {
  ...
  it('resolves currentStageLabel from stages array', () => {
    const planet = missionToPlanetOverview(makeMission({ currentStageKey: 'execute' }));
  ...
  it('returns undefined currentStageLabel when currentStageKey is missing', () => {
    const planet = missionToPlanetOverview(makeMission({ currentStageKey: undefined }));
    expect(planet.currentStageLabel).toBeUndefined();
  });

  it('returns undefined currentStageLabel when key does not match any stage', () => {
    const planet = missionToPlanetOverview(makeMission({ currentStageKey: 'nonexistent' }));
    expect(planet.currentStageLabel).toBeUndefined();
  });
  ...
  it('returns empty tags when organization is undefined', () => {
    const planet = missionToPlanetOverview(makeMission({ organization: undefined }));
  ...
  it('passes through waitingFor field', () => {
    const planet = missionToPlanetOverview(makeMission({ waitingFor: 'user approval' }));
...
describe('buildPlanetInteriorStages', () => {
  ...
  it('preserves key, label, detail, startedAt, completedAt from source stages', () => {
    const stages: MissionStage[] = [
      { key: 'recv', label: 'Receive', status: 'done', detail: 'All good', startedAt: 100, completedAt: 200 },
    ];
    const result = buildPlanetInteriorStages(stages);
...
describe('buildPlanetInteriorAgents', () => {
  ...
  it('infers worker agents from workPackages assignees', () => {
    const mission = makeMission({
      workPackages: [
        { id: 'wp1', title: 'Task A', assignee: 'alice', stageKey: 'execute', status: 'running', deliverable: 'Build UI' },
        { id: 'wp2', title: 'Task B', assignee: 'bob', stageKey: 'receive', status: 'passed', score: 90 },
      ],
    });
    const agents = buildPlanetInteriorAgents(mission, stages);
  ...
  it('skips workPackages without assignee', () => {
    const mission = makeMission({
      workPackages: [
        { id: 'wp1', title: 'Unassigned', stageKey: 'execute', status: 'pending' },
      ],
    });
    const agents = buildPlanetInteriorAgents(mission, stages);
    // Only mission-core
  ...
  it('prefers running package as active for multi-package assignee', () => {
    const mission = makeMission({
      workPackages: [
        { id: 'wp1', title: 'Done task', assignee: 'alice', stageKey: 'receive', status: 'passed' },
        { id: 'wp2', title: 'Active task', assignee: 'alice', stageKey: 'execute', status: 'running', deliverable: 'Coding' },
      ],
    });
    const agents = buildPlanetInteriorAgents(mission, stages);
    const alice = agents.find((a) => a.id === 'alice');
  ...
  it('maps mission-core status from mission status', () => {
    const runningAgents = buildPlanetInteriorAgents(makeMission({ status: 'running' }), stages);
    expect(runningAgents.find((a) => a.id === 'mission-core')!.status).toBe('thinking');

    const doneAgents = buildPlanetInteriorAgents(makeMission({ status: 'done' }), stages);
    expect(doneAgents.find((a) => a.id === 'mission-core')!.status).toBe('done');

    const failedAgents = buildPlanetInteriorAgents(makeMission({ status: 'failed' }), stages);
    expect(failedAgents.find((a) => a.id === 'mission-core')!.status).toBe('error');

    const queuedAgents = buildPlanetInteriorAgents(makeMission({ status: 'queued' }), stages);
    expect(queuedAgents.find((a) => a.id === 'mission-core')!.status).toBe('idle');
  });

  it('maps all workPackage statuses correctly', () => {
    const statuses: Array<{ wpStatus: 'pending' | 'running' | 'passed' | 'failed' | 'verified'; expected: string }> = [
    ...
    for (const { wpStatus, expected } of statuses) {
      const mission = makeMission({
        workPackages: [{ id: 'wp1', title: 'T', assignee: 'agent1', stageKey: 'execute', status: wpStatus }],
      });
      const agents = buildPlanetInteriorAgents(mission, stages);
  ...
  it('assigns angles within stage arc boundaries', () => {
    const mission = makeMission({
      currentStageKey: 'execute',
      workPackages: [
        { id: 'wp1', title: 'T1', assignee: 'a1', stageKey: 'execute', status: 'running' },
        { id: 'wp2', title: 'T2', assignee: 'a2', stageKey: 'execute', status: 'running' },
      ],
    });
    const agents = buildPlanetInteriorAgents(mission, stages);
  ...
  it('all agent angles are in [0, 360)', () => {
    const mission = makeMission({
      workPackages: [
        { id: 'wp1', title: 'T1', assignee: 'a1', stageKey: 'receive', status: 'running' },
        { id: 'wp2', title: 'T2', assignee: 'a2', stageKey: 'execute', status: 'pending' },
        { id: 'wp3', title: 'T3', assignee: 'a3', stageKey: 'finalize', status: 'passed' },
      ],
    });
    const agents = buildPlanetInteriorAgents(mission, stages);
  ...
  it('uses stageKey fallback label when stage not found in interiorStages', () => {
    const mission = makeMission({
      workPackages: [
        { id: 'wp1', title: 'T', assignee: 'ghost', stageKey: 'unknown_stage', status: 'running' },
      ],
    });
    const agents = buildPlanetInteriorAgents(mission, stages);
...
describe('GET /api/planets/:id/interior', () => {
  ...
  it('returns interior data with stages, agents, events, summary, waitingFor', async () => {
    ...
    const coreAgent = interior.agents.find((a: { id: string }) => a.id === 'mission-core');
    expect(coreAgent).toBeDefined();
    expect(coreAgent.role).toBe('orchestrator');
  });

  it('returns 404 for a non-existent mission id', async () => {
    const response = await fetch(`${baseUrl}/api/planets/nonexistent_id/interior`);
    const body = await response.json();
  ...
  it('does not break the existing /:id endpoint', async () => {
    ...
    const interiorRes = await fetch(`${baseUrl}/api/planets/${task.id}/interior`);
