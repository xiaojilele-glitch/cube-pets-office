...
const arbSoulMd: fc.Arbitrary<string> = fc.string({
  minLength: 1,
  maxLength: 100,
});
const arbModel: fc.Arbitrary<string> = fc.string({
  minLength: 1,
  maxLength: 20,
});
    ...
    fc.assert(
      fc.property(
        ...
        (agentId, soulMd, model, tpl) => {
          ...
          expect(agent.config.soulMd).toBe(
    ...
    fc.assert(
      fc.property(
        ...
        (agentId, soulMd, model, tpl) => {
          ...
          const entry = agent
            .getRoleOperationLog()
            .find(e => e.roleId === tpl.roleId && e.action === "load");
  ...
  it("after loadRole with inherited role, state reflects resolved (merged) template", () => {
    ...
    fc.assert(
      fc.property(
        ...
        arbModel,
        arbParentChild,
        (agentId, soulMd, model, { parent, child }) => {
          freshState();
          ...
          const rs = agent.getRoleState();
          expect(new Set(rs.loadedSkillIds)).toEqual(
            new Set(resolved.requiredSkillIds)
          );
    ...
    fc.assert(
      fc.property(
        ...
        (agentId, soulMd, model, tpl) => {
          ...
          const entry = agent
            .getRoleOperationLog()
            .find(e => e.roleId === tpl.roleId && e.action === "unload");
          expect(entry).toBeDefined();
          expect(entry!.agentId).toBe(agentId);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (agentId, soulMd, model, tplA, [roleAId, roleBId]) => {
          ...
          await expect(
            agent.switchRole(roleBId, "test-mission")
          ).rejects.toThrow();
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (agentId, soulMd, model, tplA, [roleAId, roleBId]) => {
          ...
          expect(agent.getRoleState().loadedSkillIds).toEqual(
            preSwitch.loadedSkillIds
          );
          expect(agent.getRoleState().loadedMcpIds).toEqual(
            preSwitch.loadedMcpIds
          );
          expect(agent.getRoleState().currentRoleLoadedAt).toBe(
            preSwitch.currentRoleLoadedAt
          );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (agentId, soulMd, model, tpl) => {
          ...
          await expect(
            agent.switchRole(tpl.roleId, "test-mission")
          ).rejects.toThrow();
...
describe("Agent Property 7: roleLoadPolicy 模型配置合并", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (agentId, soulMd, model, agentBaseConfig, tpl) => {
          ...
          const agent = createAgentWithPolicy(
            agentId,
            soulMd,
            model,
            "override",
          ...
          expect(effective!.temperature).toBe(
            tpl.defaultModelConfig.temperature
          );
      ...
      fc.property(
        ...
        (agentId, soulMd, model, agentBaseConfig, tpl) => {
          ...
          const expectedMaxTokens = Math.max(
            ...
            tpl.defaultModelConfig.maxTokens
          );
  ...
  it("all three policies produce correct effectiveModelConfig (combined check)", () => {
    const arbPolicy = fc.constantFrom(
      "override" as const,
      "prefer_agent" as const,
      "merge" as const
    );
