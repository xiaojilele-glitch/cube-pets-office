...
describe('Integration: Skill 注册 → 解析 → 激活 → 执行', () => {
  ...
  it('registers skills, resolves with dependencies, activates, and records metrics', () => {
    ...
    const bindings = registry.resolveSkills(['data-analysis', 'security-audit']);
  ...
  it('disable/enable cycle works correctly with audit trail', () => {
    ...
    // Re-enable
    registry.enableSkill('code-review', '1.0.0', 'admin', 'maintenance complete');

    const bindingsAfter = registry.resolveSkills(['code-review', 'security-audit']);
  ...
  it('version management: multiple versions coexist', () => {
    registry.registerSkill(codeReviewSkill);
    registry.registerSkill({ ...codeReviewSkill, version: '2.0.0', summary: 'Enhanced review' });
