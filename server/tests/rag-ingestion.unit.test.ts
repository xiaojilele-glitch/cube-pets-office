...
import type { IngestionPayload, VectorRecord } from '../../shared/rag/contracts.js';
...
import { RAGEventListener, RAG_EVENT_TYPES } from '../rag/ingestion/event-listener.js';
...
import type { VectorStoreAdapter, SearchHit, CollectionInfo, HealthStatus } from '../rag/store/vector-store-adapter.js';

/* ─── Mocks ─── */

class InMemoryVectorStore implements VectorStoreAdapter {
  private collections = new Map<string, Map<string, VectorRecord>>();
  async createCollection(name: string) { this.collections.set(name, new Map()); }
  async upsert(col: string, records: VectorRecord[]) {
    let c = this.collections.get(col);
    if (!c) { c = new Map(); this.collections.set(col, c); }
    for (const r of records) c.set(r.id, r);
  }
  async search() { return []; }
  async delete() {}
  async collectionInfo(name: string): Promise<CollectionInfo> { return { name, vectorCount: 0, dimension: 8, status: 'ready' }; }
  async healthCheck(): Promise<HealthStatus> { return { connected: true, backend: 'memory', latencyMs: 0 }; }
}

class SuccessProvider implements EmbeddingProvider {
  readonly dimension = 8;
  readonly modelName = 'test';
  async embed(texts: string[]) { return texts.map(() => Array.from({ length: 8 }, () => 0.5)); }
}
...
function makeContent(n: number) { return Array.from({ length: n }, (_, i) => `word${i}`).join(' '); }
...
describe('RAGEventListener', () => {
  it('converts task.completed to task_result IngestionPayload', () => {
    const event = RAGEventListener.toRAGEvent({
      type: 'task.completed',
      payload: { id: 'task-1', projectId: 'proj-1', content: 'Task done', agentId: 'agent-a' },
    });
  ...
  it('converts mission.finished to mission_log', () => {
    const event = RAGEventListener.toRAGEvent({
      ...
      payload: { id: 'mission-1', projectId: 'proj-1', content: 'Mission complete' },
    });
    expect(event).not.toBeNull();
  ...
  it('converts code.committed to code_snippet', () => {
    const event = RAGEventListener.toRAGEvent({
      type: 'code.committed',
      payload: { id: 'commit-1', projectId: 'proj-1', content: 'function foo() {}' },
    });
    expect(event).not.toBeNull();
  });
  ...
  it('converts document.uploaded to document', () => {
    const event = RAGEventListener.toRAGEvent({
      ...
      payload: { id: 'doc-1', projectId: 'proj-1', content: 'Document content' },
    });
    expect(event).not.toBeNull();
  });

  it('returns null for unsupported event types', () => {
    const event = RAGEventListener.toRAGEvent({ type: 'unknown.event', payload: {} });
...
describe('DedupChecker', () => {
  ...
  it('buildDedupKey produces expected format', () => {
    expect(buildDedupKey('task_result', 'src-1', 'abc')).toBe('task_result:src-1:abc');
  });
});
...
describe('DataCleaner', () => {
  ...
  it('normalizes line endings', () => {
    const result = cleaner.clean(makePayload({ content: 'hello\r\nworld\rfoo' }));
