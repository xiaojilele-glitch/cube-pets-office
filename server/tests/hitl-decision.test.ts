...
describe("submitMissionDecision — decision history append", () => {
  it("appends a DecisionHistoryEntry after successful decision", () => {
    ...
    const result = submitMissionDecision(runtime, "task_3", {
      optionId: "left",
    });
...
describe("submitMissionDecision — multi-step decision chain", () => {
  it("when orchestrator puts task back into waiting, task enters waiting again with new decision", async () => {
    ...
    const result = submitMissionDecision(missionRuntime, task.id, {
      optionId: "path-a",
    });
    expect(result.ok).toBe(true);
    if (result.ok) {
    ...
    const result2 = submitMissionDecision(missionRuntime, task.id, {
      optionId: "confirm",
    });
...
describe("API endpoints", () => {
  ...
  it("GET /api/decision-templates returns built-in templates", async () => {
    ...
    expect(
      body.templates.map((t: { templateId: string }) => t.templateId)
    ).toContain("execution-plan-approval");
    expect(
      body.templates.map((t: { templateId: string }) => t.templateId)
    ).toContain("stage-gate");
    expect(
      body.templates.map((t: { templateId: string }) => t.templateId)
    ).toContain("risk-confirmation");
  });
});
