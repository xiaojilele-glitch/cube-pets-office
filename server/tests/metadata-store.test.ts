...
function makeTempPath(): string {
  const dir = join(tmpdir(), 'metadata-store-test-' + Date.now() + '-' + Math.random().toString(36).slice(2));
...
describe('MetadataStore', () => {
  ...
  it('combines multiple filters (AND logic)', () => {
    store.upsertBatch([
      makeRow({ chunk_id: 'c1', project_id: 'p1', source_type: 'code_snippet', storage_tier: 'hot' }),
      makeRow({ chunk_id: 'c2', project_id: 'p1', source_type: 'conversation', storage_tier: 'hot' }),
      makeRow({ chunk_id: 'c3', project_id: 'p2', source_type: 'code_snippet', storage_tier: 'hot' }),
    ]);
    const results = store.query({ projectId: 'p1', sourceType: 'code_snippet' });
    expect(results).toHaveLength(1);
    expect(results[0].chunk_id).toBe('c1');
  });

  it('returns all rows when filter is empty', () => {
    store.upsertBatch([makeRow({ chunk_id: 'c1' }), makeRow({ chunk_id: 'c2' })]);
  ...
  it('updates last_accessed_at timestamp', () => {
    store.upsert(makeRow({ chunk_id: 'c1', last_accessed_at: '2025-01-01T00:00:00Z' }));
  ...
  it('starts empty when file does not exist', () => {
    const freshStore = new MetadataStore(join(tmpdir(), 'nonexistent-' + Date.now() + '.json'));
