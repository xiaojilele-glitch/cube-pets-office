...
describe("phase1 workspace access guard", () => {
  ...
  it("writes and reads files only inside the requested workspace scope", async () => {
    ...
    expect(
      readAgentWorkspaceFile("scout", "2026-03/report.md", "reports")
    ).toBe("# report");
    expect(
      resolveAgentWorkspacePath("scout", "2026-03/report.md", "reports")
    ).toBe(resolvedPath);
  });
  ...
  it("blocks absolute and traversal paths", async () => {
    ...
    expect(() =>
      resolveAgentWorkspacePath("scout", "/temp/outside.txt")
    ).toThrow(/Absolute/);
  });
});
