...
function makeTempPath(): string {
  const dir = join(tmpdir(), 'dedup-checker-test-' + Date.now() + '-' + Math.random().toString(36).slice(2));
...
describe('buildDedupKey', () => {
  it('formats key as sourceType:sourceId:contentHash', () => {
    expect(buildDedupKey('task_result', 'src-1', 'abc123')).toBe('task_result:src-1:abc123');
  });
});
...
describe('DedupChecker', () => {
  ...
  it('starts empty when file does not exist', () => {
    const fresh = new DedupChecker(join(tmpdir(), 'nonexistent-' + Date.now() + '.json'));
