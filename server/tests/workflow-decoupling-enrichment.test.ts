...
describe('extractWorkPackages correctness', () => {
  it('populates workPackages with valid entries having id, title, stageKey, and valid status', async () => {
    ...
    const completed = await orchestrator.applyExecutorEvent(
      completedEvent(mission.id, {
        workPackages: [
          { id: 'wp-1', title: 'Build feature', stageKey: 'execute', status: 'passed' },
          { id: 'wp-2', title: 'Write tests', stageKey: 'execute', status: 'verified' },
        ],
      }),
    );
    ...
    const completed = await orchestrator.applyExecutorEvent(
      completedEvent(mission.id, {
        workPackages: [
          ...
          { id: 'wp-2', title: 'Bad status', stageKey: 'execute', status: 'unknown' },
          { id: 'wp-3', title: 'Another bad', stageKey: 'execute', status: 'cancelled' },
        ],
      }),
    );
    ...
    const completed = await orchestrator.applyExecutorEvent(
      completedEvent(mission.id, {
        workPackages: [
          ...
          { id: 'wp-valid', title: 'Has ID', stageKey: 'execute', status: 'running' },
        ],
      }),
    );
    ...
    const completed = await orchestrator.applyExecutorEvent(
      completedEvent(mission.id, {
        workPackages: [
          ...
          'string-entry',
          undefined,
          { id: 'wp-1', title: 'Valid', stageKey: 'execute', status: 'pending' },
        ],
      }),
    );
    ...
    const completed = await orchestrator.applyExecutorEvent(
      completedEvent(mission.id, {
        workPackages: [
          ...
          null,
          { id: 'wp-5', title: 'Also good', stageKey: 'review', status: 'failed' },
        ],
      }),
    );
  ...
  it('does not apply enrichment on job.progress events even with valid payload', async () => {
    ...
    const running = await orchestrator.applyExecutorEvent({
      ...
      payload: {
        ...
        messageLog: [
          { sender: 'Agent-A', content: 'Hello', time: 1000 },
        ],
      },
    });
