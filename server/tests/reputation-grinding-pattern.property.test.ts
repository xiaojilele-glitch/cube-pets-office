...
const complexityArb = fc.constantFrom<'low' | 'medium' | 'high'>('low', 'medium', 'high');

/** Generate a single TaskSummary with a recent timestamp */
function taskSummaryArb(timestampArb: fc.Arbitrary<string>): fc.Arbitrary<TaskSummary> {
  return fc.record({
...
describe('Property 17: 刷分模式检测', () => {
  ...
  it('detects grinding when low-complexity ratio > grindingTaskRatio AND count > grindingTaskCount', () => {
    fc.assert(
      fc.property(
        fc.array(taskSummaryArb(recentTimestampArb), { minLength: 0, maxLength: 60 }),
        (tasks) => {
  ...
  it('returns weight = lowComplexityWeight when grinding is detected', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (count) => {
          const tasks: TaskSummary[] = Array.from({ length: count }, (_, i) => ({
            taskId: String(i),
            complexity: 'low' as const,
            completedAt: new Date(Date.now() - (i + 1) * 60_000).toISOString(),
          }));
    ...
    fc.assert(
      fc.property(
        ...
        (totalCount) => {
          ...
          const maxLow = Math.floor(totalCount * config.anomaly.grindingTaskRatio);
          ...
          for (let i = 0; i < nonLowCount; i++) {
            tasks.push({
              ...
              complexity: 'medium',
              completedAt: new Date(Date.now() - (lowCount + i + 1) * 60_000).toISOString(),
            });
          }
  ...
  it('ignores tasks older than 24 hours', () => {
    fc.assert(
      fc.property(
        fc.array(taskSummaryArb(oldTimestampArb), { minLength: 1, maxLength: 50 }),
        (oldTasks) => {
          // Even if all are low-complexity and many, they are outside 24h window
          const allLow = oldTasks.map((t) => ({ ...t, complexity: 'low' as const }));
