...
function makeTask(overrides?: Partial<CompetitionTaskRequest>): CompetitionTaskRequest {
  return {
    taskId: 'task-1',
...
describe('CompetitionEngine', () => {
  ...
  describe('computeUncertainty', () => {
    ...
    it('clamps to 1 for worst-case inputs', () => {
      const task = makeTask({ historicalFailRate: 1.0, descriptionAmbiguity: 1.0 });
      expect(engine.computeUncertainty(task, 0.0)).toBeCloseTo(1.0, 5);
    });

    it('returns value in [0, 1]', () => {
      const task = makeTask({ historicalFailRate: 0.6, descriptionAmbiguity: 0.3 });
  ...
  describe('runCompetition', () => {
    ...
    it('marks external agents correctly', async () => {
      ...
      const session = await engine.runCompetition(makeTask(), ['internal', 'ext'], 60_000);
  ...
  describe('computeDeadline', () => {
    ...
    it('respects custom maxDeadlineMs config', () => {
      const customConfig = makeConfig({
        competition: { defaultContestantCount: 3, maxDeadlineMs: 100_000, budgetRatio: 0.3 },
      });
