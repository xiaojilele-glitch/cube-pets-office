...
function flushAndGetEvents(store: ReturnType<typeof createMockStore>, collector: EventCollector) {
...
describe('installMissionInterceptor', () => {
  ...
  it('does not throw if orchestrator has no hooks property', () => {
    const orchestrator = {};
    expect(() => installMissionInterceptor(orchestrator, collector)).not.toThrow();
  });
});
...
describe('installMessageBusInterceptor', () => {
  ...
  it('wraps send() and emits MESSAGE_SENT + MESSAGE_RECEIVED', async () => {
    ...
    const result = await messageBus.send('agent-a', 'agent-b', 'hello', 'wf-1', 'plan');
  ...
  it('still calls original send() and returns its result', async () => {
    ...
    expect(originalSend).toHaveBeenCalledWith('a', 'b', 'msg', 'wf', 'execute', undefined);
  });
  ...
  it('does not break if original send() throws', async () => {
    ...
    await expect(messageBus.send('a', 'b', 'msg', 'wf', 'plan')).rejects.toThrow('bus error');
  });
});
...
describe('installExecutorInterceptor', () => {
  ...
  it('emits RESOURCE_ACCESSED for artifact events', async () => {
    ...
    const { req, res, next } = createMockReqRes({
      event: {
        ...
        stageKey: 'finalize',
        artifacts: [
          { kind: 'file', name: 'output.txt', path: '/tmp/output.txt', description: 'Result file' },
        ],
      },
    });
  ...
  it('emits RESOURCE_ACCESSED for container instance events', async () => {
    ...
    const events = await flushAndGetEvents(store, collector);
    const resourceEvent = events.find(e =>
