...
import { buildPlanetSummaryRecord, buildPlanetDetailRecord } from '../../client/src/lib/tasks-store';
...
describe('E2E: Mission → /api/planets → tasks-store', () => {
  ...
  it('creates a mission with enrichment, fetches interior, and produces valid MissionTaskDetail', async () => {
    ...
    runtime.patchEnrichment(task.id, {
      ...
      workPackages: [
        { id: 'wp1', title: 'Build UI', assignee: 'alice', stageKey: 'execute', status: 'running', deliverable: 'Dashboard' },
        { id: 'wp2', title: 'Write tests', assignee: 'bob', stageKey: 'execute', status: 'passed', score: 95 },
      ],
      messageLog: [
