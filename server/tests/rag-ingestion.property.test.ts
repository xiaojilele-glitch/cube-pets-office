...
import { SOURCE_TYPES, type SourceType, type IngestionPayload } from '../../shared/rag/contracts.js';
...
import type { VectorStoreAdapter, SearchOptions, SearchHit, CollectionInfo, HealthStatus } from '../rag/store/vector-store-adapter.js';
...
class InMemoryVectorStore implements VectorStoreAdapter {
  ...
  async createCollection(name: string, _dim: number) { this.collections.set(name, new Map()); }
  ...
  async collectionInfo(name: string): Promise<CollectionInfo> { return { name, vectorCount: this.collections.get(name)?.size ?? 0, dimension: 8, status: 'ready' }; }
  async healthCheck(): Promise<HealthStatus> { return { connected: true, backend: 'memory', latencyMs: 0 }; }
  getCount(col: string): number { return this.collections.get(col)?.size ?? 0; }
}
...
class FailProvider implements EmbeddingProvider {
  ...
  async embed(_texts: string[]): Promise<number[][]> { throw new Error('Embedding failed'); }
}
...
const arbPayload: fc.Arbitrary<IngestionPayload> = fc.record({
  ...
  agentId: fc.option(fc.stringMatching(/^agent-[a-z]{2,5}$/), { nil: undefined }),
});
...
describe('Property 3: Ingesting same payload twice produces same vector count', () => {
  beforeEach(() => { resetRAGConfigCache(); cleanup(); });
  afterEach(() => cleanup());
