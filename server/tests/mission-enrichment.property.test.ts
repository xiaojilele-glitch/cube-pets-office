...
const arbDepartment = fc.record({
  key: fc.string({ minLength: 1, maxLength: 12 }).map(s => s.replace(/\s/g, '_') || 'dept'),
  label: fc.string({ minLength: 1, maxLength: 20 }),
  managerName: fc.option(fc.string({ minLength: 1, maxLength: 16 }), { nil: undefined }),
});
...
const arbWorkPackage: fc.Arbitrary<MissionWorkPackage> = fc.record({
  ...
  stageKey: fc.string({ minLength: 1, maxLength: 12 }).map(s => s.replace(/\s/g, '_') || 'stg'),
  status: arbWpStatus,
...
const arbMessageLogEntry: fc.Arbitrary<MissionMessageLogEntry> = fc.record({
  ...
  stageKey: fc.option(fc.string({ minLength: 1, maxLength: 12 }), { nil: undefined }),
});
  ...
  it('patchEnrichment with organization results in mission containing that organization', () => {
    fc.assert(
      fc.property(arbOrganization, (org) => {
        ...
        const enriched = runtime.patchEnrichment(task.id, { organization: org });

        expect(enriched).toBeDefined();
        expect(enriched!.organization).toBeDefined();
        expect(enriched!.organization!.departments).toHaveLength(org.departments.length);
      ...
      fc.property(
        ...
        (workPackages) => {
          ...
          for (let i = 0; i < workPackages.length; i++) {
            ...
            expect(enriched!.workPackages![i].status).toBe(workPackages[i].status);
          }
        },
      ),
      { numRuns: 100 },
      ...
      fc.property(
        ...
        (messageLog) => {
          ...
          for (let i = 0; i < messageLog.length; i++) {
            ...
            expect(enriched!.messageLog![i].content).toBe(messageLog[i].content);
          }
        },
      ),
      { numRuns: 100 },
    ...
    fc.assert(
      fc.property(
        ...
        (org, workPackages, messageLog) => {
          ...
          runtime.patchEnrichment(task.id, { organization: org, workPackages, messageLog });
