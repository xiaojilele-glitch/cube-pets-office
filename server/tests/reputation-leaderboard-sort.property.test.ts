...
import {
  ReputationService,
  LeaderboardEntry,
} from "../core/reputation/reputation-service.js";
...
function getSortValue(entry: LeaderboardEntry, sortBy: SortByOption): number {
  return sortBy === "overallScore"
    ? entry.overallScore
    : entry.dimensions[sortBy];
}
  ...
  it("leaderboard results are in descending order by the specified sortBy dimension", () => {
    fc.assert(
      fc.property(
        ...
        sortByArb,
        fc.array(fc.integer({ min: -400, max: 400 }), {
          minLength: 2,
          maxLength: 15,
        }),
        (agentCount: number, sortBy: SortByOption, deltas: number[]) => {
          ...
          for (let i = 1; i < entries.length; i++) {
            expect(getSortValue(entries[i - 1], sortBy)).toBeGreaterThanOrEqual(
              getSortValue(entries[i], sortBy)
            );
          }
        }
      ),
    ...
    fc.assert(
      fc.property(
        ...
        (agentCount: number, sortBy: SortByOption, deltas: number[]) => {
          ...
          for (let i = 1; i < entries.length; i++) {
            expect(getSortValue(entries[i - 1], sortBy)).toBeLessThanOrEqual(
              getSortValue(entries[i], sortBy)
            );
          }
        }
