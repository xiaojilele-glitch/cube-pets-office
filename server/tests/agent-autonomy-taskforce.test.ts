...
function makeSA(pm: CapabilityProfileManager, c = DC) {
  return new SelfAssessment(pm, c);
}
...
const arbAssessmentResult = fc.record({
  ...
  decision: fc.constantFrom(
    "ACCEPT" as const,
    "ACCEPT_WITH_CAVEAT" as const,
    "REQUEST_ASSIST" as const,
    "REJECT_AND_REFER" as const
  ),
...
describe("Property 24: Lead 选举正确性", () => {
  it("electLead returns the agent with the highest fitnessScore", () => {
    fc.assert(
      fc.property(
        fc
          .array(arbAssessmentResult, { minLength: 1, maxLength: 20 })
          .filter(arr => {
        ...
        candidates => {
          ...
          const best = candidates.reduce(
            (a, b) => (b.fitnessScore > a.fitnessScore ? b : a),
            candidates[0]
          );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async app => {
          ...
          const members = await tfm.processApplications(
            "tf-test",
            applications
          );
      ...
      fc.property(
        ...
        fc.array(
          arbMember.map(m => ({
            ...m,
            role: fc.sample(
              fc.constantFrom("worker" as const, "reviewer" as const),
              1
            )[0],
          })),
...
describe("Property 29: 全局开关回退", () => {
  ...
  it("when autonomy disabled, CompetitionEngine.shouldTrigger still evaluates conditions but allocator never reaches it", () => {
    ...
    const sa = makeSA(pm, disabledConfig);
    const ce = new CompetitionEngine(
      pm,
      new CostMonitor(disabledConfig),
      disabledConfig
    );
