...
import type { IngestionPayload, ChunkRecord, ChunkMetadata } from '../../shared/rag/contracts.js';
...
import { IngestionPipeline, type IngestionPipelineDeps } from '../rag/ingestion/ingestion-pipeline.js';
...
function tmpPath(name: string): string {
  const dir = join(tmpdir(), `ingestion-pipeline-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
function createStubVectorStore(): VectorStoreAdapter & { stored: Map<string, any[]> } {
  ...
  return {
    ...
    async search() { return []; },
    async delete() {},
    async collectionInfo(name) { return { name, vectorCount: 0, dimension: 4, status: 'ready' }; },
    async healthCheck() { return { connected: true, backend: 'stub', latencyMs: 0 }; },
  };
}

/** Build full pipeline deps with stubs */
function buildDeps(overrides?: Partial<IngestionPipelineDeps>): IngestionPipelineDeps & {
...
describe('IngestionPipeline', () => {
  ...
  it('writes vectors to the correct collection (rag_{projectId})', async () => {
    ...
    expect(deps.vectorStore.stored.get('rag_my-proj')!.length).toBeGreaterThan(0);
  });
  ...
  it('returns deduplicated=true on second ingest of same content', async () => {
    const payload = makePayload({ sourceId: 'dup-src', content: 'same content' });
  ...
  it('does not deduplicate when content changes', async () => {
    ...
    const p2 = makePayload({ sourceId: 'src-x', content: 'version 2 different' });
  ...
  it('writes to DLQ when embedding fails completely', async () => {
    const failDeps = buildDeps({
      embeddingGenerator: new EmbeddingGenerator(new FailingEmbeddingProvider()),
    });
  ...
  it('writes to DLQ when vector store upsert fails', async () => {
    ...
    failingStore.upsert = async () => { throw new Error('Store down'); };
