...
describe("SelfAssessment", () => {
  ...
  describe("coarseFilter", () => {
    it("returns true when there is overlap", () => {
      expect(sa.coarseFilter(["coding", "design"], ["coding", "testing"])).toBe(
        true
      );
    });

    it("returns false when there is no overlap", () => {
      expect(sa.coarseFilter(["design", "art"], ["coding", "testing"])).toBe(
        false
      );
    });
  ...
  describe("computeFitnessScore", () => {
    it("computes correct weighted sum with default weights", () => {
      ...
      expect(
        sa.computeFitnessScore(1, 0, 1, 1, config.assessmentWeights)
      ).toBeCloseTo(1.0, 5);
    });
    ...
    it("returns 0 when all inputs are worst-case", () => {
      ...
      expect(
        sa.computeFitnessScore(0, 1, 0, 0, config.assessmentWeights)
      ).toBeCloseTo(0.0, 5);
    });

    it("clamps result to [0, 1]", () => {
      const result = sa.computeFitnessScore(
        0.5,
        0.5,
        0.5,
        0.5,
