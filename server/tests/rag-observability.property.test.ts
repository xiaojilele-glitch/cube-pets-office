      ...
      fc.property(
        ...
        (maxVectors) => {
          ...
          try {
            ...
            const belowResult = manager.checkVectorQuota('proj-quota', maxVectors - 1);
            ...
            const aboveResult = manager.checkVectorQuota('proj-quota', maxVectors + 100);
            expect(aboveResult.allowed).toBe(false);

            const noQuota = manager.checkVectorQuota('proj-no-quota', maxVectors + 100);
    ...
    fc.assert(
      fc.property(
        ...
        (operations) => {
          ...
          const totalTracked = snap.embeddingCost.tokenCount + snap.augmentation.tokenUsage;
