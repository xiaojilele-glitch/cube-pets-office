...
async function startServer(runtime: MissionRuntime, fetchImpl?: typeof fetch) {
  const app = express();
...
describe("tasks routes", () => {
  ...
  it("returns recent task events from GET /api/tasks/:id/events", async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/tasks/${task.id}/events?limit=3`
    );
    const body = await response.json();
    ...
    expect(
      body.events.map((event: { message: string }) => event.message)
    ).toContain("Collected first batch of notes");
  });
