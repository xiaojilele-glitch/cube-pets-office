...
const TEST_DIR = resolve(import.meta.dirname, '../../data/__test_audit_trail__');
...
describe('AuditTrail', () => {
  ...
  describe('query', () => {
    function seedTrail(trail: AuditTrail): AuditEntry[] {
      ...
      entries.push(trail.record({ action: 'ALERT_TRIGGERED', missionId: 'm1', details: {} }));
      entries.push(trail.record({ action: 'DOWNGRADE_APPLIED', missionId: 'm1', userId: 'u1', details: {} }));
      entries.push(trail.record({ action: 'ALERT_TRIGGERED', missionId: 'm2', details: {} }));
      entries.push(trail.record({ action: 'TASK_PAUSED', missionId: 'm2', userId: 'u2', details: {} }));
      entries.push(trail.record({ action: 'PERMISSION_CHANGED', userId: 'u1', details: {} }));
      return entries;
    }
    ...
    it('should filter by timeRange', () => {
      ...
      expect(result.every((e) => e.timestamp >= e1.timestamp && e.timestamp <= e1.timestamp)).toBe(true);
    });

    it('should combine multiple filters (AND logic)', () => {
      const trail = new AuditTrail(TEST_FILE);
      seedTrail(trail);
      const result = trail.query({ missionId: 'm1', action: 'DOWNGRADE_APPLIED' });
  ...
  describe('persist / load', () => {
    it('should persist and reload entries', () => {
      ...
      trail1.record({ action: 'ALERT_TRIGGERED', missionId: 'm1', details: { level: 'WARNING' } });
      trail1.record({ action: 'DOWNGRADE_APPLIED', missionId: 'm2', details: { model: 'gpt-4o' } });
      trail1.persist();
    ...
    it('should start with empty state when file has wrong structure', () => {
      writeFileSync(TEST_FILE, JSON.stringify({ version: 1, entries: 'not-an-array' }), 'utf-8');
      const trail = new AuditTrail(TEST_FILE);
      expect(trail.query({})).toHaveLength(0);
    });
  });
