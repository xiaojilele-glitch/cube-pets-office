...
function buildDefaultConfig(agent: AgentRow): HeartbeatConfig {
  const defaultInterval = clamp(
    Number(process.env.HEARTBEAT_INTERVAL_MINUTES) || 360,
    5,
    1440
  );
  const defaultMaxResults = clamp(
    Number(process.env.HEARTBEAT_MAX_RESULTS) || 6,
    3,
    12
  );
...
export class HeartbeatScheduler {
  ...
  syncAgent(agentId: string): HeartbeatStatus | null {
    ...
    const latestReport =
      reportStore.listHeartbeatReports(agentId, 1)[0] || null;
  ...
  async trigger(
    agentId: string,
    trigger: HeartbeatTrigger = "manual"
...
- If the search results are sparse, explicitly say the signal is weak.`,
      ...
      const summary: HeartbeatReportSummary | undefined =
        reportStore.listHeartbeatReports(agentId, 1)[0];
        ...
        emitEvent({
          ...
          status: {
            ...(this.states.get(agentId) ||
              this.createEmptyStatus(agentRow, config)),
            reportCount: summary
              ? reportStore.listHeartbeatReports(agentId, 200).length
              : 0,
          },
        });
      } else {
  ...
  private loadConfig(agentId: string): HeartbeatConfig {
    ...
    return {
      ...
      intervalMinutes: clamp(
        Number(stored.intervalMinutes ?? defaults.intervalMinutes) ||
          defaults.intervalMinutes,
        5,
        1440
      ),
      keywords:
        normalizeKeywords(stored.keywords).length > 0
          ? normalizeKeywords(stored.keywords)
          : defaults.keywords,
      focus:
        typeof stored.focus === "string" && stored.focus.trim()
          ? stored.focus.trim()
          : defaults.focus,
      maxResults: clamp(
  ...
  private scheduleNext(agentId: string, overrideDelayMs?: number): void {
    ...
    const current =
      this.states.get(agentId) || this.createEmptyStatus(agent, config);
    ...
    const timer = setTimeout(() => {
      ...
      if (remainingMs > 0) {
        this.scheduleNext(
          agentId,
          remainingMs + heartbeatRetryJitterMs(agentId)
        );
        return;
      }
      ...
      void this.trigger(agentId, "scheduled").catch(error => {
        if (isLLMTemporarilyUnavailableError(error)) {
          console.warn(`[Heartbeat] ${agentId} delayed: ${error.message}`);
          return;
        }
  ...
  private search(
    ...
    for (const workflow of workflows) {
      ...
      if (scored.score === 0 && candidates.length >= config.maxResults * 2)
        continue;
    ...
    for (const task of relatedTasks.slice(0, 40)) {
      ...
      if (scored.score === 0 && candidates.length >= config.maxResults * 3)
        continue;
    ...
    const results = Array.from(deduped.values())
      ...
      .map<HeartbeatSearchResult>(
        ({ sourceType, sourceId, title, snippet, matchedKeywords }) => ({
          sourceType,
          sourceId,
          title,
          snippet,
    ...
    return [
      {
        ...
        snippet:
          "The system has not accumulated enough matching workflow or task history for this heartbeat yet.",
        matchedKeywords: [],
      },
    ];
  }
  ...
  private getRelatedTasks(agent: AgentRow, tasks: TaskRow[]): TaskRow[] {
    ...
    if (agent.role === "manager") {
      return tasks.filter(
        task =>
          task.manager_id === agent.id || task.department === agent.department
      );
    }
