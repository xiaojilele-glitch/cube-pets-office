...
function normalizeWireApi(value?: string): "responses" | "chat_completions" {
  return value?.toLowerCase() === "responses"
    ? "responses"
    : "chat_completions";
}
...
export function getAIConfig(): AIConfig {
  ...
  return {
    ...
    wireApi: normalizeWireApi(
      process.env.OPENAI_WIRE_API || process.env.LLM_WIRE_API
    ),
    ...
    stream: normalizeBoolean(
      process.env.OPENAI_STREAM || process.env.LLM_STREAM,
      true
    ),
    chatThinkingType:
