...
import db, { type AgentCapabilityRow, type AgentRow, type TaskRow } from '../db/index.js';
...
const DIMENSIONS: ScoreDimension[] = ['accuracy', 'completeness', 'actionability', 'format'];
...
function tokenize(text: string): string[] {
  return (text.toLowerCase().match(/[\u4e00-\u9fff]{2,8}|[a-z][a-z0-9_-]{1,31}/g) || []).filter(
    (token) => !KEYWORD_STOPWORDS.has(token)
  );
}
...
function averageByDimension(tasks: TaskRow[]): Record<ScoreDimension, number> {
  ...
  for (const dim of DIMENSIONS) {
    ...
    result[dim] =
      tasks.reduce((sum, task) => sum + (Number(task[key]) || 0), 0) / Math.max(tasks.length, 1);
  }
...
function analyzeWeakDimensions(tasks: TaskRow[]): {
  ...
  return {
    weakDimensions: DIMENSIONS.filter((dim) => averages[dim] > 0 && averages[dim] < 3),
    averages,
  };
}
...
function upsertBulletSection(
  ...
  const normalizedBullets = unique(
    bullets.map((bullet) => bullet.trim()).filter((bullet) => bullet.startsWith('- '))
  );
  ...
  let endIndex = lines.findIndex((line, index) => index > startIndex && /^##\s/.test(line.trim()));
  if (endIndex === -1) {
  ...
  const additions = normalizedBullets.filter((bullet) => !existingBullets.includes(bullet));
...
function buildKeywordSignals(tasks: TaskRow[]): KeywordSignal[] {
  ...
  for (const task of tasks) {
    ...
    for (const token of tokens) {
      const current = stats.get(token) || { positive: 0, negative: 0, occurrence: 0 };
  ...
  return Array.from(stats.entries()).map(([keyword, value]) => ({
    ...
    correlation:
      value.occurrence > 0 ? clamp((value.positive - value.negative) / value.occurrence, -3, 3) : 0,
  }));
}
...
class EvolutionService {
  evolveWorkflow(workflowId: string): EvolutionWorkflowSummary {
    const tasks = db.getTasksByWorkflow(workflowId).filter((task) => task.total_score !== null);
    const capabilitySummaries = capabilityRegistry.registerWorkflow(tasks);
    const capabilityMap = new Map(
      capabilitySummaries.map((item) => [item.agentId, item.capabilities.map((row) => row.capability)])
    );
    ...
    for (const [agentId, agentTasks] of tasksByAgent.entries()) {
      ...
      const effectiveKeywords = keywordSignals
        ...
        .sort((a, b) => b.correlation - a.correlation || b.occurrence - a.occurrence)
        .slice(0, 8);
      const ineffectiveKeywords = keywordSignals
        .filter((item) => item.correlation < -0.25)
        .sort((a, b) => a.correlation - b.correlation || b.occurrence - a.occurrence)
        .slice(0, 6);
      ...
      fs.writeFileSync(heartbeatFile, renderHeartbeatMarkdown(agent, heartbeatConfig, capabilities), 'utf-8');
      ...
      summaries.push({
        ...
        capabilities: capabilityMap.get(agentId) || capabilities.map((item) => item.capability),
    ...
    return {
      ...
      totalWeakDimensions: summaries.reduce((sum, item) => sum + item.weakDimensions.length, 0),
      totalLearnedKeywords: summaries.reduce((sum, item) => sum + item.learnedKeywords.length, 0),
      totalCapabilities: summaries.reduce((sum, item) => sum + item.capabilities.length, 0),
      agents: summaries,
    };
  }
}
