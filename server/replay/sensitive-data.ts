 ...
 */
export function encryptMessage(plaintext: string, key: Buffer): EncryptedPayload {
 ...
 */
export function decryptMessage(encrypted: EncryptedPayload, key: Buffer): string {
...
const MASKING_PATTERNS: Array<[RegExp, string]> = [
  ...
  // Email addresses
  [/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g, (match: string) => {
  ...
  [/\b1[3-9]\d[\s-]?\d{4}[\s-]?\d{4}\b/g, (match: string) => {
    const digits = match.replace(/[\s-]/g, '');
    return `${digits.slice(0, 3)}****${digits.slice(-4)}`;
  }],

  // International phone numbers: +xx-xxx-xxx-xxxx or similar
  [/\+\d{1,3}[\s-]?\d{2,4}[\s-]?\d{3,4}[\s-]?\d{4}\b/g, (match: string) => {
    const digits = match.replace(/[\s-+]/g, '');
