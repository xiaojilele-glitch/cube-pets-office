...
function readSummaryIndex(agentId: string): MemorySummary[] {
  return safeParseJson<MemorySummary[]>(
    readAgentWorkspaceFile(agentId, getSummaryFile(), "memory"),
    []
  );
}
...
function buildWorkflowTranscript(entries: SessionEntry[]): string {
  ...
  const earlierPreview = earlierEntries
    ...
    .map(
      entry => `- ${entry.stage || "general"} / ${entry.type}: ${entry.preview}`
    )
    .join("\n");
  ...
  const combined = [
    earlierEntries.length > 0
      ...
      : "",
    recentTranscript
      ? `Most recent key context (${recentEntries.length} items):\n${recentTranscript}`
      : "",
  ]
    .filter(Boolean)
    .join("\n\n");
...
class SessionStore {
  appendEntry(
    agentId: string,
    entry: Omit<SessionEntry, "timestamp"> & { timestamp?: string }
  ): void {
  ...
  getRecentEntries(
    ...
    const entries = workflowId
      ? this.getWorkflowEntries(agentId, workflowId)
      : readJsonLines(agentId, null);
  ...
  searchMemories(
    ...
    const vectorHits = vectorStore.searchMemorySummaries(
      agentId,
      query,
      Math.max(topK * 2, 6)
    );
    ...
    const ranked = Array.from(keywordHits.values()).sort((left, right) => {
      ...
      const rightCombined =
        (right.keywordScore || 0) + (right.vectorScore || 0);
      return (
  ...
  buildPromptContext(
    ...
    if (workflowId) {
      ...
      if (workflowEntries.length > 0) {
        ...
        sections.push(
          `以下是你在当前 workflow 中的完整上下文记录，请延续已有判断、承诺和事实，不要丢失前文：\n${transcript}`
        );
      }
    } else {
        ...
        .map(memory => {
          const scores = [
            typeof memory.keywordScore === "number"
              ? `keyword=${memory.keywordScore}`
              : null,
            typeof memory.vectorScore === "number"
              ? `vector=${memory.vectorScore.toFixed(3)}`
              : null,
          ]
            .filter(Boolean)
            .join(", ");
  ...
  materializeWorkflowMemories(workflowId: string): void {
    ...
    for (const agentId of agentIds) {
      ...
      const agentMessages = messages.filter(
        message =>
          message.from_agent === agentId || message.to_agent === agentId
      );
      ...
      if (agentTasks.length > 0) {
        const taskSummary = agentTasks
          .map(task => {
            const scoreText =
              task.total_score === null ? "未评分" : `${task.total_score}/20`;
      ...
      const summaries = readSummaryIndex(agentId).filter(
