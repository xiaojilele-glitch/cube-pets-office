...
router.post('/heartbeat/:agentId/run', async (req, res) => {
  ...
  try {
    const report = await heartbeatScheduler.trigger(req.params.agentId, 'manual');
    res.json({ report });
  } catch (error: any) {
    res.status(400).json({ error: error?.message || 'Failed to run heartbeat' });
  }
});
...
router.get('/heartbeat/:agentId/:reportId', (req, res) => {
  ...
  const report = reportStore.readHeartbeatReport(req.params.agentId, req.params.reportId);
  if (!report) {
