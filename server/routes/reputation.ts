...
router.post('/admin/reputation/:agentId/adjust', (req, res) => {
  ...
  if (!dimension || delta == null || !reason) {
    res.status(400).json({ error: 'dimension, delta, and reason are required' });
    return;
  }
...
router.get('/admin/reputation/distribution', (_req, res) => {
  ...
  const buckets: Record<string, number> = {
    ...
    '500-599': 0, '600-699': 0, '700-799': 0, '800-899': 0, '900-1000': 0,
  };
  for (const p of profiles) {
    const bucket = p.overallScore >= 900 ? '900-1000'
