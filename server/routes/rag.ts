...
export function createRAGRouter(deps: RAGRouteDeps): Router {
  ...
  router.post("/search", async (req, res) => {
    try {
      ...
      if (!query || !options?.projectId) {
        return res
          .status(400)
          .json({ error: "query and options.projectId required" });
      }
  ...
  router.get("/admin/dlq", async (req, res) => {
    ...
    const entries = await deps.ingestionPipeline.getDeadLetters({
      limit,
      offset,
    });
  ...
  router.post("/admin/dlq/:entryId/retry", async (req, res) => {
    try {
      const result = await deps.ingestionPipeline.retryDeadLetter(
        req.params.entryId
      );
      return res.json(result);
    } catch (err) {
