...
function buildTaskTitle(title: unknown, sourceText: unknown): string | null {
...
export function createTaskRouter(
  ...
  async function buildExecutorLogFallback(
    ...
    const eventsPath = resolveExecutorJobAbsolutePath(
      missionId,
      jobId,
      "events.jsonl"
    );
  ...
  router.post("/:id/operator-actions", async (req, res) => {
    try {
      ...
      if (
        input.action === "retry" &&
        shouldAutoDispatchMission(task, undefined)
      ) {
        const dispatched = await dispatchMissionToExecutor(runtime, task.id, {
          fetchImpl,
          executorBaseUrl: defaultExecutorBaseUrl,
  ...
  router.get("/:id/artifacts", (req, res) => {
    ...
    if (!mission) {
      return res
        .status(404)
        .json({ error: `Mission not found: ${req.params.id}` });
    }
  ...
  router.get("/:id/artifacts/:index/download", async (req, res) => {
    ...
    if (!Number.isInteger(idx) || idx < 0 || idx >= raw.length) {
      return res
        .status(404)
        .json({ error: `Artifact not found at index ${req.params.index}` });
    }
    ...
    const absPath = resolveArtifactAbsolutePath(
      mission.id,
      jobId,
      artifact.path
    );
    ...
    if (artifact.name === "executor.log") {
      const fallbackLog = await resolveExecutorLogFallback(
        mission.id,
        jobId,
        absPath
      );
  ...
  router.get("/:id/artifacts/:index/preview", async (req, res) => {
    ...
    if (!isTextMime(mime)) {
      return res
        .status(415)
        .json({ error: "Binary files cannot be previewed" });
    }
