...
<p align="center">
  ...
  <img alt="specs" src="https://img.shields.io/badge/specs-67%20tracked-0ea5e9" />
  <img alt="status" src="https://img.shields.io/badge/status-office%20cockpit%20V1%20landing-22c55e" />
...
近期已经把 `/tasks` 从“观察执行”推进为“执行控制台”：任务取消、状态操作、blocker、负责人、下一步动作与界面反馈闭环均已落地，任务页已具备更完整的执行协作能力。

同时，桌面端 `/` 正在把办公室升级为默认执行壳：左侧任务队列、中间 `Scene3D`、右侧任务/上下文 tab、统一智能发起入口与底部共享操作区已经收口到同一屏，`/tasks` 继续保留为全屏工作台与深链页。最近还完成了“发起 / 任务操作”收敛第一阶段，`UnifiedLaunchComposer` 已直接承载任务操作 rail，首屏控制区进一步收口。
...
完整的 3D 场景、办公室驾驶舱、动态组织可视化和交互界面——全部在浏览器里运行。
...
下图按 2026-04-15 的项目快照更新：主干仍是前端 / Cube Brain / Intelligence / Trust / Execution / Interop 六层，但前端主路径已经升级为“办公室主壳 + 内嵌任务驾驶舱”。桌面端 `/` 在同一屏里收口任务队列、`Scene3D`、右侧任务/上下文 tab、统一智能发起入口与底部共享操作区，`/tasks` 继续保留为全屏工作台与深链页。
...
├── client/                          # 🖥️ 前端应用
│   └── src/
│       ├── components/              # Scene3D、office、tasks、nl-command、lineage、replay 等 UI
...
| 任务取消端到端       |  ✅  | P0 已完成，取消入口、执行器中断、状态落库与 UI 回显闭环已打通                                 |
| 任务状态操作栏       |  ✅  | P0 已完成，暂停 / 恢复 / 重试 / 标记阻塞 / 终止已统一到任务操作模型                           |
| 任务详情首屏重排     |  ✅  | P1 已完成，主操作、当前负责人、blocker、下一步动作已前置到第一屏                              |
| 执行协作文案收敛     |  ✅  | P1 已完成，界面文案已收口为“开发执行 / 协作交付 / 当前行动”                                   |
| 任务控制台 UI 打磨   |  ✅  | P2 已完成，反馈时机、按钮层级、状态可见性、空态和错误态已统一优化                             |
| 发起 / 任务操作共享区 |  🧪  | `UnifiedLaunchComposer` 已接入底部任务操作 rail，`OfficeTaskCockpit` 与 `/tasks` 已完成接线，当前仅剩桌面 / 窄宽手测 |
| 办公室任务驾驶舱 V1  |  🚧  | 桌面端办公室主壳、三栏驾驶舱、右侧上下文 tab、统一智能发起入口与底部共享操作区已落地，剩余回归、补测与桌面手测 |
...
| `.kiro/specs`   | **67 个目录：前三层主线已基本落地，办公室驾驶舱与桌面体验收口中**                                                  |
| 共享契约        | **14 个 `shared/**/contracts.ts` 模块\*\*                                                                       |
| 测试覆盖        | **300+ 测试文件（Vitest + fast-check）**                                                                        |
| 当前活跃增量    | `office-task-cockpit` 桌面回归与手测 + `launch-operator-surface-convergence` 最终桌面手测 + `workflow-artifacts-display` 最终验收 + `i18n-cleanup` 收口 + 工程基线欠账治理 |
| Commits         | 280+                                                                                                            |
