...
<p align="center">
  ...
  <img alt="specs" src="https://img.shields.io/badge/specs-64%20tracked-0ea5e9" />
  <img alt="status" src="https://img.shields.io/badge/status-41%20done%20%C2%B7%2013%20active-22c55e" />
...
同时，桌面端 `/` 正在把办公室升级为默认执行壳：左侧任务队列、中间 `Scene3D`、右侧任务/上下文 tab 与统一双通道发起入口已经收口到同一屏，`/tasks` 继续保留为全屏工作台与深链页。
...
下图按 2026-04-13 的项目快照更新：主干仍是前端 / Cube Brain / Intelligence / Trust / Execution / Interop 六层，但前端主路径已经升级为“办公室主壳 + 内嵌任务驾驶舱”。桌面端 `/` 在同一屏里收口任务队列、`Scene3D`、右侧任务/上下文 tab 与统一发起入口，`/tasks` 继续保留为全屏工作台与深链页。
...
| 办公室任务驾驶舱 V1  |  🚧  | 桌面端办公室主壳、三栏驾驶舱、右侧上下文 tab 与统一双通道发起已落地，剩余回归、补测与桌面手测 |
...
| `.kiro/specs`   | **64 个目录：41 已完成 / 13 开发中或待回归 / 9 未开始 / 1 待补 `tasks.md`**                                     |
| 共享契约        | **14 个 `shared/**/contracts.ts` 模块\*\*                                                                       |
| 测试覆盖        | **300+ 测试文件（Vitest + fast-check）**                                                                        |
| 当前活跃增量    | `office-task-cockpit` 回归补测 + `workflow-artifacts-display` 最终验收 + `i18n-cleanup` 收口 + 工程基线欠账治理 |
| Commits         | 280+                                                                                                            |
